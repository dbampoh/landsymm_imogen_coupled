...
// MF: SPITFIRE Classes.  This is not such a pretty way to do this...
#include "fuel.h"
...
typedef enum {RAINFED, IRRIGATED_WILT, IRRIGATED_SAT, INUNDATED} hydrologytype;
...
#ifdef GCP
#define npftconst 128  // setup for a max of 64, ToDo: need to use some std::vector<...> and maybe resize
#endif
...
public:
	...
	*/
	...
	bool write_outputs(int year_offset = 0) {
		return (get_calendar_year() + year_offset)>=firstoutyear && (get_calendar_year() + year_offset)<=lastoutyear;
	}
	
	// SSR
	/// Return TRUE if this is the first non-spinup year OR it's the first year after a restart
...
public:
	...
	// SPITFIRE - for prescribing burnt area
	// WARNING: This code is not finished (or even working) so these variables are essentially placeholders
	//          At this stage I wouldn't even vouch for the basic strategy, some more probabilistic approach
	//          based on RoS or fire intensity might be better

	//number of fires (prescribed from data)
	double burntarea[365];

	// number of iterations required to allocate fire to the patches
	double monthly_mean_num_iterations[12];

	// the number of fails in the allocation procedure
...
public:
	...
	enum PerPFTFluxType {
		...
		MT_TBOC,
		MT_OTHR,
		/// Burnt area fraction (1), used for SPITFIRE
		BA,
		/// C flux from fires, used for SPITFIRE
		C_FIRE,
#ifdef GCP
		/// LAI (need to div by days per time period to get time average, later)
		LAI,
		/// daily tracked cmass wood (need to div by days per time period to get time average, later)
		CMASS_WOOD,
		/// daily tracked cmass leaf (need to div by days per time period to get time average, later)
		CMASS_LEAF,
		/// daily tracked cmass root (need to div by days per time period to get time average, later)
		CMASS_ROOT,
		/// daily tracked cmass veg (need to div by days per time period to get time average, later)
		CMASS_VEG,
		/// Flux from atmosphere to vegetation associated with establishment (kgC/m2) //Report ESTC by PFT - TP 31.08.15
		PESTC,
		/// Evapotranspiration (does not include soil evaporation) (mm)
		AET,
...
#ifdef GCP
	/// Stores one flux value per PFT and flux type per month 
...
public:
	...
	double nyears;
	/// hydrology (RAINFED,IRRIGATED_WILT,IRRIGATED_SAT,INUNDATED) 
...
//	double firr;
	...
	// SSR
	bool isforpotyield;
	double N_appfert_mt; // the amount of N that is applied (kg N m-2); overrides Pft N_appfert
...
//		firr = 0.0;
	...
	bool is_managed() {
		...
		if(pftname != "" || planting_system != "" || selection != ""||  harvest_system != "" ||  hydrology > RAINFED || fallow || nfert > -1.0)
			return true;
		else
			return false;
	}
...
public:
	...
	// MF SPITFIRE required PFT parameters
	double dens_fuel;							//fuel bulk density kg/m2
	double em_CO2; 								//em_CO2  emission factor CO2
	double em_CO;								//em_CO  emission factor CO
	double em_CH4;								//em_CH4  emission factor CH4
	double em_VOC;								//em_VOC  emission factor VOC
	double em_TPM;								//em_TPM  emission factor TPM
	double em_NOx;								//em_NOx  emission factor NOx
	double crown_l;								//crown_l  proportion of tree height as crown
	double flame;								//flame  f parameter for flame length
	double barka;								//barka  param1 for bark thickness
	double barkb;								//barkb  param2 for bark thickness
	double r_ck;								//r_ck  r(ck) crown damage: postfire mortality
	double p;									//p   crown damage: postfire mortality
	double MoE;									//Moisture of Extinction
	double sigma_leaf;							//surface area to volume of leaves (cm^2/cm^3)
	// end SPITFIRE required PFT parameter

	// DKB 30-07-2024 N_harvest_scale Pft based
...
public:
	...
	Pft() {
		...
		// SPITFIRE
		sigma_leaf=66.0; // original value Thonicke et al 2010
		// SSR: Adding default values; if any of these are unset and firemodel==SPITFIRE, run will abort.
		dens_fuel = -99999.0;
		em_CO2 = -99999.0;
		em_CO = -99999.0;
		em_CH4 = -99999.0;
	...
	// SSR: Have to do it this way instead of "if !itemparsed()" because, in parameters.cpp, PFT "if !itemparsed()" checks happen before firemodel is set.
	/// Checks whether invalid SPITFIRE value is present, aborting with a clear error message if not.
	void check_spitfire_pft_settings() {
		if (dens_fuel < -9999.0) {
			fail("You must supply dens_fuel for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_CO2 < -9999.0) {
			fail("You must supply em_CO2 for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_CO < -9999.0) {
			fail("You must supply em_CO for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_CH4 < -9999.0) {
			fail("You must supply em_CH4 for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_NOx < -9999.0) {
			fail("You must supply em_NOx for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_VOC < -9999.0) {
			fail("You must supply em_VOC for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (em_TPM < -9999.0) {
			fail("You must supply em_TPM for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (crown_l < -9999.0) {
			fail("You must supply crown_l for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (dens_fuel < -9999.0) {
			fail("You must supply dens_fuel for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (flame < -9999.0) {
			fail("You must supply flame for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (barka < -9999.0) {
			fail("You must supply barka for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (barkb < -9999.0) {
			fail("You must supply barkb for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (r_ck < -9999.0) {
			fail("You must supply r_ck for every PFT in pftlist (missing from %s)", (char*)name) ;
		} else if (MoE < -9999.0) {
			fail("You must supply MoE (parameter name 'inflame') for every PFT in pftlist (missing from %s)", (char*)name) ;
		}
	}
...
//	bool issecondarycrop;
	...
	cropindiv_struct() {
		...
		harv_nmass_leaf=0.0;
		harv_nmass_root=0.0;
		harv_nmass_root=0.0; // MF extra
		harv_nmass_ho=0.0;
		harv_nmass_agpool=0.0;
		nmass_dead_leaf_luc = 0.0;
		nmass_agpool_luc = 0.0;
		nmass_ho_luc = 0.0;
...
public:
	...
	Sompool() {
		...
		// tracking of fuel characteristic properties (doesn't include 1000hr fuel)
		fuelbulkdensity = 0.0;
		surfacetovolume = 0.0;
		moistureofextinction = 0.25;

		// tracking of emissions factors (*does* include 1000hr fuel)
		em_CO2 = 0.0;
		em_CO = 0.0;
		em_CH4  = 0.0;
		em_TPM = 0.0;
	...
	/// litter characteristic fuel bulk density (doesn't include 1000hr fuel)
	double fuelbulkdensity;
	/// litter characteristic surface area to volume ratio (doesn't include 1000hr fuel)
	double surfacetovolume;
	/// litter characteristic moisture of extinction (doesn't include 1000hr fuel)
	double moistureofextinction;


	// tracking of emissions factors
	double	em_CO2 ;
	double	em_CO;
	double	em_CH4;
	double	em_TPM;
	double	em_VOC;
	// MF: NOTE NOx is currently doing via the "standard way" as per BLAZE, its emission is ignored and so not tracked for the SOM pools
...
public:
	...
	// SSR: GGCMI: Soil moisture during growing season
...
// CENTURY SOM pools and other variables
	...
	// SSR: GGCMI
	/// growing-season leaching from available nitrogen pool
	double sminleach;
...
public:
	...
	// SSR: Whether to enable certain hydrological processes
	bool do_saturate();
...
public:
	...
	/// SPITFIRE VARIABLES
	// WARNING:  These are not currently being calculated or accessed!

	// Total fuel consumed for this PFT
	double annual_fuel_consumed_gC;

	// FPC - for saving burnt fractions on a per-PFT basis
	double fpc;

	//  Total area of this PFT burned per month
...
public:
	...
	/// Rainfed, irrigated (to wilting point or saturation), or inundated?
	hydrologytype hydrology;
    ...
    double grs_w_transp;
    double grs_w_runoff;
    /// growing season's nitrogen fertilization (kgN/m2/season)
    double snfert;
	double semis_n2o;
	double semis_n2;
	double semis_c;
	...
	// MF: SPITFIRE fuel object for this patch
	Fuel_patch fuel;

	// MF: SPITFIRE (put this here and not in Fuel class so it can be easily serialised)
	int days_since_last_burn;

	// MF: Scaling factor for this patch to the gridcell level, only taking into account land cover types that can burn
	// This us updated on the first day of the year, after landcover change.  This means that spitfire_dailyaccounting()
	// must be run after landcover_dynamics() in simulate_day()
	double to_gridcell_average_flammable_only;

	// MF: SPITFIRE calculated and applied monthly burnt fractions
	// monthly burned fraction as calculated by SPITFIRE for output [ratio] -- DEPRECATED
	double mfirefrac[12];
	// monthly burned fraction resulting from stochastic determination output [ratio] -- maybe move somewhere else??
	...
	void print_all_members() const {
		//std::cout << "Patch ID: " << id << std::endl;
		//std::cout << "Stand Reference ID: " << stand.id << std::endl; // Assuming `stand` has an `id` member

		//std::cout << "Number of Patchpft objects: " << pft.size() << std::endl;

		std::cout << "fpar_grass: " << fpar_grass << std::endl;
		std::cout << "fpar_ff: " << fpar_ff << std::endl;
		std::cout << "par_grass_mean: " << par_grass_mean << std::endl;
		std::cout << "nday_growingseason: " << nday_growingseason << std::endl;
		std::cout << "fpc_total: " << fpc_total << std::endl;
		std::cout << "disturbed: " << disturbed << std::endl;
		std::cout << "age: " << age << std::endl;
		std::cout << "fireprob: " << fireprob << std::endl;
		std::cout << "burned: " << burned << std::endl;
		std::cout << "fire_line_intensity: " << fire_line_intensity << std::endl;
		std::cout << "rate_of_spread: " << rate_of_spread << std::endl;
		std::cout << "wood_to_atm: " << wood_to_atm << std::endl;
		std::cout << "leaf_to_atm: " << leaf_to_atm << std::endl;
		std::cout << "leaf_to_lit: " << leaf_to_lit << std::endl;
		std::cout << "wood_to_str: " << wood_to_str << std::endl;
		std::cout << "wood_to_fwd: " << wood_to_fwd << std::endl;
		std::cout << "wood_to_cwd: " << wood_to_cwd << std::endl;
		std::cout << "litf_to_atm: " << litf_to_atm << std::endl;
		std::cout << "lfwd_to_atm: " << lfwd_to_atm << std::endl;
		std::cout << "lcwd_to_atm: " << lcwd_to_atm << std::endl;

		for (int i = 0; i < N_YEAR_BIOMEAVG; i++) {
			std::cout << "fapar_grass_avg[" << i << "]: " << fapar_grass_avg[i] << std::endl;
			std::cout << "fapar_ndlt_avg[" << i << "]: " << fapar_ndlt_avg[i] << std::endl;
			std::cout << "fapar_brlt_avg[" << i << "]: " << fapar_brlt_avg[i] << std::endl;
			std::cout << "fapar_trbr_avg[" << i << "]: " << fapar_trbr_avg[i] << std::endl;
			std::cout << "fapar_shrub_avg[" << i << "]: " << fapar_shrub_avg[i] << std::endl;
			std::cout << "fapar_total_avg[" << i << "]: " << fapar_total_avg[i] << std::endl;
		}

		std::cout << "managed: " << managed << std::endl;
		std::cout << "man_strength: " << man_strength << std::endl;
		std::cout << "managed_this_year: " << managed_this_year << std::endl;
		std::cout << "plant_this_year: " << plant_this_year << std::endl;
		std::cout << "growingseasondays: " << growingseasondays << std::endl;

		std::cout << "intercep: " << intercep << std::endl;
		std::cout << "aaet: " << aaet << std::endl;
		std::cout << "aevap: " << aevap << std::endl;
		std::cout << "aintercep: " << aintercep << std::endl;
		std::cout << "asurfrunoff: " << asurfrunoff << std::endl;
		std::cout << "adrainrunoff: " << adrainrunoff << std::endl;
		std::cout << "abaserunoff: " << abaserunoff << std::endl;
		std::cout << "arunoff: " << arunoff << std::endl;
		std::cout << "wetland_water_added_today: " << wetland_water_added_today << std::endl;
		std::cout << "awetland_water_added: " << awetland_water_added << std::endl;
		std::cout << "apet: " << apet << std::endl;
		std::cout << "eet_net_veg: " << eet_net_veg << std::endl;
		std::cout << "wdemand: " << wdemand << std::endl;
		std::cout << "wdemand_day: " << wdemand_day << std::endl;
		std::cout << "wdemand_leafon: " << wdemand_leafon << std::endl;
		std::cout << "fpc_rescale: " << fpc_rescale << std::endl;

		for (int i = 0; i < 12; i++) {
			std::cout << "maet[" << i << "]: " << maet[i] << std::endl;
			std::cout << "mevap[" << i << "]: " << mevap[i] << std::endl;
			std::cout << "mintercep[" << i << "]: " << mintercep[i] << std::endl;
			std::cout << "mrunoff[" << i << "]: " << mrunoff[i] << std::endl;
			std::cout << "mpet[" << i << "]: " << mpet[i] << std::endl;
		}

		std::cout << "ndemand: " << ndemand << std::endl;
		std::cout << "anfert: " << anfert << std::endl;
		std::cout << "dnfert: " << dnfert << std::endl;
		std::cout << "hydrology type: " << hydrology << std::endl;
		std::cout << "irrigation_d: " << irrigation_d << std::endl;
		std::cout << "irrigation_y: " << irrigation_y << std::endl;
		std::cout << "is_litter_day: " << is_litter_day << std::endl;
		std::cout << "nharv: " << nharv << std::endl;
		std::cout << "isharvestday: " << isharvestday << std::endl;

		std::cout << "grs_n_uptake: " << grs_n_uptake << std::endl;
		std::cout << "grs_n_input: " << grs_n_input << std::endl;
		std::cout << "grs_n_losses: " << grs_n_losses << std::endl;
		std::cout << "grs_w_irr: " << grs_w_irr << std::endl;
		std::cout << "grs_w_evapo: " << grs_w_evapo << std::endl;
		std::cout << "grs_w_intercep: " << grs_w_intercep << std::endl;
		std::cout << "grs_w_transp: " << grs_w_transp << std::endl;
		std::cout << "grs_w_runoff: " << grs_w_runoff << std::endl;
		std::cout << "snfert: " << snfert << std::endl;
		std::cout << "semis_n2o: " << semis_n2o << std::endl;
		std::cout << "semis_n2: " << semis_n2 << std::endl;
		std::cout << "semis_c: " << semis_c << std::endl;
		std::cout << "grs_w_irr_lastyear: " << grs_w_irr_lastyear << std::endl;
		std::cout << "grs_w_irr_thisyear: " << grs_w_irr_thisyear << std::endl;

		std::cout << "days_since_last_burn: " << days_since_last_burn << std::endl;
		std::cout << "to_gridcell_average_flammable_only: " << to_gridcell_average_flammable_only << std::endl;

		for (int i = 0; i < 12; i++) {
			std::cout << "mfirefrac[" << i << "]: " << mfirefrac[i] << std::endl;
			std::cout << "mfirefrac_applied[" << i << "]: " << mfirefrac_applied[i] << std::endl;
		}
	}
...
public:
	...
	/// This PFT's irrigation/inundation in this stand
	hydrologytype hydrology;
	///
    ...
    /// SSR: GGCMI: phu sum specified in stand type or read from input file
    double phu_force;
    /// SSR: GGCMI: pvd sum specified in stand type or read from input file
    int pvd_force;
	/// SSR: GGCMI: like hdate_force, but instead of day of year, units length of season (1-based)
	int growseaslength_force;
	/// SSR: GGCMI: 2nd fertilization date read from input file
...
public:
	...
	/// Returns current crop management's hydrology, updated during rotation
	hydrologytype hydrology;
...
public:
    ...
    /// SSR: GGCMI: phu sum from input file
    double phu_force;
    /// SSR: GGCMI: pvd sum from input file
    int pvd_force;
	/// SSR: GGCMI: like hdate_force, but instead of day of year, units length of season (1-based)
	int growseaslength_force;
	/// SSR: GGCMI: 2nd fertilization date read from input file
	...
	// BLAZE PFT params - MF moved from Pft class
...
public:
	...
	// BLAZE-FireMIP
	// Monthly rate-of-spread weighted by burned area
	...
	// SPITFIRE
	/// lightning strokes (count/km^2/day)
	double lightning;

	/// The tendency for people to start fires, a(Nd), for SPITFIRE
	double aNd;
