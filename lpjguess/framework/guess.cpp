...
void Patch::serialize(ArchiveStream& arch) {
	...
	arch & vegetation
		...
		& semis_c
		& snfert
		& days_since_last_burn;  // SPITFIRE ONLY
...
bool Patch::has_fires() const {
	...
	// For BLAZE AND GlobFIRM just reproduce above logic
	if(firemodel == BLAZE || firemodel == GLOBFIRM) {
		return stand.landcover != CROPLAND && stand.landcover != PEATLAND && !managed &&
			(stand.landcover != PASTURE || disturb_pasture);
	}
	// For SPITFIRE have more explicit switching
	...
	// Catch other case
		...
		dprintf("Please define a clause for this new fire mode in has_fires().");
		abort();
	}
}
...
bool Patch::can_establish(Pft& pft) const {
	...
	// Is there too little snow for this PFT to establish?
	double snow_djf = (soil.msnowdepth[0] + soil.msnowdepth[1] + soil.dec_snowdepth) / 3.0;
//	bool too_little_snow = snow_djf<pft.min_snow;
	double snow_jja = (soil.msnowdepth[5] + soil.msnowdepth[6] + soil.msnowdepth[7]) / 3.0;
	bool too_little_snow = (climate.lat >= 0.0 && snow_djf < pft.min_snow) || (climate.lat < 0.0 && snow_jja < pft.min_snow);
	
	return !(too_little_snow ||
			 climate.agdd0_20.mean() <= pft.gdd0_min || climate.agdd0_20.mean() >= pft.gdd0_max);
}
...
void Stand::init_stand_lu(StandType& st, double fraction) {
	...
	if (mt0.hydrology > RAINFED) {
		hydrology = mt0.hydrology;								// First main crop, may change during crop rotation
		if (pftid >= 0)
...
void Stand::rotate() {
	...
	if (pftid >= 0 && stid >= 0) {
		...
		if (mt.hydrology > RAINFED) {
			hydrology = mt.hydrology;
			standpft.hydrology = mt.hydrology;
		}
		else {
			hydrology = RAINFED;
			standpft.hydrology = RAINFED;
		}
...
Gridcell::Gridcell():climate(*this) {
	...
	seed = 12345678;
	for (int m=0;m<12;m++) {
		monthly_burned_area[m] = 0.0;
		monthly_fire_risk[m] = 0.0;
	     // FireMIP
	     blaze_monthly_weighted_fli[m] = 0.;
	     blaze_monthly_total_no_fires[m]= 0;
	     blaze_monthly_mean_no_fires[m] = 0.;
	     blaze_monthly_firetreemortality[m] = 0.;
	     blaze_monthly_weighted_mean_fire_size[m] = 0.;
	     blaze_monthly_weighted_ros [m] = 0.;
	     for (int fc = 0; fc < 4; fc++) {
	          blaze_monthly_C_fuelClas [m][fc] = 0.;
	          blaze_monthly_CC_fuelClas[m][fc] = 0.;
	     }
	}
	burned_area = 0.0;
}
...
void Sompool::serialize(ArchiveStream& arch) {
	arch & cmass
		...
		& fireresist
		& mfracremain_mean
		& surfacetovolume // SPITFIRE
		& fuelbulkdensity // SPITFIRE
		& moistureofextinction // SPITFIRE
		& em_CO // SPITFIRE
		& em_CO2 // SPITFIRE
		& em_CH4 // SPITFIRE
		& em_VOC // SPITFIRE
...
void MassBalance::check_year_N(Gridcell& gridcell) {
	...
	if (date.year == start_year) {
		...
		// MF - also set nflux to zero at the same point as setting the ncont_zero, otherwise we see fallacious period imbalances after a restart
		nflux = 0.0;
	}
	else {
...
void MassBalance::check_year_C(Gridcell& gridcell) {
	...
	if (date.year == start_year) {
		...
		// MF - also set cflux to zero at the same point as setting the ccont_zero, otherwise we see fallacious period imbalances after a restart
		cflux = 0.0;
	}
	else {
