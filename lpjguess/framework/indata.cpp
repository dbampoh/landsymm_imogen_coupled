...
// SSR: For testing whether mapfile was created after text file
#ifdef __APPLE__
#include <sys/types.h>
#include <sys/stat.h>
#include <time.h>
...
// SSR: GGCMI: Changed to ins-file parameter lutomemory
...
double TimeDataD::Get(int calender_year, const char* name, bool allow_missing_warnfirstcell, bool is_first_cell) const {
...
//		if(calender_year == firstyear)
		if((firstyear == -1 && date.year==0) || date.is_firsthist_or_restart_year()) {	// SSR
			// SSR: Warn OR fail
			if (allow_missing_warnfirstcell && is_first_cell) {
				dprintf("WARNING: Value for %s not found in %s. Will not warn on subsequent cells.\n", name, fileName);
			} else if (!allow_missing_warnfirstcell) {
				fail("Failing: Value for %s not found in %s.\n", name, fileName);
			}
		}
		return NOTFOUND;
	}
...
void TimeDataD::CreateFileMap() {
	...
	if(ifp_map) {
		...
		// SSR: Setting up for second test of whether mapfile is out of date
		bool mapfile_is_outdated = false;
		
		// SSR: (This was here before.) Test whether mapfile is out of date based on expected number of gridcells (?).
		fseek(ifp_map, 0 ,SEEK_END);
		lSize = ftell(ifp_map);
		rewind(ifp_map);
		mapfile_is_outdated = lSize != nCells * sizeof(CoordPos);
		
		// SSR: Test whether mapfile was created after text file.
#ifdef __APPLE__
	...
	if(!ifp_map) {
		dprintf("Mapping text data file (%s).\n", fileName);
		rewind(ifp);
...
//		if(calender_year == firstyear) {	// firstyear set to -1 for static inputs
		if(calender_year == firstyear || (firstyear == -1 && date.year==0)) {	// SSR: GGCMI
