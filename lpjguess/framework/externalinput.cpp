...
LandcoverInput::LandcoverInput()
	: nyears_cropland_ramp(0),
	  firstfullyear_crop(-1), nyears_lu_ramp(0), firstfullyear_lu(-1), force_linear_ramp(0), // SSR: LandSyMM (from g2p in old codebase)
      start_lu_in_first_filelu_year(0) { // SSR: LandSyMM
	...
	// SSR: LandSyMM (from g2p in old codebase)
	declare_parameter("nyears_lu_ramp", &nyears_lu_ramp, 0, 10000, "Number of years to increase LU fractions linearly from 0 to first year's values");
	declare_parameter("firstfullyear_lu", &firstfullyear_lu, -1, 10000, "What is the first year (calendar) that should have full LU areas (if nyears_lu_ramp>0)? Default (-1) as old behavior for nyears_cropland_ramp: if file_lu has years, then first year in file_lu, otherwise firsthistyear.");
		  
	// SSR: LandSyMM (from g2p in old codebase): Added second sentence.
	declare_parameter("nyears_cropland_ramp", &nyears_cropland_ramp, 0, 10000, "Number of years to increase cropland fraction linearly from 0 to first year's value. Only applies if nyears_lu_ramp unspecified or zero.");

	// SSR: LandSyMM (from g2p in old codebase)
	declare_parameter("firstfullyear_crop", &firstfullyear_crop, -1, 10000, "What is the first year (calendar) that should have full cropland area (if nyears_cropland_ramp>0)? Default (-1) as old behavior: if file_lu has years, then first year in file_lu, otherwise firsthistyear.");
		  
	// SSR: LandSyMM (from g2p in old codebase)
	declare_parameter("force_linear_ramp", &force_linear_ramp, "Should ramp happen linearly to firstyear (TRUE, new behavior), or be a linear *fraction reduction* but potentially a nonlinear actual trend (FALSE, old behavior)?");
		  
    // SSR: LandSyMM
    declare_parameter("start_lu_in_first_filelu_year", &start_lu_in_first_filelu_year, "Should we ignore all LU ramp settings and just begin land use with the first year in file_LU??");
...
void LandcoverInput::init() {
	...
	// SSR: LandSyMM (from g2p in old codebase)
	if (nyears_cropland_ramp == 0 && firstfullyear_crop >= 0) {
		dprintf("firstfullyear_crop setting (%d) ignored because nyears_cropland_ramp==0\n", firstfullyear_crop);
	}
	if (nyears_lu_ramp >= 0 && nyears_cropland_ramp >= 0) {
		dprintf("Warning: ramped all-LU and crop-only requested; will ramp all LU.\n");
	}
...
bool LandcoverInput::loadlandcover(double lon, double lat) {
	...
	if (!lcfrac_fixed) {
		...
		if (loadLU) {
			...
			if (!LUdata.Load(c)) {
				//fail("Problems with %.3f,%.3f in landcover fractions input file. Failing\n",c.lon,c.lat); // SSR GGCMI
				dprintf("Problems with %.3f,%.3f in landcover fractions input file.\n", c.lon, c.lat); // DKB
	...
	for(int lc=0; lc<NLANDCOVERTYPES && !LUerror; lc++) {
		...
		if(run[lc] && !frac_fixed[lc]) {
			...
			if(!st_data[lc].Load(c)) {
				fail("Problems with %.3f,%.3f in stand type fractions input file. Failing\n",c.lon,c.lat); // SSR GGCMI
				LUerror = true;	// skip this stand
			}
		}
	}
	...
	if(lcfrac_fixed) {		// Set land covers to equal fractions
		...
		if(getLU) {	
			...
			if(LUdata.Get(year, 0) < 0.0			// Missing data (negative values)
			   || waiting_for_first_filelu_year) {	// SSR: LandSyMM
		...
		if(nyears_lu_ramp || nyears_cropland_ramp) {
			...
			else if (!nyears_lu_ramp && nyears_cropland_ramp && firstfullyear_crop >= 0) {
			...
			if(doramp) {
				...
				// Sanity check
				if (max_ramp_years <= 0) {
					dprintf("doramp:       max_ramp_years %d\n", max_ramp_years);
					dprintf("                   firstyear %d\n", firstyear);
					dprintf("        first_reduction_year %d\n", first_reduction_year);
					dprintf("         first_historic_year %d\n", first_historic_year);
					dprintf("                  freenyears %d\n", freenyears);
					dprintf("                nyear_spinup %d\n", nyear_spinup);
					dprintf("        SOLVESOMCENT_SPINEND %g\n", SOLVESOMCENT_SPINEND);
					if (first_historic_year > firstyear) {
						fail("max_ramp_years <= 0. Try increasing nyear_spinup. (This might only be a problem because firstyear [first year of full LU] is < first_historic_year [first simulation year after spinup].)");
					} else {
						fail("max_ramp_years <= 0. Try increasing nyear_spinup.");
					}
				}
				...
				if (nyears_ramp > max_ramp_years && year_saved == first_reduction_year)
					dprintf("Requested LU ramp period (%d years) too long for given nyear_spinup. Maximum is %d; will use this.\n", nyears_ramp, max_ramp_years);
				
				// Get target value for firstyear (first year with no reduction)
				double firstyear_urban, firstyear_cropland, firstyear_pasture, firstyear_forest, firstyear_natural;
				firstyear_urban = max(0.0, LUdata.Get(firstyear, "URBAN", true));
				firstyear_cropland = max(0.0, LUdata.Get(firstyear, "CROPLAND", true));
				firstyear_pasture = max(0.0, LUdata.Get(firstyear, "PASTURE", true));
				firstyear_forest = max(0.0, LUdata.Get(firstyear, "FOREST", true));
				firstyear_natural = max(0.0, LUdata.Get(firstyear, "NATURAL", true));

				// Move to NATURAL (not PEATLAND or BARREN, to match previous assumption for nyears_cropland_ramp)
				double frac2reduce = min((double)(firstyear - year_saved) / min(nyears_ramp, max_ramp_years), 1.0);
				double reduce_urban, reduce_cropland, reduce_pasture, reduce_forest;
				reduce_urban = reduce_cropland = reduce_pasture = reduce_forest = 0.0;
				if (force_linear_ramp) {
					reduce_cropland = frac2reduce * firstyear_cropland;
					lc.frac[CROPLAND] = firstyear_cropland - reduce_cropland;
					if (nyears_lu_ramp) {
						reduce_urban = frac2reduce * firstyear_urban;
						reduce_pasture = frac2reduce * firstyear_pasture;
						reduce_forest = frac2reduce * firstyear_forest;
					...
					reduce_cropland = frac2reduce * lc.frac[CROPLAND];
					lc.frac[CROPLAND] -= reduce_cropland;
					if (nyears_lu_ramp) {
						reduce_urban = frac2reduce * lc.frac[URBAN];
						reduce_pasture = frac2reduce * lc.frac[PASTURE];
						reduce_forest = frac2reduce * lc.frac[FOREST];
				...
				if (year_saved == firstyear - 1) {
					double firstyear_toprint;
					xtring whichlu;
					if (nyears_lu_ramp) {
						firstyear_toprint = firstyear_urban + firstyear_cropland + firstyear_pasture + firstyear_forest;
						whichlu = "URBAN+CROPLAND+PASTURE+FOREST";
					} else {
						firstyear_toprint = firstyear_cropland;
						whichlu = "CROPLAND";
					}
					if (force_linear_ramp) {
						dprintf("%s fraction ramp (forced linear) from 0 to %.6f during period %d to %d\n", (const char*)whichlu, firstyear_toprint, max(first_reduction_year, firstyear - nyears_ramp), firstyear
						);
					} else {
						dprintf("%s fraction ramp from 0 to %.6f during period %d to %d\n", (const char*)whichlu, firstyear_toprint, max(first_reduction_year, firstyear - nyears_ramp), firstyear
						);
					}
				}
			}
		...
		for(int y=year;y<=max(year,last_data_year) + 1;y++) {
			...
			for(int i=0; i<nst; i++) {
				if(stlist[i].landcover == CROPLAND)	{
					...
					double cropfrac = CFTdata.Get(y,stlist[i].name,stlist[i].get_management().isforpotyield,gridcell.is_first_gridcell);
...
void ManagementInput::init() {
	...
	if(run[CROPLAND]) {
        ...
        if (readphu || readpvd) {
            if (ifcalcdynamic_phu) {
                if (readphu && readpvd) {
                    ...
                    fail("Input file specified for PHU, but also ifcalcdynamic_phu==TRUE.");
                } else {
                    fail("Input file specified for PVD, but also ifcalcdynamic_phu==TRUE.");
                }
            } else if (readphu != readpvd){
                dprintf("readphu=%d but readpvd=%d. This is okay, but be aware.", readphu, readpvd);
            }
        }
		...
		if(file_Nfertdate2_in != "")	{
			...
			fail("ggcmi2 is true, which means you must specify file_Nfertdate2_in");
		}
	}
...
bool ManagementInput::loadmanagement(double lon, double lat) {
	...
	if(readsowingdates) { 
		if(!sdates.Load(c)) {
			fail("Problems with %.3f,%.3f in sowing date input file. Failing.\n\n", c.lon, c.lat);
			LUerror = true;	// skip this stand
		}
	}
	...
	if(readharvestdates && !LUerror) {
		if(!hdates.Load(c)) {
			fail("Problems with %.3f,%.3f in harvest date input file. Failing.\n\n", c.lon, c.lat);
			LUerror = true;	// skip this stand
		}
	}
	...
	if(readNfert && !LUerror) {
		if(!Nfert.Load(c)) {
			fail("Problems with %.3f,%.3f in N fertilization input file. Failing.\n\n", c.lon, c.lat);
			LUerror = true;	// skip this stand
	...
	if(readNman && !LUerror) {
		if(!NfertMan.Load(c)) {
			fail("Problems with %.3f,%.3f in manure data input file. Failing.\n\n", c.lon, c.lat);
			LUerror = true;	// skip this stand
		}
	}
	if(readNfert_st && !LUerror) {
		if(!Nfert_st.Load(c)) {
			fail("Problems with %.3f,%.3f in N fertilization (stand) input file. Failing.\n\n", c.lon, c.lat);
			LUerror = true;	// skip this stand
...
void ManagementInput::getNfert(Gridcell& gridcell) {
	...
	// SSR: GGCMI
	if (fixed_nfert==1) {
		year = fixed_nfert_year;
	} else if (fixed_nfert==2) {
		year = min(year, fixed_nfert_year);
	} else if (fixed_nfert==3) {
		year = max(year, fixed_nfert_year);
	}

	if(Nfert.isloaded()) {
		for(int i=0; i<npft; i++)	{
			if(pftlist[i].phenology == CROPGREEN) {		
				gridcell.pft[pftlist[i].id].Nfert_read = Nfert.Get(year,pftlist[i].name,pftlist[i].isforpotyield,gridcell.is_first_gridcell)*Nfert_scale_factor;
				if(NfertMan.isloaded()) {
					gridcell.pft[pftlist[i].id].Nfert_man_read = NfertMan.Get(year,pftlist[i].name,pftlist[i].isforpotyield,gridcell.is_first_gridcell);
				}
			}
		}
	}
	...
	if(Nfert_st.isloaded()) {
		for(int i=0; i<nst; i++)	{
			gridcell.st[stlist[i].id].nfert = Nfert_st.Get(year,stlist[i].name,stlist[i].get_management().isforpotyield,gridcell.is_first_gridcell)*Nfert_scale_factor;
		}
	}
}
	...
	for(int i=0; i<npft; i++)	{
		if(pftlist[i].phenology == CROPGREEN) {
			...
			for(unsigned int j=0; j<gridcell.nbr_stands(); j++) {
				Standpft& standpft = gridcell[j].pft[i];
				if(standpft.active)
					standpft.growseaslength_force = gridcell.pft[i].growseaslength_force;
			}
		}
	}
}
...
void ManagementInput::getNfertdate2(Gridcell& gridcell) {
	...
	for(int i=0; i<npft; i++)	{
		if(pftlist[i].phenology == CROPGREEN) {
			...
			if (gridcell.pft[pftlist[i].id].Nfertdate2_force < 0) {
				fail("Negative value found in file_nfertdate2_in");
			}
			...
			for(unsigned int j=0; j<gridcell.nbr_stands(); j++) {
				Standpft& standpft = gridcell[j].pft[i];
				if(standpft.active)
					standpft.Nfertdate2_force = gridcell.pft[i].Nfertdate2_force;
			}
		}
	}
}
...
void ManagementInput::getmanagement(Gridcell& gridcell) {
	...
	if(run[CROPLAND]) {
        ...
        // SSR: GGCMI
        //Read phu sums from input file, put into gridcellpft.phu_force
        if(readphu)
            getphu(gridcell);
        //Read pvd sums from input file, put into gridcellpft.pvd_force
        if(readpvd)
            getpvd(gridcell);
		//Read growing season legnth from input file, put into gridcellpft.growseaslength_force
        if(readgrowseaslength)
            getgrowseaslength(gridcell);
		//Read 2nd fertilization date from input file, put into gridcellpft.Nfertdate2_force
        if(readNfertdate2)
