...
int fire_popdens_method = 2; // default file_popdens
...
bool restart;
bool save_state;
bool lutomemory; // SSR: GGCMI
int save_year = -9999;
int restart_year = -9999;
int verbosity;

// SSR
int firsthistyear = -1;
int lasthistyear = -1;
int firstoutyear = -1; // First simulation year has some weird results sometimes (too high?). Set this to the year after the first simulation year.
int lastoutyear = -1;  // Last simulation year sometimes does not have a harvest because it would take place after the new year. Set this to the year before the last simulation year.

// SSR (moved from hard-coding into input code files): Minimum number of sec to wait between progress messages
int mutesec = 20; // Default value from old input code files
...
int fixed_nfert=0;
int fixed_nfert_year=-1;
bool just_phu_pvd=false;
double manure_cn=30.0;
double manure_organic_frac=1.0;
std::queue<int> save_years;
std::queue<int> save_years_remaining;
...
///////////////////////////////////////////////////////////////////////////////////////
// MF: SPITFIRE
// SSR: Added default values (will warn if not parsed in input files)

bool ifburnfullcell = true; 						// whether to burn the full cell interpreting the burnedarea ratio as a probability
bool ifnesterovdistr = true;              		 	// Whether to distribute the monthly fires using probabilites proportional to daily Nesterov (0,1)
double firereturninterval; 		   			// fire return interval for fixed or random fire return regimes
double pixeldegree = 0.5; 						// size of a pixel, used in fire
double sapsizedecrease = 1.0;          			// in gap models Sapling size is very high and might be already outside the flame zone this gives the number to divide the sapsize by
int fixburnday = 150;					   			// ! if larger than -1 prescribed fire will be applied on this day in the year
double fuelthreshold = 0.2;						// minimum fuel required for a ptach to burn
double mouillotmultiplier = 1.0;					// multiplier for the Mouillot data as this data is not totally trustedminimum fuel required for a ptach to burn
bool ifPandRresidencetime = false;          		// whether to use Peterson and Ryan for residence time (default is residence time from Rothermel Ros)
bool ifallocatebytreecover = false;         		// whether to allocate fire using the empirical distribution from GFED4/MODIS treecover
int min_days_between_burns = 180;       			// minimum numbers of days between patches burning
double human_ignition_constant = 0.35;				// constant in the equation for human ignitions
double fractionsoilmoistureinfuel = 0.0;			// fraction with which to weight soil moisture with fuel moisture
int max_fire_duration = 12;						// maximum fire duration (hours), zero means day length
int min_fire_duration = 4;						// min fire duration (hours), zero means zero (ie. fire can be completely suppressed)
bool ifpopulationsuppression = true; 				// whether or not to suppress fire duration based on population density
double lightning_ctg_factor = 2.0;				// scaling factor to convert input lightning data to cloud-to-ground strikes (also potentially containing dataset-specific detection efficiency correction)
double crop_fraction_suppression_exponent = 0.0;	// Exponential coefficient for reducing fire size as a function of crop fraction (0 = OFF, -4 = recommended)

// firemodetype firemode;						//the mode for burnt area/fire return interval calculation
ignitionmodetype ignitionmode;				//the mode for ignitions when using full SPITFIRE
fuelmoisturemodeltype  fuelmoisturemodel; 	// which fuel moisture model to use
pasturefiretype  pasturefiremode; 	// which pasture fire mode to use
cropfiretype  cropfiremode; 		// which crop fire mode to use
windlimittype windlimit; 			// which limit limitation function to use

// FireMIP
bool iffixedco2;					// Whether or not to hold CO2 constant at PI value
bool iffixedlightning;  			// Whether or not to hold lighning constant at PI values
bool iffixedburntarea;  			// Whether or not to hold burnt area constant at PI values
bool iffixedlanduse;  				// Whether or not to hold land use constant at PI values
bool iffixedhumanpopulation;  		// Whether or not to hold human population constant at PI values
...
namespace IMOGENConfig {
	...
	// Boolean parameters
	 ...
	 //Enums
	 //lpjgimogensimulationmode simulation_mode;
	// lpjgimogenfeedbackmode feedback_mode;
	...
	//  parameters.h same-named comment block for rationale. - DKB 2026-05-07]
	 ...
	 int YEAR1; //!IN First year of the numerical experiment
	 int IYEND;  //!IN Stop year of the ENTIRE run
	 int YEAR1_LPJG; //!IN First year of the whole LPJ - GUESS simulation
	 bool SPINUP; //!IN Are we in the spin - up phase of LPJ - GUESS ?
	 bool KEEPRUNNING;	//!IN control flag to keep imogen running
	 bool FIRSTCALL; //!IN Is this the very first call to IMOGEN from LPJ - GUESS(start of spin - up) ?
	 ...
	 // Add a print function to the namespace
	 void print_all() {
...
// MF: SPITFIRE ENUMS FOR MODES ADDED AT THE END
// SSR: GGCMI: Enum for save_years added after that
enum {CB_NONE,CB_VEGMODE,CB_CHECKGLOBAL,CB_LIFEFORM,CB_LANDCOVER,CB_PHENOLOGY,CB_LEAFPHYSIOGNOMY,CB_SELECTION,	
	...
	CB_FIREMODEL,CB_WEATHERGENERATOR,CB_IGNITIONMODE, CB_FMMODEL, CB_PASTUREMODE, CB_CROPMODE, CB_WINDLIMIT,
	CB_SAVEYEARS};
...
void initsettings() {
	...
	save_state = false;
    lutomemory = false; // SSR: Setting to false after Matt Forrest's change in firemip branch 2020-07-03.
	restart = false;
	verbosity=WARNING;
	lcfrac_fixed = true;
	...
	// MF SPITFIRE-related
	sapsizedecrease = 1;  // no sapling size decrease by default
	ignitionmode = BOTH; // assume full SPITFIRE model with both human and lightning ignitions by default
	lightning_ctg_factor = 0.2; // the original SPITFIRE default value for LIS/OTD data
	ifpopulationsuppression = false; // currently not enabled by default since this was not in the original SPITFIRE
	max_fire_duration = 4; // original SPITFIRE
	min_fire_duration = 4; // original SPITFIRE
	crop_fraction_suppression_exponent = 0; // by default no crop fragmentation of fire size
...
void initpft(Pft& pft,xtring& setname) {
	...
	//DKB: 30-07-2024 Initialize pft based n_harvest_scale_param here
	pft.n_harvest_scale = -1.0;
}
...
// SSR: GGCMI: Taken from crop_ncep version
...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
        ...
        // SSR
		declare_parameter("firsthistyear", &firsthistyear, 0, 10000,
			"First historic year after spinup");
		declare_parameter("lasthistyear", &lasthistyear, 0, 10000,
			"Last historic year of simulation");
        declareitem("firstoutyear", &firstoutyear, 1, 1e5, 1, CB_NONE,
            "First output year");
        declareitem("lastoutyear", &lastoutyear, 1, 1e5, 1, CB_NONE,
            "Last output year");
		declare_parameter("mutesec", &mutesec, 1, 25200,
			"Minimum number of seconds to wait between progress messages (default 20)");

		// SSR: GGCMI
		declareitem("Nfert_scale_factor",&Nfert_scale_factor,0.0,1.0e12,1,CB_NONE,
			"Scaling factor for Nfert input (applies only to file_Nfert and file_Nfert_st)");
		declareitem("manure_cn",&manure_cn,0.0,1.0e12,1,CB_NONE,
			"Manure C:N ratio (default 30.0)");
		declareitem("manure_organic_frac",&manure_organic_frac,0.0,1.0,1,CB_NONE,
			"Fraction of manure N in organic form (default 1.0)");
		...
		declareitem("remove_smallvolfrac",&remove_smallvolfrac,1,CB_NONE,"Whether to remove small amounts of water/ice in update_ice_fraction(). Previously used DEBUG_SOIL_WATER for this. Default false.");
			
#ifdef GCP // GCP related change: allow to keep population density fixed at a certain year
		declareitem("fire_popdens_method",&fire_popdens_method,0,2,1,CB_NONE,"Fire popdens input methods: 0=(undefined), 1=simfire binary, 2=netcdf within cfxinput (default if not specified)");
		declareitem("fixed_popdens_hist",&fixed_popdens_hist,0,2,1,CB_NONE,"cfxinput only: Keep fire popdens level at fixed_popdens_year: 0=no influence (even in spinup popdens changes) 1=popdens start from year fixed_popdens_year onward, 2=allways use popdens of fixed_popdens_year");
		declareitem("fixed_popdens_year",&fixed_popdens_year,-10000,2100,1,CB_NONE, "cfxinput only: Year of popdens to use for fixed_popdens_hist=1");
#endif
		...
		declareitem("save_year", &save_year, 1, 20000, 1, CB_NONE, "Save year (calendar year): The year at whose BEGINNING the state is saved. Default: firsthistyear.");
		declareitem("restart_year", &restart_year, 1, 20000, 1, CB_NONE, "Restart year (calendar year). Default: firsthistyear.");
		...
		// SSR: GGCMI
		declareitem("ggcmi2",&ggcmi2,1,CB_NONE,"Whether to do GGCMI2-specific stuff");
        declareitem("isimip3",&isimip3,1,CB_NONE,"Whether to do ISIMIP3-specific stuff");
		declareitem("crop_gs_out",&crop_gs_out,1,CB_NONE,"Whether to save per-growing-season outputs for each crop");
		declare_parameter("fixed_nfert", &fixed_nfert, 0, 3, "Use time varying Nfert (fixed_nfert=0) or Nfert of fixed_nfert_year for the whole run (fixed_nfert=1), starting with that year and onward (fixed_nfert=2), or before that year (fixed_nfert=3).");
		declare_parameter("fixed_nfert_year", &fixed_nfert_year, 0, 10000, "Year to use if fixed_nfert>0");
		declareitem("just_phu_pvd",&just_phu_pvd,1,CB_NONE,"Whether to skip ALL processes except for PHU and PVD calculation. Default false.");
		declareitem("save_years",&strparam,200,CB_SAVEYEARS,"String of years (calendar) at the beginning of which restarts should resume.");
			
		// SSR
		declareitem("do_potyield",&do_potyield,1,CB_NONE,"Whether to ignore file_lucrop and just evenly split among stands marked as isforpotyield");

		// MF: SPITFIRE parameters
		//mode swithes
		declareitem("ignition_mode", &strparam, 30, CB_IGNITIONMODE, "Which ignition mode to use (only relevant for SPITFIRE runs)");
		declareitem("fuel_moisture_model", &strparam, 30, CB_FMMODEL, "Which fuel moisture model to use (ORIGINAL/FBD)");
		declareitem("pasture_fire_mode", &strparam, 30, CB_PASTUREMODE, "Which pasture fire mode to use (NONE/FULL/LIGHTNING/PRESCRIBED/MODELLED)");
		declareitem("crop_fire_mode", &strparam, 30, CB_CROPMODE, "Which crop fire mode to use (NONE/FULL/LIGHTNING/PRESCRIBED/MODELLED)");
		declareitem("wind_limit", &strparam, 30, CB_WINDLIMIT, "Which wind limit to use (NOLIMIT/LASSLOP/ROTHERMEL/ANDREWS)");


		declareitem("ifburnfullcell", &ifburnfullcell, 1, CB_NONE, "Whether to burn the full cell interpreting the burned area ratio as a probability (0,1)");
		declareitem("ifnesterovdistr", &ifnesterovdistr, 1, CB_NONE, "Whether to distribute the monthly fires using probabilites proportional to daily Nesterov (0,1)");
		declareitem("firereturninterval", &firereturninterval, -1.0, 1000.0, 1, CB_NONE, "fire return interval for fixed or random fire return regimes");
		declareitem("fixburnday", &fixburnday, 0, 365, 1, CB_NONE, "if larger than -1 this value is used as the day when the fire is applied");
		declareitem("pixeldegree", &pixeldegree, 0, 100.0, 1, CB_NONE, "size of a pixel, used in despite-fire");
		declareitem("sapsizedecrease", &sapsizedecrease, 0, 100.0, 1, CB_NONE, "Value to divide the standard sapsize by");
		declareitem("fuelthreshold", &fuelthreshold, 0, 10000, 1, CB_NONE, "Minimum fuel for a patch to burn");
		declareitem("mouillotmultiplier", &mouillotmultiplier, 0, 10000, 1, CB_NONE, "Multiplier for Mouillot data");
		declareitem("ifPandRresidencetime", &ifPandRresidencetime, 1, CB_NONE, "Whether to use standard Rothermel RoS formulation for residence time (=0) or the Peterson and Ryan formulation (=1)");
		declareitem("ifallocatebytreecover", &ifallocatebytreecover, 1, CB_NONE, "whether to allocate fire using the empirical distribution from GFED4/MODIS treecover (=1) or not (=0)");
		declareitem("min_days_between_burns", &min_days_between_burns, 0, 100000.0, 1, CB_NONE, "minimum number of days between patch burns in SPITFIRE");
		declareitem("human_ignition_constant", &human_ignition_constant, 0, 10000, 1, CB_NONE, "Constant for human ignition equation");
		declareitem("fractionsoilmoistureinfuel", &fractionsoilmoistureinfuel, 0, 1, 1, CB_NONE, "Fraction by which to weight soil moisture into fuel moisture");
		declareitem("max_fire_duration", &max_fire_duration, 0, 24, 1, CB_NONE, "Maximum fire duration (hours), zero means day length");
		declareitem("min_fire_duration", &min_fire_duration, 0, 24, 1, CB_NONE, "Minimum fire duration (hours), zero means zero (fires can be completely suppressed).  Only used ifpopulationsuppression is turned on");
		declareitem("ifpopulationsuppression", &ifpopulationsuppression, 1, CB_NONE, "whether to suppress fire duration based population density (following Hantson et al 2015 International Journal of Wildland Fire");
		declareitem("lightning_ctg_factor", &lightning_ctg_factor, 0, 100, 1, CB_NONE, "Lightning cloud-to-ground scale factor (also possible including dataset-specific detection efficiency correction)");
		declareitem("crop_fraction_suppression_exponent", &crop_fraction_suppression_exponent, -10, 0, 1, CB_NONE, "Exponential coefficient for reducing fire size as a function of crop fraction (0 = OFF, -4 = recommended)");

		// MF: FireMIP options
		declareitem("iffixedco2", &iffixedco2, 1, CB_NONE, "Whether to use pre-industrial CO2)");
		declareitem("iffixedlightning", &iffixedlightning, 1, CB_NONE, "Whether to use pre-industrial lightning)");
		declareitem("iffixedburntarea", &iffixedburntarea, 1, CB_NONE, "Whether to use pre-industrial burnt area)");
		declareitem("iffixedlanduse", &iffixedlanduse, 1, CB_NONE, "Whether to use pre-industrial land-use)");
		declareitem("iffixedhumanpopulation", &iffixedhumanpopulation, 1, CB_NONE, "Whether to use pre-industrial human population)");
	...
	case BLOCK_PFT:
		...
		declareitem("n_harvest_scale", &ppft->n_harvest_scale, 0.0, 1.0, 1, CB_NONE,
			"parameter of nitrogen harvest scale");//DKB: 30-07-2024
		...
		// SSR
		declareitem("isforpotyield",&ppft->isforpotyield,1,CB_NONE,"Is this one of the irrigation*Nfert stands for potential yields?");
		declareitem("cropphen_col",&ppft->cropphen_col,32,CB_NONE,"What column does this use?");

		// MF: SPITFIRE modifications for PFT-specific factors
		declareitem("dens_fuel", &ppft->dens_fuel, -100.0, 100.0, 1, CB_NONE,
				"fuel bulk density kg/m3 (zero or negative means use the relationship between grass biomass and FBD in Hoffman et al 2012 Austral Ecology)");
		declareitem("em_CO2", &ppft->em_CO2, 0.001, 10000.0, 1, CB_NONE,
				"emission factor CO2");
		declareitem(	"em_CO", &ppft->em_CO, 0.001, 1000.0, 1, CB_NONE,
				"emission factor CO");
		declareitem("em_CH4", &ppft->em_CH4, 0.001, 100.0, 1, CB_NONE,
				"emission factor CH4");
		declareitem("em_NOx", &ppft->em_NOx, 0.001, 100.0, 1, CB_NONE,
				"emission factor NOx");
		declareitem("em_VOC", &ppft->em_VOC, 0.001, 100.0, 1, CB_NONE,
				"emission factor VOC");
		declareitem("em_TPM", &ppft->em_TPM, 0.001, 100.0, 1, CB_NONE,
				"emission factor TPM");
		declareitem("crown_l", &ppft->crown_l, 0.00, 1.0, 1, CB_NONE,
				"proportion of tree height as crown  ");
		declareitem("flame", &ppft->flame, 0.00, 1.0, 1, CB_NONE,
				"f parameter for flame length");
		declareitem("barka", &ppft->barka, -1000.0, 100.0, 1, CB_NONE,
				"param1 for bark thickness");
		declareitem("barkb", &ppft->barkb, -1000.0, 100, 1, CB_NONE,
				"param2 for bark thickness");
		declareitem("r_ck", &ppft->r_ck, 0.00, 10.0, 1, CB_NONE, ""
				"r(ck) crown damage: postfire mortality");
		declareitem("p", &ppft->p, 0.00, 10.0, 1, CB_NONE,
				"p   crown damage: postfire mortality");
		declareitem("inflame", &ppft->MoE, 0.00, 10.0, 1, CB_NONE,
				"moisture of extinction");
		declareitem("sigma_leaf", &ppft-> sigma_leaf, 0.00, 500.0, 1, CB_NONE,
						"leaf surface area to volume ratio (cm^2/cm^3)");
...
//		declareitem("irrigation",&pmt->firr,0.0,1.0,1,CB_NONE,"Irrigation of crop");
		...
		// SSR
		declareitem("isforpotyield",&pmt->isforpotyield,1,CB_NONE,"Is this one of the irrigation*Nfert stands for potential yields?");
		declareitem("N_appfert_mt",&pmt->N_appfert_mt,0.0,1000.0,1,CB_NONE,"Fertilisation rate (kgN/m2); overrides Pft N_appfert.");
	...
	case BLOCK_ST:
		...
		for(int i = 0; i < NROTATIONPERIODS_MAX; ++i) {
			if(i == 0) {
				...
				declareitem("pft",&strparam,64,CB_PFT,"PFT name");
...
//				declareitem("irrigation",&pst->management.firr,0.0,1.0,1,CB_NONE,"Irrigation of crop 1");
				...
				// SSR
				declareitem("isforpotyield",&pst->management.isforpotyield,1,CB_NONE,"Is this one of the irrigation*Nfert stands for potential yields? (management 1)");
				declareitem("N_appfert_mt",&pst->management.N_appfert_mt,0.0,1000.0,1,CB_NONE,"Fertilisation rate (kgN/m2) of crop 1; overrides Pft N_appfert.");
			}
			else if(i == 1) {
...
void plib_callback(int callback) {
	...
	case CB_FIREMODEL:
		if (strparam.upper()=="BLAZE") firemodel=BLAZE;
			...
			sendmessage("Error",
				"Unknown fire model setting (valid types: \"BLAZE\", \"GLOBFIRM\", \"SPITFIRE\", \"NOFIRE\", \"nil\" , \"\")" );
			plibabort();
		}
		break;
	// MF: SPITFIRE IGNITION MODES
	case CB_IGNITIONMODE:
		if (strparam.upper() == "BOTH")
			ignitionmode = BOTH;
		else if (strparam.upper() == "HUMAN")
			ignitionmode = HUMAN;
		else if (strparam.upper() == "LIGHTNING")
			ignitionmode = LIGHTNING;
		else if (strparam.upper() == "NOINGITIONS")
			ignitionmode = NOINGITIONS;
		else {
			sendmessage("Error", " Unknown ignition mode type\n  (valid types: \"BOTH\", \"HUMAN\", \"LIGHTNING\" or \"NOINGITIONS\")");
			plibabort();
		}
		break;
	// MF: SPITFIRE FUEL MOISTURE MODES
	case CB_FMMODEL:
		if (strparam.upper() == "ORIGINAL")
			fuelmoisturemodel = ORIGINAL;
		else if (strparam.upper() == "DAILY_VPD")
			fuelmoisturemodel = DAILY_VPD;

		else {
			sendmessage("Error", " Unknown fuel moisture model\n  (valid types: \"ORIGINAL\", \"VPD\")");
			plibabort();
		}
		break;
	// MF: SPITFIRE PASTURE FIRE MODE
	case CB_PASTUREMODE:
		if (strparam.upper() == "NONE")
			pasturefiremode = NO_PASTURE_BURNING;
		else if (strparam.upper() == "FULL")
			pasturefiremode = FULL_PASTURE_BURNING;
		else if ( strparam.upper() == "LIGHTNING")
			pasturefiremode = LIGHTNING_PASTURE_BURNING;
		else if (strparam.upper() == "PRESCRIBED")
			pasturefiremode = PRESCRIBED_PASTURE_BURNING;
		else if (strparam.upper() == "MODELLED")
			pasturefiremode = MODELLED_PASTURE_BURNING;
			else {
				sendmessage("Error", " Unknown pasture burn mode\n  (valid types: \"NONE\", \"FULL\", \"PRESCRIBED\", \"MODELLED\",  \"NATURAL\"/\"LIGHTNING\" )");
				plibabort();
			}
		break;
	// MF: SPITFIRE CROP FIRE MODE
	case CB_CROPMODE:
		if (strparam.upper() == "NONE")
			cropfiremode = NO_CROPLAND_BURNING;
		else if (strparam.upper() == "FULL")
					cropfiremode = FULL_CROPLAND_BURNING;
		else if (strparam.upper() == "LIGHTNING")
			cropfiremode = LIGHTNING_CROPLAND_BURNING;
		else if (strparam.upper() == "PRESCRIBED")
			cropfiremode = PRESCRIBED_CROPLAND_BURNING;
		else if (strparam.upper() == "MODELLED")
			cropfiremode = MODELLED_CROPLAND_BURNING;
		else {
			sendmessage("Error", " Unknown crop burn mode\n  (valid types: \"NONE\", \"FULL\", \"PRESCRIBED\", \"MODELLED\",  \"NATURAL\"/\"LIGHTNING\" )");
			plibabort();
		}
		break;
	// MF: SPITFIRE WIND LIMIT
	case CB_WINDLIMIT:
		if (strparam.upper() == "NOLIMIT")
			windlimit = NOLIMIT;
		else if (strparam.upper() == "LASSLOP")
			windlimit = LASSLOP;
		else if (strparam.upper() == "ROTHERMEL")
			windlimit = ROTHERMEL;
		else if (strparam.upper() == "ANDREWS")
			windlimit = ANDREWS;
		else {
			sendmessage("Error", " Unknown wind limit type \n  (valid types: \"NOLIMIT\", \"LASSLOP\", \"ROTHERMEL\", \"ANDREWS\" )");
			plibabort();
		}
		break;
	...
	case CB_MTHYDROLOGY:
		...
		else if (strparam.upper()=="IRRIGATED_WILT") pmt->hydrology = IRRIGATED_WILT;
		else if (strparam.upper()=="IRRIGATED_SAT") pmt->hydrology = IRRIGATED_SAT;
		else if (strparam.upper()=="INUNDATED") pmt->hydrology = INUNDATED;
		else 
		{
			sendmessage("Error",
				"Unknown hydrology type (valid types: \"RAINFED\", \"IRRIGATED_WILT\", \"IRRIGATED_SAT\", \"INUNDATED\")");
			plibabort();
		}
		break;
	case CB_MANAGEMENT1:
	...
	// SSR: GGCMI
	case CB_SAVEYEARS: {
		// Skip this if save_year is specified
		if (save_year != -9999) {
			break;
		}
		
		// Set up
		char saveyears_cp[200] = {0};
		char *p_sav = saveyears_cp;
		int count_sav = 0;
		strcpy(saveyears_cp, strparam);
		
		// Tokenize
		count_sav = split_string(saveyears_cp);
		
		// Save tokens to save_years queue (or just to save_year if only 1 provided)
		if (count_sav == 1) {
			save_year = std::stoi(p_sav);
		} else if (count_sav > 0) {
			
			// Convert string to vector of integers
			std::vector<int> v;
			int year;
			for(int i=0;i<count_sav;i++) {
				// Convert token to integer
				year = std::stoi(p_sav);
				v.push_back(year);
				p_sav += strlen(p_sav) + 1;
			}
			
			// Sort and save to save_years queue
			sort(v.begin(),v.end());
	...
	case CB_CHECKGLOBAL:
		...
		if (firemodel == SPITFIRE
			...
			|| firemodel == FIXED_FIRE_RETURN
			|| firemodel == RANDOM_FIRE_RETURN) {
			if (!itemparsed("ifburnfullcell")) {
				dprintf("Warning: SPITFIRE parameter ifburnfullcell not found; using default value %d\n", ifburnfullcell);
			}
			if (!itemparsed("ifnesterovdistr")) {
				dprintf("Warning: SPITFIRE parameter ifnesterovdistr not found; using default value %d\n", ifnesterovdistr);
			}
			if (!itemparsed("pixeldegree")) {
				dprintf("Warning: SPITFIRE parameter pixeldegree not found; using default value %g\n", pixeldegree);
			}
			if (!itemparsed("sapsizedecrease")) {
				dprintf("Warning: SPITFIRE parameter sapsizedecrease not found; using default value %g\n", sapsizedecrease);
			}
			if (!itemparsed("fixburnday")) {
				dprintf("Warning: SPITFIRE parameter fixburnday not found; using default value %d\n", fixburnday);
			}
			if (!itemparsed("fuelthreshold")) {
				dprintf("Warning: SPITFIRE parameter fuelthreshold not found; using default value %g\n", fuelthreshold);
			}
			if (!itemparsed("mouillotmultiplier")) {
				dprintf("Warning: SPITFIRE parameter mouillotmultiplier not found; using default value %g\n", mouillotmultiplier);
			}
			if (!itemparsed("ifPandRresidencetime")) {
				dprintf("Warning: SPITFIRE parameter ifPandRresidencetime not found; using default value %d\n", ifPandRresidencetime);
			}
			if (!itemparsed("ifallocatebytreecover")) {
				dprintf("Warning: SPITFIRE parameter ifallocatebytreecover not found; using default value %d\n", ifallocatebytreecover);
			}
			if (!itemparsed("min_days_between_burns")) {
				dprintf("Warning: SPITFIRE parameter min_days_between_burns not found; using default value %d\n", min_days_between_burns);
			}
			if (!itemparsed("human_ignition_constant")) {
				dprintf("Warning: SPITFIRE parameter human_ignition_constant not found; using default value %g\n", human_ignition_constant);
			}
			if (!itemparsed("fractionsoilmoistureinfuel")) {
				dprintf("Warning: SPITFIRE parameter fractionsoilmoistureinfuel not found; using default value %g\n", fractionsoilmoistureinfuel);
			}
			if (!itemparsed("max_fire_duration")) {
				dprintf("Warning: SPITFIRE parameter max_fire_duration not found; using default value %d\n", max_fire_duration);
			}
			if (!itemparsed("min_fire_duration")) {
				dprintf("Warning: SPITFIRE parameter min_fire_duration not found; using default value %d\n", min_fire_duration);
			}
			if (!itemparsed("ifpopulationsuppression")) {
				dprintf("Warning: SPITFIRE parameter ifpopulationsuppression not found; using default value %d\n", ifpopulationsuppression);
			}
			if (!itemparsed("lightning_ctg_factor")) {
				dprintf("Warning: SPITFIRE parameter lightning_ctg_factor not found; using default value %g\n", lightning_ctg_factor);
			}
		}
		if (firemodel == FIXED_FIRE_RETURN || firemodel == RANDOM_FIRE_RETURN) {
			if (!itemparsed("firereturninterval")) {
				fail("SPITFIRE parameter firereturninterval not found in ins-file(s)");
			}
		}
		...
		if (fixed_nfert > 0 && !itemparsed("fixed_nfert_year")) {
			fail("Failing: fixed_nfert > 0 but fixed_nfert_year not specified");
		} else if (fixed_nfert == 1) {
			dprintf("Using fertilizer/manure from %d for all years\n", fixed_nfert_year);
		} else if (fixed_nfert == 2) {
			dprintf("Using fertilizer/manure from %d for all years after that\n", fixed_nfert_year);
		} else if (fixed_nfert == 3) {
			dprintf("Using fertilizer/manure from %d for all years before that\n", fixed_nfert_year);
		} else if (itemparsed("fixed_nfert_year")) {
			dprintf("fixed_nfert_year specified (%d) but fixed_nfert was not. Assuming fixed_nfert=1 (constant for entire run)\n", fixed_nfert_year);
			fixed_nfert = 1;
		}
		if (ggcmi2 && isimip3) {
		...
		if (save_state && itemparsed("save_year") && itemparsed("save_years")) {
			sendmessage("Information",
			"Both save_year (one value, integer) and save_years (one or more values, space-separated string of integers) specified. Using save_year only.");
		}

		if (save_state && restart) {
			if (itemparsed("save_year") && restart_year >= save_year) {
				sendmessage("Error",
					"If both restarting and saving, restart_year must be < save_year.");
				plibabort();
			}
		}
		...
		if (save_state && restart && (!itemparsed("restart_year") || !(itemparsed("save_year") || itemparsed("save_years")))) {
			sendmessage("Error",
				"If both restarting and saving, you must specify both restart_year and save_year(s).");
			plibabort();
		}
