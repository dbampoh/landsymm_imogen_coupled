...
void Soil::init_states() {
	...
	for (int mth=0; mth<12; mth++) {
		...
		// SSR: GGCMI
		grs_mwcont_top1m[mth] = -1.0;
	}
	
	// SSR: GGCMI
	grs_days_thismonth = 0;
	...
	// extra SPITFIRE stuffs
	sompool[SURFSTRUCT].moistureofextinction = 0.25;
	sompool[SURFMETA].moistureofextinction = 0.25;
	sompool[SURFFWD].moistureofextinction = 0.25;
	sompool[SURFCWD].moistureofextinction = 0.25;
...
void Soil::hydrology_lpjf_twolayer(const Climate& climate, double fevap) {
	...
	while (vegetation.isobj) {
		...
		for (int s = 0; s<NSOILLAYER; s++) {
			...
			if (patch.stand.pft[indiv.pft.id].hydrology==INUNDATED && ppft.cropphen->growingseason) {
				isinundated = true;
			}
		}
		vegetation.nextobj();
	...
	// TODO: SSR: This will crash if DEBUG_SOIL_WATER is on and you have an inundated cell that did not have rain_melt completely absorbed in initial_infiltration(). The problem is that this next line creates wcont > 1.0, and then the rest of this function does not allow runoff or percolation for inundated stands.
...
void Soil::hydrology_lpjf(const Climate& climate, double fevap) {
	...
	// *** DETERMINE WATER AND ICE CONTENT OF EACH SOIL LAYER ***
	...
	*/
	...
	if (negative_potential) {
		fail("hydrology_lpjf() - error in a soil layer's water balance - negative potential before any additions/removals\n");
	}
	
	
	// *** REMOVE TRANSPIRED WATER FROM SOIL ***
	...
	// Update soil water status variables
	get_soil_water_status(patch.soil, NSOILLAYER, total_potential, Faw_layer, ice_layer, potential_layer, negative_potential);
	if (negative_potential) {
	...
	if (DEBUG_SOIL_WATER) {
		...
		double water_in_storage_in = initial_water_in_column + rain_melt_orig;
		...
		if (fabs(water_in_storage_in - water_out_storage_out) > maxerr) {
			if (do_saturate()) {
				dprintf("Soil::hydrology_lpjf - error in the TOTAL water balance! (%g)\n", water_in_storage_in - water_out_storage_out);
			} else {
				fail("Soil::hydrology_lpjf - error in the TOTAL water balance! (%g) Ignoring because do_saturate()==true\n", water_in_storage_in - water_out_storage_out);
			}
		}
	}
...
void Soil::update_ice_fraction(const int& daynum, const int& MIDX) {
	...
	// SSR: At various points, if remove_smallvolfrac (previously DEBUG_SOIL_WATER) is true, this function removes values < SMALLVOLFRAC for numerical stability. It's possible that the total sum of these across water AND ice could be > SMALLVOLFRAC, which would trigger a non-conservation error at the end according to the original check that abs(before-after) < SMALLVOLFRAC. Now, water_removed is tracked, and failure only happens if non-conservation WITH water_removed accounted for is too high (arbitrarily, >1e-12). Warning given if water_removed > SMALLVOLFRAC.
	double water_removed;

	for (int i = MIDX; i<NLAYERS; i++) {
		
		// Initialize this tracking diagnostic variable
		water_removed = 0.0;
		...
		if (T_soil[i] < 0.0 && T_soil[i] < T_old[i]) { // Wania conditions 
			...
			if (Frac_water[i] > 0.0 /*&& T_old[i] < 1.0*/) {
				...
				if (Frac_water[i] > Ffreez) {
					...
					if (remove_smallvolfrac) {
						if (Frac_ice[i] > 0.0 && Frac_ice[i] < SMALLVOLFRAC) {
							water_removed += Frac_ice[i];
							Frac_ice[i] = 0.0;
					    }
						if (Frac_water[i] > 0.0 && Frac_water[i] < SMALLVOLFRAC){
							water_removed += Frac_water[i];
							Frac_water[i] = 0.0;
						}
						if (Frac_water_belowpwp[i] > 0.0 && Frac_water_belowpwp[i] < SMALLVOLFRAC){
							water_removed += Frac_water_belowpwp[i];
		...
		if ((fabs(wtotal_initial - (wtotal_final + water_removed))) > 1.0e-12) {
			dprintf("Non-conservation of water during freezing or thawing in Soil::update_ice_fraction. Init %g, Final %g, water_removed %g, Diff %g \n", wtotal_initial, wtotal_final, water_removed, wtotal_initial - (wtotal_final + water_removed));
			fail("Failing.");
		} else if (water_removed > SMALLVOLFRAC) {
			dprintf("Warning: Layer %d: water_removed (%g) > SMALLVOLFRAC (%g)\n", i, water_removed, SMALLVOLFRAC);
		}
...
bool Soil::soil_temp_multilayer(const double &dailyairtemp) {
	...
	const double ERROR_TEMP = 90.0; // degrees C
