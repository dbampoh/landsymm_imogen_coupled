...
int RUN_IMOGEN_ENGINE() {
    ...
    // [Step 17b (F-12 sub-milestone C2; B3 LANDED) — close-by-architectural-
    //  reframing: this `regrid` local is currently DEAD CODE — declared but
    ...
    //  block at line ~890 below + notes/STEP_17b.md §3e). Until that
    ...
    while (keepRunning) {
        ...
        for (iyear = year1; iyear <= iyend; ++iyear) {
            ...
            if (anlg && anom && stepDay == 1 && mm == 12 && md == 30) {
                ...
                // branch in this C++ in-process port — see B3 canonical doc
                ...
                //  Decision #11 algebra — same Kelvin units as T_anom).
                //  - DKB 2026-05-07]
                //
                // [Step 17b (F-12 sub-milestone C2; B3 LANDED 2026-05-12) — CANONICAL
                ...
                //  anticipating that a C++ REGRID branch — analogous to the
                //  Fortran engine's IF (REGRID) THEN ... ELSE ... ENDIF block
                //  at imogen/code/imogen_lpjg.f lines ~962-1117 — would later
                ...
                //  §3e + this B3 commit): the anticipated C++ REGRID branch was
                ...
                //  B3 docs-only.
                //
                //  Cross-references: notes/STEP_17b.md §3e (full B3 forensic
                ...
                //  blocker for C1.3 sub-step 7.3.1 multi-cell xval (4-cell — any
                //  nyear_spinup hits EVEN imogen_years for some cell — year_idx).
                //
...
GcmAnlgOutput gcm_anlg(double q, int land_pts, int n_olevs, const std::string& dir_patt,
    ...
    // [oceanfix 2026-06-06] Persist the ocean thermal state across years: initialize from the
    ...
    // [oceanfix 2026-06-06] Compute the land-mean temperature anomaly ONCE per year (stepping the
    ...
    for (int im = 0; im < mm; ++im) {
        // [oceanfix 2026-06-06] dtemp_l now computed once before this loop (applies to all months).
