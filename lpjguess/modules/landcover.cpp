	...
	while (stlist.isobj) {
		...
		if(gcst.gross_frac_increase || gcst.gross_frac_decrease || gcst.frac == 0.0) {
			...
			if((gcst.frac_old == 0.0 || stand_sum == 0.0) && gcst.gross_frac_increase > 0.0) {
				int npatch;
				if(date.year == 0 || st.landcover==CROPLAND)
			...
			if(LCchangeCtransfer) {
				...
				while(stand.isobj) {
					...
					for(int i=0; i<NSOMPOOL; i++) {
						...
						// SPITFIRE MF
						double temp_original_MoE = patch.soil.sompool[i].moistureofextinction;
						double temp_original_cmass = patch.soil.sompool[i].cmass;
						...
						// keeps check enabled for now
						if (!(patch.soil.sompool[i].moistureofextinction <= 0.300000001 && patch.soil.sompool[i].moistureofextinction >= 0.199999999)
								&& (i == 0 || i == 5 || i == 6 || i ==7)
								&& patch.soil.sompool[i].cmass > 0) {
							dprintf("i = %i\n", i);
							dprintf("patch = %i\n", patch.id);
							dprintf("old_frac = %.20f\n", old_frac);
							dprintf("new_frac = %.20f\n", new_frac);
							dprintf("added_frac = %.20f\n", added_frac);

							dprintf("Added cmass = %.20f\n", from.transfer_sompool[i].cmass);
							dprintf("Added cmass scaled = %.20f\n", from.transfer_sompool[i].cmass * added_frac);
							dprintf("Initial cmass = %.20f\n",temp_original_cmass);
							dprintf("Final cmass = %.20f\n", patch.soil.sompool[i].cmass);
							dprintf("receiving original MoE = %.20f\n", temp_original_MoE);
							dprintf("receiving patch.soil.sompool[i].moistureofextinction = %.20f\n", patch.soil.sompool[i].moistureofextinction);
							dprintf("receiving from.transfer_sompool[i].moistureofextinction = %.20f\n", from.transfer_sompool[i].moistureofextinction);
							dprintf("receiving patch.soil.sompool[i].fireresist = %.20f\n", patch.soil.sompool[i].fireresist);
							dprintf("receiving from.transfer_sompool[i].fireresist = %.20f\n", from.transfer_sompool[i].fireresist);

							dprintf("Year = %i, Day = %i\n", date.year, date.day);
							// abort();
						}
