...
void SoilInput::load_mineral_soils(const char* fname, const std::set<coord>& coords) {
	...
	while (getline(ifs, line)) {
		...
		if (iss >> lon >> lat) {	// Get lon and lat from current line.
			...
			if (sand_i == -1) {
				fail("sand column not found in soildata");
			} else if (clay_i == -1) {
				fail("clay column not found in soildata");
			} else if (orgc_i == -1) {
				fail("orgc column not found in soildata");
			} else if (ph_i == -1) {
				fail("ph column not found in soildata");
			}
...
void SoilInput::get_soil_mineral(double lon, double lat, Gridcell& gridcell) {
    ...
    // SSR: GGCMI. Make it so this doesn't fail silently and potentially cause hard-to-diagnose problems.
    if (soilprop.sand==0.0 && soilprop.clay==0.0) {
        dprintf("Warning: sand==clay==0.0. Invalid soil? Will try to continue, but might cause errors.\n");
    }
	...
	soiltype.pH_value = soilprop.pH; //--------------JM-20240910
