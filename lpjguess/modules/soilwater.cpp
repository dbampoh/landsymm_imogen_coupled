...
/// SSR: Get current soil water status. For use in infiltration, runoff,
...
void get_soil_water_status(Soil soil, int nlayers_to_use, double& total_potential,
	...
	/*
	 Faw_layer[nlayers_to_use]: plant-available water for each soil layer (mm)
	 ice_layer[nlayers_to_use]: ice in each soil layer (mm)
	 potential_layer[nlayers_to_use]: water that can still be added to each soil layer (mm)
	 total_potential: sum of all potential_layer[nlayers_to_use]
	...
	for (int ly = 0; ly < nlayers_to_use; ly++) {
		...
		// Remaining capacity of this layer
		potential_layer[ly] = soil.aw_max[ly] - layerwater;
		
		// Allow only small overages of layerwater relative to soil.aw_max[ly]
		...
		// Update total capacity of column
...
/// SSR: Saturate wetlands/inundated stands.
/** This function was created to unify behavior across
 * initial_infiltration() and hydrology_lpjf().
 */
...
//	
//	// Inundated stands should absorb water in all layers, not just upper.
//	int nlayers_to_use;
//	if (patch.hydrology==INUNDATED) {
//		nlayers_to_use = NSOILLAYER;
//	} else {
//		nlayers_to_use = NSOILLAYER_UPPER;
//	}
//
//	/*
//	Faw_layer[nlayers_to_use]: plant-available water for each soil layer (mm)
...
//	negative_potential: for error checking
//	*/
//	double Faw_layer[nlayers_to_use];
//	double ice_layer[nlayers_to_use];
//	double potential_layer[nlayers_to_use];
//	double total_potential = 0.0;
//	bool negative_potential = false;
//	get_soil_water_status(soil, nlayers_to_use, total_potential, Faw_layer, ice_layer, potential_layer, negative_potential);
...
//		
//		// Assume there is enough potential to absorb all of rain_melt. If not, any excess will be saved
//		// in overflow and added back to rain_melt.
//		double overflow = 0.0;
//
...
//
//			// Add water to the layer, update wcont and Frac_water for this layer,
//			// and save overflow
...
void infiltrate_upland(Patch& patch) {
	...
	/*
	Faw_layer: plant-available water for each soil layer (mm)
	ice_layer: ice in each soil layer (mm)
	potential_layer: water that can still be added to each soil layer (mm)
	total_potential: sum of all potential_layer values
	negative_potential: for error checking
	*/
...
/** This function was created to unify behavior across
 * initial_infiltration() and hydrology_lpjf().
 * Using initial_infiltration() method. Differences:
 * 	- initial_infiltration() allows ALL soil layers to absorb rain_melt for
 * 	  these stands; hydrology_lpjf() only allowed upper layers.
 * Note that this function adds enough water to saturate the soil, REGARDLESS
 * of whether there was actually enough rain_melt to do so.
 */
...
void initial_infiltration(Patch& patch, Climate& climate) {
	...
	if (soil.percolate) {
		...
		else if (patch.stand.is_highlatitude_peatland_stand()) {
 ...
 */
// This function only serves to update the diagnostic variables patch.irrigation_{d,y}. It does NOT affect how much water actually gets added to the soil.
void irrigation(Patch& patch) {
	...
	if (!(patch.stand.hydrology > RAINFED || patch.hydrology==INUNDATED)) {
		return;
	}
	...
	for (int i = 0; i < npft; i++) {
		...
		if (patch.stand.pft[i].hydrology > RAINFED && ppft.growingseason()) {
    ...
    patch.grs_w_irr += patch.irrigation_d; // SSR GGCMI
	patch.grs_w_irr_thisyear += patch.irrigation_d; // SSR
	//soil.rain_melt += patch.irrigation_d;
	//soil.max_rain_melt += patch.irrigation_d;
	//
	//if (patch.irrigation_y > 0) {
	//	dprintf("irrigation_y: %f\n", patch.irrigation_d);
	//	system("pause");
	//}
}
