...
void phu_init(cropphen_struct& ppftcrop, Gridcellpft& gridcellpft, Patch& patch) {
    ...
    // SSR: GGCMI: Deal with input PHU and PVD files
    if (!(readphu==readpvd)) {
		...
		if (patch.stand.pft[pft.id].active) {
			...
			if (pft.phenology == CROPGREEN) {
				...
				if (date.day == ppftcrop.sdate) {
                    ...
                    patch.grs_n_uptake = 0.0; //not used
                    ppftcrop.dap_anth = 0;
                    ppftcrop.dap_mat = 0;
					...
					/*dprintf("Patch.hydrology: %d", patch.hydrology);
					system("pause");*/

					// calculate pvd, phu & tb
					phu_init(ppftcrop, gridcellpft, patch);
					
					// SSR: GGCMI. For some reason, after first gridcell in list, this fails an assertion in year 0. Adding a check that date.year>0 shouldn't make any difference, since land use isn't supposed to happen before then, and anyway this is just a flag in the output data.
					if (date.year>0) {
				...
				if (ppftcrop.growingseason) {
					...
					// true if harvest today is prescribed
					bool force_harvest;
					...
					// true if maturity not yet reached
					bool pre_maturity = ifnlim ? ppftcrop.dev_stage < 2.0 : ppftcrop.husum < ppftcrop.phu;

					// Annual crops must be harvested no later than the dates prescribed in the ins file for the northern and southern hemispheres;  see Ma et al. (2022) in GMD
					if (gridcell.get_lat() >= 0.0) {   //--JM
						ppftcrop.hlimitdate = pft.hlimitdatenh;  //--JM
					}  //--JM
					else {   //--JM
						ppftcrop.hlimitdate = pft.hlimitdatesh;   //--JM
					}   //--JM

                    // true if crop hasn't reached maximum growing season length
                    bool pre_hlimit = dayinperiod(date.day, ppftcrop.sdate, stepfromdate(ppftcrop.hlimitdate, -1)) ;
                    
                    // harvest will happen today if true
                    bool harvest_today;
                    if (ggcmi2 || isimip3) {
					...
					if (!harvest_today) {
                        ...
                        ppftcrop.dap_mat = ppftcrop.growingdays; // SSR: GGCMI
						...
						// Previously, this set patch.isinundated to false.
