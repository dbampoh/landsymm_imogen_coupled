...
bool CFXInput::getgridcell(Gridcell& gridcell) {
	...
	if (restart&&(restart_year < climate_first_year))
		sync_first_year = restart_year;
	...
	if (extend_climate) {
		...
		if (restart && restart_year-1 > extend_climate_startyear) {
			...
			year_offset = restart_year-1 - extend_climate_startyear - 1; // -1 since there will be an extclim_data_nextyear later
...
// SSR			else if (calendar_year <= last_date.get_year()) {
				...
				if (historic_timestep + 1 < cf_historic->get_timesteps()) {
					...
					if (restart) { // in case of restart from state need to advance to the restart year
						while(dt.get_year() < restart_year) {
							++historic_timestep;
...
bool CFXInput::getclimate(Gridcell& gridcell) {
	...
	if (date.day == 0) {
		...
		if (tmute.getprogress()>=1.0) {
			...
			if (restart) {
				years_to_simulate = lasthistyear - restart_year + 1;
				yearsdone_thiscell = date.get_calendar_year() - restart_year;
			} else {
