...
void crop_sowing_date(Patch& patch, Pft& pft) {
	...
	if (seasonality == SEASONALITY_TEMP || seasonality == SEASONALITY_TEMPPREC ||
			(IRRIGATED_USE_TEMP_SDATE && seasonality == SEASONALITY_PRECTEMP && standpft.hydrology>=IRRIGATED_WILT)){
		temp_sdate = true;
	} else if ((seasonality == SEASONALITY_PREC || seasonality == SEASONALITY_PRECTEMP &&
				(IRRIGATED_USE_TEMP_SDATE && standpft.hydrology==RAINFED)) && climate.prec_range != WET) {
		prec_sdate = true;
	} else {
		def_sdate = true;
	}
	...
	if (dayinperiod(date.day, patchpft.swindow[0],patchpft.swindow[1])) {
		...
		if (date.day != patchpft.swindow[1]) {
			...
			if (temp_sdate) {
				...
				if (climate.prec > prec_limit || standpft.hydrology>=IRRIGATED_WILT)
					ppftcrop.sdate = date.day;
			}
			else // if (def_sdate)
	...
	if (stlist[patch.stand.stid].get_management(patch.stand.current_rot).multicrop) {
		...
		if (!(prec_sdate && climate.prec_seasonality <= DRY_WET) || standpft.hydrology>=IRRIGATED_WILT) {// No water stress
	...
	while(pftlist.isobj) {
		...
		if (patch.stand.pft[pft.id].active && pft.phenology == CROPGREEN) {
			...
			if (date.day == climate.testday_temp) {
				...
				if (patch.stand.pft[pft.id].hydrology>=IRRIGATED_WILT) {
