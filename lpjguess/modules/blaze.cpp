...
double survival_probability_temp_needleleaf(double diameter_at_breast_height, double fire_line_intensity, double mass_cwd) {
	...
	double cwd = mass_cwd * 0.1 * 2.0; // in Mg/ha
...
bool blaze(Patch& patch, Climate& climate) {
	...
	if ( vegmode == POPULATION ) {
		...
		while (vegetation.isobj) {
			...
			if (indiv.pft.lifeform==GRASS) {
				if (patch.stand.landcover==PASTURE) { //CLN
				...
				Gridcellpft& gridcellpft=gridcell.pft[indiv.pft.id];
				gridcellpft.blaze_burned_area[date.month] += MAX_GRASS_BURN*to_gridcell_average;
				...
				if (ifstochmort) {
					...
					double fpc_tot_patch = 0.;
					int nindiv=(int)(indiv.densindiv*patcharea+0.5);
					int nindiv_prev=nindiv;
					//CLN get actuall fpc of the day (why does it differ?)
					for (int i=0;i<nindiv_prev;i++) {
					    fpc_tot_patch += indiv.fpc;
					}
					for (int i=0;i<nindiv_prev;i++) {
						if (randfrac(patch.stand.seed) > survival_probability(patch, indiv)) {
							nindiv--;
							// FireMIP
							gridcell.blaze_monthly_firetreemortality[date.month] += indiv.fpc/fpc_tot_patch*to_gridcell_average;
							Gridcellpft& gridcellpft=gridcell.pft[indiv.pft.id];
							gridcellpft.blaze_burned_area[date.month]  += indiv.fpc/fpc_tot_patch*to_gridcell_average;
						}
					}
...
void blaze_accounting_gridcell(Climate& climate) {
	...
	if (date.day == 0 ) {
		...
		for (int m = 0; m < 12; m++) {
			...
			// FireMIP
			gridcell.blaze_monthly_weighted_fli           [m] = 0.;
			gridcell.blaze_monthly_total_no_fires         [m] = 0;
			gridcell.blaze_monthly_mean_no_fires          [m] = 0.;
			gridcell.blaze_monthly_firetreemortality      [m] = 0.;
			gridcell.blaze_monthly_weighted_mean_fire_size[m] = 0.;
			gridcell.blaze_monthly_weighted_ros           [m] = 0.;
			for (int fc = 0; fc < 4; fc++) {
			  gridcell.blaze_monthly_C_fuelClas [m][fc] = 0.;
			  gridcell.blaze_monthly_CC_fuelClas[m][fc] = 0.;
			}
			pftlist.firstobj();
			while (pftlist.isobj) {
			  Pft& pft=pftlist.getobj();
			  Gridcellpft& gridcellpft=gridcell.pft[pft.id];
			  gridcellpft.blaze_burned_area[m] = 0.;
			  pftlist.nextobj();
			}
		}
