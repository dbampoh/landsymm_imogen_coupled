...
void wdemand(Patch& patch, Climate& climate, Vegetation& vegetation, const Day& day) {
    ...
    while (vegetation.isobj) {
        ...
        // (lambda = lambda_max)
        if (indiv.growingseason()) {
           // dprintf("In the growing season | Date.day:%d\n", date.day);
            ...
            //dprintf("GP Patch: %f Date.day: %d\n", gp_patch, date.day);

           /* if (gp_patch > 0) {
                dprintf("GP Patch: %f\nindiv.gpterm: %f\npft.gmin: %f\nindiv.fpc_today(): %f\n", gp_patch, pft.gmin, indiv.fpc_today());
                system("pause");
            }*/
        }
        vegetation.nextobj();
    ...
    if (!negligible(gp_patch) && !negligible(patch.fpc_total)) {
       ...
       /* dprintf("water demand is: %f", patch.wdemand);
        system("pause");*/
    }
    else
        patch.wdemand = 0.0;
...
double irrigated_water_uptake(Patch& patch, Pft& pft, const Day& day) {
	...
	// Get current soil water status
	/*
	Faw_layer[NSOILLAYER]: plant-available water for each soil layer (mm)
	ice_layer[NSOILLAYER]: ice in each soil layer (mm)
	potential_layer[NSOILLAYER]: water that can still be added to each soil layer (mm)
	total_potential: sum of all potential_layer[NSOILLAYER]
	negative_potential: for error checking
	*/
	...
	if (negative_potential) {
		fail("irrigated_water_uptake() - error in a soil layer's water balance - negative potential before adding water\n");
	}

    // Prepare for water balance tests
    double initial_water_in_column = 0.0;
    for (int ly = 0; ly < NSOILLAYER; ly++) {
        initial_water_in_column += Faw_layer[ly] + ice_layer[ly]; // mm
    } // for loop (ly)
    ...
    int nsoillayer_to_irrigate;
    if (!just_inund_upper) {
        nsoillayer_to_irrigate = NSOILLAYER;
    } else {
        nsoillayer_to_irrigate = NSOILLAYER_UPPER;
    }

    /*double wcont_opt[nsoillayer_to_irrigate];
    bool add_water[nsoillayer_to_irrigate];*/
    std::vector<double> wcont_opt(nsoillayer_to_irrigate, 0.0);
    std::vector<bool> add_water(nsoillayer_to_irrigate, true);
    for (int i = 0; i<nsoillayer_to_irrigate; i++) add_water[i] = true;
    ...
    if (ppft.phen > 0.0 && patch.soil.get_soil_water_upper()<restrict_irr_wcont)    {
        ...
        // How much water does the plant want to take up? Given as a fraction of the maximum it possibly _could_ take up given today's "pipes."
		double wr_opt = min(1.0, patch.wdemand / ppft.phen / pft.emax);

       // dprintf("debug\npatch.wdemand: %f\nppft.phen:%f\npft.emax:%f\n", patch.wdemand, ppft.phen, pft.emax);
       ...
       // dprintf("wr_opt: %f\n", wr_opt);
        ...
        if (wateruptake == WR_ROOTDIST) {
            ...
            // Determine how much water the plant would try to take up from the upper layer if no irrigation were added. Calculated as the total amount of water the plant wants/is capable of taking up (wr_opt*pft.emax) minus the amount the plant would try to take up from the lower layer absent irrigation.
            // SSR: This doesn't account for how the ratio of upper:lower soil moisture would actually end up looking after water_uptake(). In my previous version, I changed it so it does.
            
            /*dprintf("water hydrology int: %d\n", patch.hydrology);
            system("pause");*/

            // This is only used for two-layer soil.
            if (patch.hydrology>=IRRIGATED_SAT) {
                wcont_0_opt = 1.0;
                wcont_1_opt = 1.0; // SSR: Only needed if inundated and !just_inund_upper
            } else {
        ...
        // Turn on irrigation if (any) upper layer doesn't have enough water, according to how much the plant wants from that layer.
		if (iftwolayersoil) {
        ...
        /* If irrigating, this is where we determine how much is needed to get the soil to the right moisture
			...
			*/
			...
			if (iftwolayersoil) {
				...
				if (wcont_0 + ice_0 + water_to_add_ly[0]/awc0 > 1.0) {
					// Apparently, wcont_0 + ice_0 can be slightly but non-negligibly >1.0
            ...
            //dprintf("Debug water deficit here: %f\n", ppft.water_deficit_d);
            //system("pause");

			// Local storage

			//// available (liquid) water in each soil layer (mm)
			//double Fw_liq_layer[nsoillayer_to_irrigate];
			//// ice in each soil layer (mm)
			//double Fw_ice_layer[nsoillayer_to_irrigate];
			//// water that can still be added to each soil layer (mm)
			//double potential_layer[nsoillayer_to_irrigate];

			//// initialise to 0 mm
			//for (int sl = 0; sl<nsoillayer_to_irrigate; sl++) {
			//	Fw_liq_layer[sl] = 0.0;
			//	Fw_ice_layer[sl] = 0.0;
            ...
            std::vector<double> potential_layer(nsoillayer_to_irrigate, 0.0);

			Soil& soil = patch.soil;
			double potential_water = 0.0;

			// SSR: This uses the conventions normally associated with !iftwolayersoil, but presumably applies to 2-layer soil as well. Does that imply that the rest of the code in this function (and elsewhere?) can be similarly generalized for simplicity?
			...
			/*
			 Actually add the irrigation to the soil.
			 */
    ...
    */
	get_soil_water_status(patch.soil, NSOILLAYER, total_potential, Faw_layer, ice_layer, potential_layer, negative_potential);
	if (negative_potential) {
		fail("irrigated_water_uptake() - error in a soil layer's water balance - negative potential after adding water\n");
	}
    double final_water_in_column = 0.0;
    for (int ly = 0; ly < NSOILLAYER; ly++) {
        final_water_in_column += Faw_layer[ly] + ice_layer[ly]; // mm
    } // for loop (ly)
    ...
    /*
	 Actually calculate the value that's really needed from this function: wr (plant water uptake as fraction of maximum possible)
    */
...
void aet_water_stress(Patch& patch, Vegetation& vegetation, const Day& day) {
    ...
    for (int p=0; p<npft; p++) {
        ...
        if (day.isstart || (spft.hydrology > RAINFED && pft.id == patch.stand.pftid)) {
            ...
            if (spft.hydrology > RAINFED && pft.id == patch.stand.pftid) {
                /*dprintf("Debugging hydrology type");
                system("pause");*/
                /*dprintf("Hydrology type: %d\n", spft.hydrology);
                system("pause");*/
			...
			// SSR: GGCMI: Track soil moisture as the plant sees it today BEFORE uptake/evaporation/etc.
			// Not tested with two-layer soil
			double soil_water_top1m_today = 0.0;
			double accum_depth = 0.0;
			double thisdepth, thisweight;
			int ly = 0;
			while (accum_depth < 1000.0) {
				if (ly==NSOILLAYER) {
					fail("Infinite loop?");
				}
				thisdepth = min(Dz_soil, 1000.0-accum_depth);
				thisweight = thisdepth / 1000.0;
			...
			if (patch.soil.grs_days_thismonth > 0) {
				...
				patch.soil.grs_mwcont_top1m[date.month] += soil_water_top1m_today * 1.0/(double)patch.soil.grs_days_thismonth;
			}
...
void leaf_senescence(Vegetation& vegetation) {
    ...
    while (vegetation.isobj) {
		...
		if (indiv.patchpft().cropphen->dev_stage > indiv.pft.d3) {
        ...
        if (indiv.pft.fixer) {
            ...
            if (ndeficit > 0.0 && indiv.dnpp > 0.0) {
                ...
                if (indiv.pft.phenology==CROPGREEN) {
					...
					if (ppftcrop.dev_stage > indiv.pft.d3) {
...
void canopy_exchange(Patch& patch, Climate& climate) {
  ...
  /*  if (vegetation.nobj > 0) {
    dprintf("The number of vegetation objects here is: %d\n", vegetation.nobj);
    dprintf("Patch wdemand: %f\n", patch.wdemand);
    system("pause");
