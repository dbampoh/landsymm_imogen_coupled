...
CommonOutput::CommonOutput() {
	...
	if ( firemodel == BLAZE || firemodel == SPITFIRE ) {
		...
		if( firemodel == BLAZE ) declare_parameter("file_simfireanalysis_out", &file_simfireanalysis_out, 300, "SIMFIRE analytics output");
		//FireMIP
		declare_parameter("file_mblaze_ba",               &file_mblaze_ba,               300, "Monthly BLAZE burned area file");
		declare_parameter("file_mblaze_FLI",              &file_mblaze_FLI,              300, "Monthly BLAZE fire line intensity file");
		declare_parameter("file_mblaze_mean_number_fires",&file_mblaze_mean_number_fires,300, "Monthly BLAZE mean #fires file");
		declare_parameter("file_mblaze_tree_mortality",   &file_mblaze_tree_mortality,   300, "Monthly BLAZE tree mortality file");
		declare_parameter("file_mblaze_mean_fire_size",   &file_mblaze_mean_fire_size,   300, "Monthly BLAZE mean fire size file");
		declare_parameter("file_mblaze_weighted_ROS",     &file_mblaze_weighted_ROS,     300, "Monthly BLAZE weighted rate-of-spread file");
	}
...
void CommonOutput::define_output_tables() {
	...
	dens_columns += ColumnDescriptors(landcovers,         13, 4);
	 ...
	 // CMASS
	ColumnDescriptors baPerPFT_columns;
	baPerPFT_columns += ColumnDescriptors(pfts,               8, 3);
	baPerPFT_columns += ColumnDescriptor("Total",             8, 3);
	baPerPFT_columns += ColumnDescriptors(landcovers,        13, 3);
	...
	if ( firemodel == BLAZE || firemodel == SPITFIRE) {
		...
		if ( firemodel == BLAZE ) create_output_table(out_simfireanalysis,	file_simfireanalysis_out,	simfireanalysis_columns);
	...
	if (firemodel == BLAZE) {
	    ...
	    // per PFT burned area - now done into the SPITFIRE monthly_burned_area files
...
void CommonOutput::outannual(Gridcell& gridcell) {
	...
	while (gc_itr != gridcell.end()) {
		...
		while (stand.isobj) {
			...
			for (m=0;m<12;m++) {
				...
				// monthly fire fluxes
				mcflux_fire[m] += patch.fluxes.get_monthly_flux(Fluxes::FIREC, m)*to_gridcell_average;	// kg C/m2
	...
	for (m=0;m<12;m++) {
		...
		double mfli = 0.;
		double mros = 0.;
		if(gridcell.monthly_burned_area[m] > 0.) {
		...
		// FireMIP PFTs - see SPITFIRE output
	...
	if (!(date.year%PLOT_INTERVAL)) {
		if(gridcell.nbr_stands() > 0)	//Fixed bug here if no stands were present.
		{
			...
			plot("C flux [kgC/m2/yr]","manure",  date.year, flux_man);
			...
			plot("C flux [kgC/m2/yr]","NEE",  date.year, flux_man + flux_veg + flux_repr + flux_soil + flux_fire + flux_est);
...
/// SSR: GGCMI
void CommonOutput::outharvest(Gridcell& gridcell) {
}
void CommonOutput::outharvest_justphupvd(Gridcell& gridcell) {
}
void CommonOutput::outannual_ggcmi(Gridcell& gridcell) {
}
} // namespace
