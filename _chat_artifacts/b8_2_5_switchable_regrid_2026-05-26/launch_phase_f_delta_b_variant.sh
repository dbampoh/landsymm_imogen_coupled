#!/usr/bin/env bash
# Block 8.2.5 Phase F δ-B-variant pipeline launcher (hist → scen sequential)
...
{
  echo "================================================================================"
  echo "Phase F δ-B-variant HIST phase"
...
if [ "$HIST_RC" != "0" ]; then
  echo "[ABORT] δ-B-variant HIST failed (rc=$HIST_RC); skipping SCEN" >> "$HIST_LOG"
  exit "$HIST_RC"
fi
...
{
  echo "================================================================================"
  echo "Phase F δ-B-variant SCEN phase (restart from HIST saved state)"
  echo "Start: $(date '+%Y-%m-%d %H:%M:%S %Z')"
  echo "Dir: $SCEN_DIR"
  echo "================================================================================"
} > "$SCEN_LOG"
