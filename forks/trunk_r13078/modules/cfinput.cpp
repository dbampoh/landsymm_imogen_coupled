...
bool CFInput::getgridcell(Gridcell& gridcell) {
	...
	* we need to use the RCP data for the last decade. */
	...
	if ((cf_prec && !(cf_prec->get_date_time(0) == dt_temp)) ||
		(cf_insol && !(cf_insol->get_date_time(0) == dt_temp)) ||
		(cf_wetdays && !(cf_wetdays->get_date_time(0) == dt_temp)) ||
		(cf_min_temp && !(cf_min_temp->get_date_time(0) == dt_temp)) ||
		(cf_max_temp && !(cf_max_temp->get_date_time(0) == dt_temp)) ||
		(cf_pres && !(cf_pres->get_date_time(0) == dt_temp)) ||
		(cf_specifichum && !(cf_specifichum->get_date_time(0) == dt_temp)) ||
		(cf_relhum && !(cf_relhum->get_date_time(0) == dt_temp)) ||
		(cf_wind && !(cf_wind->get_date_time(0) == dt_temp))) {
	...
	dprintf("\nCommencing simulation for gridcell at (%g,%g) (index %d,%d)\n", lon, lat, current_gridcell->rlon, current_gridcell->rlat);
	if (current_gridcell->descrip != "") {
		dprintf("Description: %s\n", current_gridcell->descrip.c_str());
	}
	dprintf("Using soil code and Nitrogen deposition for (%3.2f,%3.2f)\n", cru_lon, cru_lat);
