		...
		while(stand.isobj) {
			...
			for(int i=0; i<NSOMPOOL; i++) {
				...
				if(!negligible(transfer_sompool[i].cmass)) {
					...
					transfer_sompool[i].fuelbulkdensity = 0;
					transfer_sompool[i].surfacetovolume = 0;
			...
			if (!(transfer_sompool[i].moistureofextinction <= 0.300000001 && transfer_sompool[i].moistureofextinction >= 0.199999999)
				&& (i == 0 || i == 5 || i == 6 || i ==7)
										&& !(transfer_sompool[i].moistureofextinction == 0 && transfer_sompool[i].cmass == 0)) {
					dprintf("i = %i\n", i);
					dprintf("Year = %i, Day = %i\n", date.year, date.day);
					dprintf("scale = %f\n", scale);
				 	dprintf("add_from_stand initial transfer MoE = %.20f\n", initial_MoE);
					dprintf("add_from_stand initial cmass = %.20f\n", initial_cmass);
					dprintf("add_from_stand patch cmass = %.20f\n", patch.soil.sompool[i].cmass * scale);
					dprintf("add_from_stand final cmass = %.20f\n", transfer_sompool[i].cmass);
					dprintf("add_from_stand transfer_sompool[i].moistureofextinction = %.20f\n", transfer_sompool[i].moistureofextinction);
					dprintf("add_from_stand patch.soil.sompool[i].moistureofextinction = %.20f\n", patch.soil.sompool[i].moistureofextinction);
					dprintf("add_from_stand transfer_sompool[i].fireresist = %.20f\n", transfer_sompool[i].fireresist);
					dprintf("add_from_stand patch.soil.sompool[i].fireresist = %.20f\n", patch.soil.sompool[i].fireresist);
