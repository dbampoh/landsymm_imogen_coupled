...
//				dprintf("ATTENTION: pft %s: cmass_debt > cmass_heart; difference=%f\n", (char*)pft.name, i.cmass_debt-i.cmass_heart);
			...
			i.cmass_litter_sap += to_partition_sap * (1.0 - res_outtake_twig * twig_frac - res_outtake_coarse_root * coarse_root_frac - harv_eff * stem_frac) * frac_cut;
			i.cmass_litter_heart += to_partition_heart * (1.0 - res_outtake_twig * twig_frac - res_outtake_coarse_root * coarse_root_frac - harv_eff * stem_frac) * frac_cut;
		}
...
void harvest_pasture(Harvest_CN& i, Pft& pft, bool alive) {
	...
	double N_harvest_scale = pft.n_harvest_scale >= 0 ? pft.n_harvest_scale : 0.25; // DKB: 30-07-2024. Previously 0.25 constant 
...
bool harvest_year(Individual& indiv) {
	...
	if (landcover.updated && !indiv.has_daily_turnover()) {
		if ( ! indiv.vegetation.patch.burned) { //CLN
...
void yield_crop(Individual& indiv) {
	...
	// SSR: Was originally c_to_dm=2.0
	double c_to_dm = 1.0 / 0.446;
	...
	if (indiv.pft.phenology == ANY) {			// grass intercrop yield
		...
		if (cropindiv.ycmass_leaf > 0.0)
			cropindiv.yield = cropindiv.ycmass_leaf * indiv.pft.harv_eff_ic * c_to_dm;
		else
			cropindiv.yield = 0.0;

		// Yield dry wieght of actually harvest products this year; NB as above
		if (cropindiv.harv_cmass_leaf > 0.0)
			cropindiv.harv_yield = cropindiv.harv_cmass_leaf * indiv.pft.harv_eff_ic * c_to_dm;
		else
		...
		if (cropindiv.ycmass_ho > 0.0)
			cropindiv.yield = cropindiv.ycmass_ho * indiv.pft.harv_eff * c_to_dm;// Should be /0.446 instead
		else
			cropindiv.yield = 0.0;

		// Yield dry wieght of actually harvest products this year; NB as above
		if (cropindiv.harv_cmass_ho > 0.0)
			cropindiv.harv_yield=cropindiv.harv_cmass_ho * indiv.pft.harv_eff * c_to_dm;
		else
			cropindiv.harv_yield = 0.0;
		...
		for (int i=0;i<2;i++) {
			if (cropindiv.cmass_ho_harvest[i] > 0.0)
				cropindiv.yield_harvest[i] = cropindiv.cmass_ho_harvest[i] * indiv.pft.harv_eff * c_to_dm;
			else
...
/// SSR: Function that apportions manure C and N to various pools
void add_fertilizer_manure(Patch& patch, cropphen_struct& ppftcrop, double nfert) {
	...
	if (nfert_manure > 0.0) {
		// Inorganic N fraction of manure
		patch.dnfert += nfert_manure * (1.0 - manure_organic_frac);

		// Organic N fraction of manure: Half to metabolic pool, half to structural pool
		...
		patch.snfert += nfert_manure * manure_organic_frac;  // SSR: GGCMI

		// C content of manure: 25% to metabolic pool, 75% to structural pool
		...
		// Total N content of manure
		patch.fluxes.report_flux(Fluxes::MANUREN,nfert_manure);
		// anfert and snfert got organic manure N added above; inorganic manure N will be added at end of PFT loop in nfert_crop() with +=dnfert
...
void nfert_crop(Patch& patch) {
	...
	while(pftlist.isobj) {
		...
		if (patch.stand.pft[pft.id].active && pft.phenology == CROPGREEN) {
			...
			if (ppftcrop.growingdays==1) {
				...
				// SSR
				double N_appfert_mt = stlist[patch.stand.stid].get_management().N_appfert_mt;
				if (N_appfert_mt >= 0.0) {
					ppftcrop.nfert = N_appfert_mt;
				}
    ...
    patch.snfert += patch.dnfert; // SSR: GGCMI
...
//DKB: Pasture fertilization routine: 08.04.2023
void nfert_pasture(Patch& patch) {
	...
	// To be doubly sure, perform check here even though check is performed in calling function
	...
	// Get a reference to the PFT
	Pft& pft = pftlist[patch.stand.stid];

	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///Below are the initializers in the Pft constructor. In order for the checks below to work correctly
	//without directly tampering with the initializers in the Pft constructor, as they may be used for validity checks
	//elsewhere in the code, I re-initialized the pasture Pfts here
	/*fertrate[0] = 0.0;
	fertrate[1] = 1.0;
	fertdates[0] = 0;
	fertdates[1] = 30;*/

	//dprintf("The rates before are: (%f, %f) for the pft %s and stand %s\n", pft.fertrate[0], pft.fertrate[1], (char*)pft.name, (char*)st.name);
	...
	// Step 1: Check if the fertilizer amount is specified
	if (st.get_management().N_appfert_mt < 0) {
		// Fertilizer amount not specified, set amount to zero
		if (pft.fertdates[0] != -1 || pft.fertdates[1] != -1 || pft.fertrate[0] != -1 || pft.fertrate[1] != -1) {
			//dprintf("!!Fertilizer dates and/or rates specified but no fertilizer amount provided. Setting fertilizer amount to zero!!\n");
		}
		st.get_management().N_appfert_mt = 0.0;
		//dprintf("here 1\n");
	}
	else {
		// Step 2: Fertilizer amount specified, handle different cases

		//dprintf("here 2\n");
		// Case 1: Amount and Dates specified but No Rates
		if (pft.fertrate[0] == -1 && pft.fertrate[1] == -1 && pft.fertdates[0] != -1 && pft.fertdates[1] != -1) {
			//dprintf("!!Fertilizer amount and dates specified but no rates provided. Setting rates to 0.5 each!!\n");
			pft.fertrate[0] = 0.5;
			pft.fertrate[1] = 0.5;
		}

		// Case 2: Amount and Rates specified but No Dates
		if (pft.fertdates[0] == -1 && pft.fertdates[1] == -1 && pft.fertrate[0] != -1 && pft.fertrate[1] != -1) {
			//dprintf("!!Fertilizer amount and rates specified, but no fertilizer dates provided. Setting fertilizer dates to day 120 and day 240!!\n");
			pft.fertdates[0] = 120;
			pft.fertdates[1] = 240;
		}

		// Case 3: Amount specified but No Rates and No Dates
		if ((pft.fertrate[0] == -1 && pft.fertrate[1] == -1) && (pft.fertdates[0] == -1 && pft.fertdates[1] == -1)) {
			// Apply an equal amount of fertilizer every day
			//dprintf("Applied an equal amount everyday\n");
			nfert = st.get_management().N_appfert_mt / date.year_length();
		}

		// Case 4: All Three (Amount, Dates, and Rates) Specified
		if (pft.fertdates[0] != -1 && pft.fertdates[1] != -1 && (pft.fertrate[0] != -1 && pft.fertrate[1] != -1)) {
			// Perform the check to ensure that rates sum to 1
			if (pft.fertrate[0] + pft.fertrate[1] != 1.0) {
				//dprintf("!!Fertilizer application rates do not sum up to 1 in one of your pasture PFT definitions. Normalizing the rates to sum up to 1!!\n");
				//dprintf("The rates are: (%f, %f) for the pft %s\n", pft.fertrate[0], pft.fertrate[1], (char*)pft.name);
				//system("pause");
				

				// Normalize the rates to ensure they sum to 1
		...
		// Apply the fertilizer for specific days if dates and rates are set
	...
	// Apply the fertilizer to the patch
	patch.dnfert = nfert;
	patch.anfert += patch.dnfert;
}
...
void nfert(Patch& patch) {
	...
	if (stand.landcover == PASTURE) {
		// //DKB:Pasture fertilization here: 08.05.2023
		nfert_pasture(patch);
		return;
	}
