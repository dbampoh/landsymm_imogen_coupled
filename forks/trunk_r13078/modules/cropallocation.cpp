	...
	while (vegetation.isobj) {
		...
		if(indiv.pft.phenology == CROPGREEN) {
			...
			if(ppftcrop.growingseason) {
				...
				// SSR
				if (ppftcrop.growingdays - 1 > date.day) {
					indiv.patchpft().cmass_ho_harvest_lastyear = cropindiv.cmass_ho_harvest_ggcmi;
					indiv.vegetation.patch.grs_w_irr_lastyear = indiv.vegetation.patch.grs_w_irr;
				} else {
					indiv.patchpft().cmass_ho_harvest_thisyear = cropindiv.cmass_ho_harvest_ggcmi;
					indiv.vegetation.patch.grs_w_irr_thisyear = indiv.vegetation.patch.grs_w_irr;
				}
