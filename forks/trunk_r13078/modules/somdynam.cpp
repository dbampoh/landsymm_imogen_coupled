...
void decayrates_century(Soil& soil, double temp_soil, double wcont_soil, bool tillage) {
	...
	bool ismineralwetland = soil.patch.stand.is_true_wetland_stand() || soil.patch.stand.hydrology==INUNDATED;
...
void somfluxes(Patch& patch, bool ifequilsom, bool tillage) {
	...
	if (ifnlim) {
		...
		if(!(soil.NH4_mass > -EPS)) {
			dprintf("soil.NH4_mass = %f, -EPS = -%20.20f, Year = %i, Day = %i\n", soil.NH4_mass, EPS, date.year, date.day);
		}
	}
	...
	if(!ifntransform) {
        ...
        // SSR: GGCMI
        patch.grs_n_losses += nflux;
	}
	soil.nmass_inc(nmin_inc,NH4);
    
    // SSR: GGCMI
    patch.grs_n_input += nmin_inc;
...
void transfer_litter(Patch& patch) {
	...
	// SPITFIRE
	double fuelbulkdensity[NSOMPOOL];
	double surfacetovolume[NSOMPOOL];
	double moistureofextinction[NSOMPOOL];
	double em_CO2[NSOMPOOL];
	double em_CO[NSOMPOOL];
	double em_CH4[NSOMPOOL];
	double em_TPM[NSOMPOOL];
	double em_VOC[NSOMPOOL];

	if( firemodel == SPITFIRE ) {

		fuelbulkdensity[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].fuelbulkdensity;
		fuelbulkdensity[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].fuelbulkdensity;
		fuelbulkdensity[SURFFWD]    = soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFFWD].fuelbulkdensity;
		fuelbulkdensity[SURFCWD]    = soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFCWD].fuelbulkdensity;

		surfacetovolume[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].surfacetovolume;
		surfacetovolume[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].surfacetovolume;
		surfacetovolume[SURFFWD]    = soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFFWD].surfacetovolume;
		surfacetovolume[SURFCWD]    = soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFCWD].surfacetovolume;

		moistureofextinction[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].moistureofextinction;
		moistureofextinction[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].moistureofextinction;
		moistureofextinction[SURFFWD]    = soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFFWD].moistureofextinction;
		moistureofextinction[SURFCWD]    = soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr * soil.sompool[SURFCWD].moistureofextinction;

		em_CO2[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].em_CO2;
		em_CO2[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].em_CO2;
		em_CO2[SURFFWD]    = soil.sompool[SURFFWD].cmass * soil.sompool[SURFFWD].em_CO2;
		em_CO2[SURFCWD]    = soil.sompool[SURFCWD].cmass * soil.sompool[SURFCWD].em_CO2;

		em_CO[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].em_CO;
		em_CO[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].em_CO;
		em_CO[SURFFWD]    = soil.sompool[SURFFWD].cmass * soil.sompool[SURFFWD].em_CO;
		em_CO[SURFCWD]    = soil.sompool[SURFCWD].cmass * soil.sompool[SURFCWD].em_CO;

		em_CH4[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].em_CH4;
		em_CH4[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].em_CH4;
		em_CH4[SURFFWD]    = soil.sompool[SURFFWD].cmass * soil.sompool[SURFFWD].em_CH4;
		em_CH4[SURFCWD]    = soil.sompool[SURFCWD].cmass * soil.sompool[SURFCWD].em_CH4;

		em_VOC[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].em_VOC;
		em_VOC[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].em_VOC;
		em_VOC[SURFFWD]    = soil.sompool[SURFFWD].cmass * soil.sompool[SURFFWD].em_VOC;
		em_VOC[SURFCWD]    = soil.sompool[SURFCWD].cmass * soil.sompool[SURFCWD].em_VOC;

		em_TPM[SURFSTRUCT] = soil.sompool[SURFSTRUCT].cmass * soil.sompool[SURFSTRUCT].em_TPM;
		em_TPM[SURFMETA]   = soil.sompool[SURFMETA].cmass   * soil.sompool[SURFMETA].em_TPM;
		em_TPM[SURFFWD]    = soil.sompool[SURFFWD].cmass * soil.sompool[SURFFWD].em_TPM;
		em_TPM[SURFCWD]    = soil.sompool[SURFCWD].cmass * soil.sompool[SURFCWD].em_TPM;
	...
	while (patch.pft.isobj) {
		...
		double current_pft_FBD;
		if (firemodel==SPITFIRE) {
			current_pft_FBD = fuel_bulk_density(pft.pft, patch.fuel.total_grass_biomass_kgDM, patch.get_climate().gdd5);

//			// MF DEBUG can be cleaned out eventually
//			if(current_pft_FBD != current_pft_FBD || current_pft_FBD > 35.0) {
//			   dprintf("FBD NAN: PFT SOM: (%.2f, %.2f): Year = %i, Day = %i, PFT = %s, grass biomass = %f,  current_pft_FBD =%f \n", patch.stand.get_gridcell().get_lon(), patch.stand.get_gridcell().get_lat(),  date.year, date.day, (char*)pft.pft.name, patch.fuel.total_grass_biomass_kgDM, current_pft_FBD);
//			}
		}
		...
		if (!negligible(cmass_litter_leaf) || !negligible(nmass_litter_leaf)) {
			...
			// SPITFIRE
				...
				fuelbulkdensity[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * current_pft_FBD;
				surfacetovolume[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.sigma_leaf;
				moistureofextinction[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.MoE;
				em_CO2[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.em_CO2;
				em_CO[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.em_CO;
				em_CH4[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.em_CH4;
				em_VOC[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.em_VOC;
				em_TPM[SURFSTRUCT] += cmass_litter_leaf * (1.0 - fm) * pft.pft.em_TPM;

				fuelbulkdensity[SURFMETA] += cmass_litter_leaf * fm * current_pft_FBD;
				surfacetovolume[SURFMETA] += cmass_litter_leaf * fm * pft.pft.sigma_leaf;
				moistureofextinction[SURFMETA] += cmass_litter_leaf * fm * pft.pft.MoE;
				em_CO2[SURFMETA] += cmass_litter_leaf * fm * pft.pft.em_CO2;
				em_CO[SURFMETA] += cmass_litter_leaf * fm * pft.pft.em_CO;
				em_CH4[SURFMETA] += cmass_litter_leaf * fm * pft.pft.em_CH4;
				em_VOC[SURFMETA] += cmass_litter_leaf * fm * pft.pft.em_VOC;
				em_TPM[SURFMETA] += cmass_litter_leaf * fm * pft.pft.em_TPM;
		...
		if (!negligible(cmass_litter_sap) || !negligible(nmass_litter_sap)) {
			...
			// MF DEBUG
			if(cmass_litter_sap < 0.0 && !negligible(cmass_litter_sap, -15.0)) {
				dprintf("NEG LITTER ERR: SAP: (%.2f, %.2f): Year = %i, Day = %i, Stand = %i, Patch = %i, litter_sap = %f \n", patch.stand.get_gridcell().get_lon(), patch.stand.get_gridcell().get_lat(),  date.year, date.day, patch.stand.id, patch.id, cmass_litter_sap);
				// abort();
			}
			...
			if (firemodel == GLOBFIRM) {
				...
				fuelbulkdensity[SURFFWD]   +=  (cmass_litter_sap * wood_fraction_1hr * current_pft_FBD) + (cmass_litter_sap * wood_fraction_10hr * current_pft_FBD * fbd_a) + (cmass_litter_sap * wood_fraction_100hr * current_pft_FBD * fbd_b);
				moistureofextinction[SURFFWD] += cmass_litter_sap * wood_fraction_1_10_100hr * pft.pft.MoE;
				surfacetovolume[SURFFWD] += (cmass_litter_sap * wood_fraction_1hr * sigma_1hr) + (cmass_litter_sap * wood_fraction_10hr * sigma_10hr) + (cmass_litter_sap * wood_fraction_100hr * sigma_100hr);
				em_CO2[SURFFWD] += cmass_litter_sap * pft.pft.em_CO2;
				em_CO[SURFFWD] += cmass_litter_sap * pft.pft.em_CO;
				em_CH4[SURFFWD] += cmass_litter_sap * pft.pft.em_CH4;
				em_VOC[SURFFWD] += cmass_litter_sap * pft.pft.em_VOC;
				em_TPM[SURFFWD] += cmass_litter_sap * pft.pft.em_TPM;
			}
		...
		if (!negligible(cmass_litter_heart) || !negligible(nmass_litter_heart)) {
			...
			// MF DEBUG
			if(cmass_litter_heart < 0.0 && !negligible(cmass_litter_heart, -15.0)) {
				dprintf("NEG LITTER ERR: HEART: (%.2f, %.2f): Year = %i, Day = %i, Stand = %i, Patch = %i, litter_heart = %f \n", patch.stand.get_gridcell().get_lon(), patch.stand.get_gridcell().get_lat(),  date.year, date.day, patch.stand.id, patch.id, cmass_litter_heart);
				// abort();
			}
			...
			// SPTFIRE
				...
				fuelbulkdensity[SURFCWD]   += (cmass_litter_heart * wood_fraction_1hr * current_pft_FBD) + (cmass_litter_heart * wood_fraction_10hr * current_pft_FBD * fbd_a) + (cmass_litter_heart * wood_fraction_100hr * current_pft_FBD * fbd_b);
				moistureofextinction[SURFCWD] += cmass_litter_heart * pft.pft.MoE * wood_fraction_1_10_100hr;
				surfacetovolume[SURFCWD] += (cmass_litter_heart * wood_fraction_1hr * sigma_1hr) + (cmass_litter_heart * wood_fraction_10hr * sigma_10hr) + (cmass_litter_heart * wood_fraction_100hr * sigma_100hr);
				em_CO2[SURFCWD] += cmass_litter_heart * pft.pft.em_CO2;
				em_CO[SURFCWD] += cmass_litter_heart * pft.pft.em_CO;
				em_CH4[SURFCWD] += cmass_litter_heart * pft.pft.em_CH4;
				em_VOC[SURFCWD] += cmass_litter_heart * pft.pft.em_VOC;
				em_TPM[SURFCWD] += cmass_litter_heart * pft.pft.em_TPM;
			}
		}
	...
	// FIRE
		...
		// NOTE that for each surface pool below we check that the denominator is not very small because that may introduce rounding/precision issues
		// in that case we set it to standard values

		// SURFSTRUCT
		if (!negligible(soil.sompool[SURFSTRUCT].cmass, -10)) {
			soil.sompool[SURFSTRUCT].fuelbulkdensity = fuelbulkdensity[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].surfacetovolume = surfacetovolume[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].moistureofextinction = moistureofextinction[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].em_CO2 = em_CO2[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].em_CO = em_CO[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].em_CH4 = em_CH4[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].em_VOC = em_VOC[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;
			soil.sompool[SURFSTRUCT].em_TPM = em_TPM[SURFSTRUCT] / soil.sompool[SURFSTRUCT].cmass;

		}
		else{
			set_standard_SOM_values(soil.sompool[SURFSTRUCT]);
		}

		// SURFMETA
		if (!negligible(soil.sompool[SURFMETA].cmass, -10)) {
			soil.sompool[SURFMETA].fuelbulkdensity = fuelbulkdensity[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].surfacetovolume = surfacetovolume[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].moistureofextinction = moistureofextinction[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].em_CO2 = em_CO2[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].em_CO = em_CO[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].em_CH4 = em_CH4[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].em_VOC = em_VOC[SURFMETA] / soil.sompool[SURFMETA].cmass;
			soil.sompool[SURFMETA].em_TPM = em_TPM[SURFMETA] / soil.sompool[SURFMETA].cmass;

		}
		else {
			set_standard_SOM_values(soil.sompool[SURFMETA]);
		}


		// SURFWD
		if (!negligible(soil.sompool[SURFFWD].cmass, -10)) {

			// for RoS variables we have only accumulated values the 1hr, 10hr and 100hr fuels so only divide by that
			soil.sompool[SURFFWD].fuelbulkdensity = fuelbulkdensity[SURFFWD] / (soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr);
			soil.sompool[SURFFWD].surfacetovolume = surfacetovolume[SURFFWD] / (soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr);
			soil.sompool[SURFFWD].moistureofextinction = moistureofextinction[SURFFWD] / (soil.sompool[SURFFWD].cmass * wood_fraction_1_10_100hr);

			// for emissions we accumulate everything
			soil.sompool[SURFFWD].em_CO2 = em_CO2[SURFFWD] / soil.sompool[SURFFWD].cmass;
			soil.sompool[SURFFWD].em_CO = em_CO[SURFFWD] / soil.sompool[SURFFWD].cmass;
			soil.sompool[SURFFWD].em_CH4 = em_CH4[SURFFWD] / soil.sompool[SURFFWD].cmass;
			soil.sompool[SURFFWD].em_VOC = em_VOC[SURFFWD] / soil.sompool[SURFFWD].cmass;
			soil.sompool[SURFFWD].em_TPM = em_TPM[SURFFWD] / soil.sompool[SURFFWD].cmass;

		}
		else {
			set_standard_SOM_values(soil.sompool[SURFFWD]);
		}

		// SURCWD
		if (!negligible(soil.sompool[SURFCWD].cmass, -10)) {

			// for RoS variables we have only accumulated values the 1hr, 10hr and 100hr fuels so only divide by that
			soil.sompool[SURFCWD].fuelbulkdensity = fuelbulkdensity[SURFCWD] / (soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr);
			soil.sompool[SURFCWD].surfacetovolume = surfacetovolume[SURFCWD] / (soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr);
			soil.sompool[SURFCWD].moistureofextinction = moistureofextinction[SURFCWD] / (soil.sompool[SURFCWD].cmass * wood_fraction_1_10_100hr);

			// for emissions we accumulate everything
			soil.sompool[SURFCWD].em_CO2 = em_CO2[SURFCWD] / soil.sompool[SURFCWD].cmass;
			soil.sompool[SURFCWD].em_CO = em_CO[SURFCWD] / soil.sompool[SURFCWD].cmass;
			soil.sompool[SURFCWD].em_CH4 = em_CH4[SURFCWD] / soil.sompool[SURFCWD].cmass;
			soil.sompool[SURFCWD].em_VOC = em_VOC[SURFCWD] / soil.sompool[SURFCWD].cmass;
			soil.sompool[SURFCWD].em_TPM = em_TPM[SURFCWD] / soil.sompool[SURFCWD].cmass;

		}
		else {
			set_standard_SOM_values(soil.sompool[SURFCWD]);
		}



		if (!(soil.sompool[SURFFWD].moistureofextinction >= 0.195 && soil.sompool[SURFFWD].moistureofextinction <= 0.305)) {
			dprintf("MOE ERR: SURFFWD SOM: (%.2f, %.2f): Year = %i, Day = %i, Stand = %i, Patch = %i, soil.sompool[SURFFWD].moistureofextinction =%f \n", patch.stand.get_gridcell().get_lon(), patch.stand.get_gridcell().get_lat(),  date.year, date.day, patch.stand.id, patch.id, soil.sompool[SURFFWD].moistureofextinction);
		}
...
void leaching(Soil& soil) {
	...
	if (nmin_avail > 0.0) {
        ...
        // SSR: GGCMI
        soil.patch.grs_n_losses += leaching;
		soil.sminleach += leaching;
	}
...
void soilnadd(Patch& patch) {
	...
	// TODO: SSR: Replace patch.hydrology==INUNDATED with soil.do_saturate() or patch.hydrology>=IRRIGATED_SAT in this next block?
	
	if (nmin_avail < NMASS_SAT || patch.hydrology==INUNDATED) {
		double k = 1.0;

		if (patch.hydrology==INUNDATED) {
			k = 7.0/3.0;
		}
		const double daily_nfix = soil.anfix_calc / (double)date.year_length() * k;

		if (nmin_avail + daily_nfix < NMASS_SAT || patch.hydrology==INUNDATED) {
	...
	// SSR: GGCMI
	patch.grs_n_input += soil.NH4_input + soil.NO3_input;
...
void vegetation_n_uptake(Patch& patch) {
	...
	while (vegetation.isobj) {
        ...
        if (indiv.pft.phenology == CROPGREEN && ifnlim) {
            ...
            // SSR: GGCMI
            indiv.cropindiv->nmass_uptake_harvest += nuptake_day;
        } else {
...
void equilsom(Soil& soil) {
	...
	// Set SPITFIRE-related SOM properties to sensible values if they are not defined (ie set them to not zero)
	// MF Consider doing this with the function set set_standard_SOM_values() 
