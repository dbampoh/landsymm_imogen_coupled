...
public:
	...
	/// SSR: Checks that relative humidity units are acceptable.
	/** Original version only allowed relative humidity with units '1', which needed
	 *  to be 0-1. Now only allows '1' if the variable has no scale_factor attribute,
	 *  or its scale_factor == 1.0. Also now allows units '%' with scale_factor == 0.01.
	 */
	void ssr_test_relhum_units() const;

	/// SSR: Checks that precip units are acceptable.
