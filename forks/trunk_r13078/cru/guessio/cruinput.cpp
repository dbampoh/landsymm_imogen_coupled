...
void CRUInput::init() {
	...
	// SSR: Assign (first,last)(hist,out)year. UNTESTED!
	if (firsthistyear != FIRSTHISTYEAR) {
		if (firsthistyear >= 0) {
			dprintf("firsthistyear in ins-file does not match FIRSTHISTYEAR; using FIRSTHISTYEAR (%d)\n", firsthistyear);
		}
		firsthistyear = FIRSTHISTYEAR;
	}
	if (lasthistyear != FIRSTHISTYEAR + NYEAR_HIST - 1) {
		if (lasthistyear >= 0) {
			dprintf("lasthistyear in ins-file does not match FIRSTHISTYEAR + NYEAR_HIST - 1; using FIRSTHISTYEAR + NYEAR_HIST - 1 (%d)\n", firsthistyear);
		}
		lasthistyear = FIRSTHISTYEAR + NYEAR_HIST - 1;
	}
	if (firstoutyear < 0) {
...
bool CRUInput::getclimate(Gridcell& gridcell) {
	...
	if (date.day == 0) {
        ...
        int nyear_hist_tmp = NYEAR_HIST;
		if (date.year < nyear_spinup) {

			// During spinup period

			if(date.get_calendar_year() == restart_year && restart) {

				int year_offset = date.year % NYEAR_SPINUP_DATA;
...
//        else if (date.year < nyear_spinup + NYEAR_HIST) {
		else if (date.year < nyear_spinup + min(nyear_hist_tmp, lasthistyear-firsthistyear+1)) {
