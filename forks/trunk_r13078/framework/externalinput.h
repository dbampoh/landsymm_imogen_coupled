...
private:
	...
	/// Number of years to increase land use fractions linearly from 0 to first year's values
	int nyears_lu_ramp;
	
	// SSR: LandSyMM (from g2p in old codebase): What is the first year (calendar) that should have full LU area (if nyears_cropland_ramp>0)?
	...
	// SSR: LandSyMM (from g2p in old codebase): What is the first year (calendar) that should have full cropland area (if nyears_cropland_ramp>0)?
	int firstfullyear_crop;
	
	// SSR: LandSyMM (from g2p in old codebase): Should ramp happen linearly to firstyear (TRUE, new behavior), or be a linear *fraction reduction* but potentially a nonlinear actual trend (FALSE, old behavior)?
	bool force_linear_ramp;
	
	// SSR: LandSyMM: Should we ignore all the above settings and just begin land use with the first year in file_LU?
...
private:
    ...
    /// SSR: GGCMI: Input objects for each GGCMI management text input file
    InData::TimeDataD phus;
    InData::TimeDataD pvds;
	InData::TimeDataD growseaslength;
	InData::TimeDataD Nfertdate2;

	/// Files names for management input file
	xtring file_sdates, file_hdates, file_Nfert, file_Nfert_st, file_NfertMan;
    
    /// SSR: GGCMI: Management inputs
    ...
    /// SSR: GGCMI: Gets PHU data for a year
    void getphu(Gridcell& gridcell);
    /// SSR: GGCMI: Gets PVD data for a year
    void getpvd(Gridcell& gridcell);
	/// SSR: GGCMI: Gets growing season length data for a year
    void getgrowseaslength(Gridcell& gridcell);
	/// SSR: GGCMI: Gets 2nd fertilization date for a year
