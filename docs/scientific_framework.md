...
**Version:** v0.15.0 (created at step 15 of the unified-codebase rebuild, 2026-05-08; **POST-BLOCK-8.2 climatemodel.cpp fork-divergence clarification banner added 2026-05-23 session 10 day 1 close**; **POST-BLOCK-8.2.4 banner B61 ✅ CLOSED + engine-side fork-parity reached 2026-05-26 session 11 day 2 afternoon**; **POST-BLOCK-8.2.5 banner chosen-pipeline = δ-B-variant locked at Phase G + B57+B59 ✅ CLOSED 2026-05-27 session 12 day 2**).

> **POST-BLOCK-8.2.5 chosen-pipeline + engine-side fork-parity reached banner** (per **B57 + B59 + B61 ✅ CLOSED** at session 11 day 2 afternoon + session 12 day 2; supersedes original session-10-day-1 fork-divergence banner — preserved below for forensic value per Rule #10 amendment-vs-rewrite):
>
> At v1.0 paper publication, the C++ IMOGEN engine is implemented at **`forks/trunk_r13078/modules/climatemodel.cpp`** (trunk's own fork; **byte-identical to rebuild's `lpjguess/modules/climatemodel.cpp`** per block 8.2.4 250/250 byte-identity acceptance test; engine-side Installment-2 slice ~1300 LOC forward-ported at block 8.2.4 commit `efe5ee7` + tag `v0.23.0-trunk-engine-forwardport-complete`; B61 ✅ CLOSED). Per **block 8.2.5 Phase G user choice (session 12 day 2 ~15:30 CEST 2026-05-27)** = **δ-B-variant pipeline** (trunk-C++ engine throughout: engine + LPJG + natural-emission preprocessor — methodologically consistent per Methods §2.2 framing locked at block 8.2.4; double-precision numerics throughout B62-clean; cleaner CO2_all.dat output ergonomics; warm/wet biome NPP fidelity 8/10 biomes within Smith2014/Pugh2019/Hickler2012/Friedlingstein2025GCB literature ranges per Phase F evidence). The trunk-T_seq LPJG consumes the pre-baked 5 × 18 GB δ-B-variant 62892-grid climate library at `forks/trunk_r13078_runs/<SSP>/Common-directory/IMOGEN/output_62892_cppengine/` on cluster as a pure consumer (`-input imogencfx` + `skip_inprocess_engine_run=1`; engine bypassed; LPJG main loop runs in sequential-standalone mode). The **δ-B Fortran-engine pipeline** at `runs/<SSP>/Common-directory-fortranengine/IMOGEN/output_62892/` (5 × 18 GB Fortran 62892-grid library; B62-noisy with ~+15-31 ppm secular CO2 drift hypothesized from single-precision accumulated rounding) is retained as v1+ switchable alternative per B57/B59 v1+ trajectory. **B57 + B59 ✅ CLOSED at block 8.2.5 Phase G**. The v1.0 architectural documentation below describes the **C++ engine architecture** (operational reference: trunk_r13078's engine post-block-8.2.4 = byte-identical to lpjguess's). See `_chat_artifacts/b8_2_4_trunk_engine_forwardport_2026-05-24/B8_2_4_evaluation_2026-05-26.md` for block 8.2.4 engine-side fork-parity evidence + `_chat_artifacts/b8_2_5_switchable_regrid_2026-05-26/B8_2_5_evaluation_2026-05-27.md` for block 8.2.5 + 8.4 chosen-pipeline evidence + Methods §2.2 PROPOSED disclosure text at `notes/PAPER_COMPLETION_AND_VALIDATION.md` §4.5.0a.

> **POST-BLOCK-8.2 fork-divergence clarification banner (ORIGINAL, 2026-05-23 session 10 day 1 close)** — **[SUPERSEDED by POST-BLOCK-8.2.5 banner above; preserved for forensic value per Rule #10 amendment-vs-rewrite — at the time of writing, B61 was OPEN + the v1+ Backport Sprint deferral framing was correct; B61 was subsequently ✅ CLOSED at block 8.2.4 session 11 day 2 afternoon via the engine-side fork-parity forward-port; chosen pipeline locked at block 8.2.5 Phase G session 12 day 2]**:
>
...
| **IMOGEN** | Pattern-scaling climate emulator + Joos ocean uptake + FAIR non-CO2 budget; takes integrated emissions → year-by-year monthly climate forcing | Huntingford et al. 2010 *GMD*; Smith et al. 2018 *GMD* (FAIR) | **v1.0 production**: C++ port `lpjguess/modules/climatemodel.cpp::RUN_IMOGEN_ENGINE()` via `-input imogencfx` (per block 8.1.5 findings §1; B53 doc-drift resolution). **Parity reference + REGRID-capable standalone**: Fortran `imogen/code/imogen_lpjg.f` (per Decision #2; ALLOCATABLE NGPOINTS at step 3; used for Option δ-B regrid pathway per block 8.1.5 §13) |
