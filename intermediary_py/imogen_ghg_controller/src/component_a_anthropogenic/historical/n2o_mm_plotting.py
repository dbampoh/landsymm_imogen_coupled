...
dg=pd.read_csv(OUT_DIR+'n2o_mm_global.csv'); dr=pd.read_csv(OUT_DIR+'n2o_mm_regional.csv')
yr=dg['Year'].values; t06s=dg['Total_2006split_GgN2O'].values/1000; t19s=dg['Total_2019split_GgN2O'].values/1000
fao=dg['FAO_published_GgN2O'].values/1000; edgar=dg['EDGAR_GgN2O'].values/1000
SP=['Dairy_GgN2O','OtherCattle_GgN2O','Buffalo_GgN2O','Swine_GgN2O','Layers_GgN2O','Broilers_GgN2O','Ducks_GgN2O','Turkeys_GgN2O']
SPL=['Dairy cattle','Other cattle','Buffalo','Swine','Layer hens','Broilers','Ducks','Turkeys']
sp=[dg[c].values/1000 if c in dg.columns else np.zeros(51) for c in SP]
...
rd={r:dr[dr['Region']==r].sort_values('Year')['Total_2006split_GgN2O'].values/1000 if len(dr[dr['Region']==r])==51 else np.zeros(51) for r in R9}
...
SC2=['#1f77b4','#aec7e8','#ff7f0e','#d62728','#2ca02c','#98df8a','#9467bd','#c5b0d5']
...
fig.suptitle('N2O Emissions -- Manure Management\nIPCC Tier 1 vs FAO FAOSTAT GLE vs EDGAR (3.A.2) vs RCMIP CMIP6  |  1970-2020',fontsize=12,fontweight='bold',color='#1a1a1a',y=0.99)
ax=axes[0,0]; ax.fill_between(yr,fao,t06s,alpha=0.10,color=CO)
ax.plot(yr,rcmip_mm,color=CR,linewidth=2.0,linestyle=RL,label='RCMIP CMIP6 (EDGAR prop.)')
ax.plot(yr,t19s,color=C19,linewidth=1.5,linestyle='--',label='2019 params (region-specific AWMS)')
ax.plot(yr,edgar,color=CE,linewidth=1.8,linestyle='-.',label='EDGAR 2025 (3.A.2)')
ax.plot(yr,t06s,color=CO,linewidth=2.0,label='2006 params, detailed AWMS (best FAO match)')
ax.plot(yr,fao,color=CF,linewidth=2.5,linestyle=':',label='FAO TIER 1')
ax.set_title('Global N2O Totals',**TK); ax.set_ylabel('N2O (Mt yr\u207b\u00b9)',**LK); ax.set_xlabel('Year',**LK); sax(ax,(0.15,1.10)); ax.legend(fontsize=7.5,loc='upper left',framealpha=0.9,edgecolor=SC,fancybox=False)
ax=axes[0,1]; ax.stackplot(yr,sp,labels=SPL,colors=SC2,alpha=0.80)
ax.plot(yr,rcmip_mm,color='#333333',linewidth=2.2,linestyle=RL,label='RCMIP total',zorder=11)
...
ax.set_title('Species Breakdown (2006 params, detailed AWMS)',**TK); ax.set_ylabel('N2O (Mt yr\u207b\u00b9)',**LK); ax.set_xlabel('Year',**LK); sax(ax,(0,0.80)); ax.legend(fontsize=7,loc='upper left',framealpha=0.9,edgecolor=SC,fancybox=False)
ax=axes[1,0]; ax.axhline(1.0,color='#888888',linewidth=1.2)
_lo,_hi=emp_band(fao,t06s,edgar,rcmip_mm)
...
ax.plot(yr,t19s/fao,color=C19,linewidth=1.5,linestyle='--',label='2019 params / FAO')
ax.plot(yr,edgar/fao,color=CE,linewidth=1.8,linestyle='-.',label='EDGAR / FAO')
ax.plot(yr,t06s/fao,color=CO,linewidth=2.0,label='2006 params / FAO')
ax.set_title('Ratio to FAO Published N2O',**TK); ax.set_ylabel('Ratio (Series / FAO)',**LK); ax.set_xlabel('Year',**LK); sax(ax,(0.50,2.60)); ax.legend(fontsize=8,loc='upper left',framealpha=0.9,edgecolor=SC,fancybox=False)
...
ax.plot(yr,edgar,color=CE,linewidth=1.8,linestyle='-.',label='EDGAR total',zorder=10)
ax.plot(yr,fao,color=CF,linewidth=2.5,linestyle=':',label='FAO total',zorder=12)
ax.set_title('Regional Breakdown (2006 params, detailed AWMS)',**TK); ax.set_ylabel('N2O (Mt yr\u207b\u00b9)',**LK); ax.set_xlabel('Year',**LK); sax(ax,(0,0.80)); ax.legend(fontsize=7,loc='upper left',framealpha=0.9,edgecolor=SC,fancybox=False)
fig.text(0.5,0.003,f'2020: 2006-params={t06s[-1]:.3f}Mt | FAO={fao[-1]:.3f}Mt | EDGAR={edgar[-1]:.3f}Mt | RCMIP-derived={rcmip_mm[-1]:.3f}Mt  |  Note: EDGAR 3.A.2 uses strict managed-manure boundary; RCMIP: CMIP6 hist+SSP2-4.5 (2016-19 interpolated)',ha='center',fontsize=7,color='#555555',style='italic',bbox=dict(boxstyle='round,pad=0.4',facecolor='#f5f5f0',edgecolor='#cccccc',alpha=0.9))
plt.tight_layout(rect=[0,0.04,1,0.97]); plt.savefig(FIG_DIR + 'n2o_mm_trends.png',dpi=300,bbox_inches='tight',facecolor=fig.get_facecolor()); plt.close(); print("Saved: n2o_mm_trends.png")
