"""
scenario_ch4_ef_plotting.py
============================
Self-contained plotting script for CH4 Enteric Fermentation scenario
emissions (2020-2100, PLUMv2 s1 central realisations).

Reads:
  scenario_ch4_ef_global.csv   — global total per scenario and year
...
Produces:
  scenario_ch4_ef_trends.png  (4-panel, 300 dpi)
...
HIST_DIR = os.path.join(str(OUT_A_DATA), 'ch4_ef')
...
rcmip_yrs, rcmip_vals = build_rcmip_hist('Emissions|CH4','3.A.1',EDGAR_CH4)

# =============================================================================
# LOAD DATA
# =============================================================================
print("Loading emissions data ...")
sg = pd.read_csv(os.path.join(str(OUT_A_DATA), 'scenario_pipeline', 'scenario_ch4_ef_global.csv'))
sr = pd.read_csv(os.path.join(str(OUT_A_DATA), 'scenario_pipeline', 'scenario_ch4_ef_regional.csv'))
hg = pd.read_csv(os.path.join(HIST_DIR,'ch4_ef_global.csv'))
hr = pd.read_csv(os.path.join(HIST_DIR,'ch4_ef_regional.csv'))
...
fig.suptitle('CH4 \u2014 Enteric Fermentation\n'
...
# Single combined legend
legend_handles = [
    Line2D([0],[0],color=C_HIST, lw=2,  label='LandSyMM IPCC Tier 1 (2019 Ref.)'),
