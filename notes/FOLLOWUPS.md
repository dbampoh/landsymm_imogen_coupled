...
| **B33** 🔧 PARTIAL | **`runs/<SCEN>/imogen_intermediary.ins` Option C robustness: the POSIX-path-concat at `imogen_lpjg.f:565-566` (`OPEN(63, FILE=TRIM(ADJUSTL(DIR_COMMON))//'/LPJG_main/IMOGEN/'//FILE_LPJG_FLUX)`) silently resolves to the absolute path when `FILE_LPJG_FLUX` starts with `/`** — collapsing the intended `<DIR_COMMON>/LPJG_main/IMOGEN/` prefix into a no-op. This is the "loose-masquerading-as-tight" architectural footgun documented in `COUPLED_MODEL_INVESTIGATION.md` §3.7. Currently DORMANT in our rebuild (Option C is commented out + uses correct relative paths when commented), but a maintainer adding absolute paths to Option C while reactivating tight mode in v1.1+ would silently re-trigger the bug. Fix: (a) ensure `imogen_intermediary.ins` Option C entries permanently use plain relative filenames (e.g., `imogen_lpjg_flux.txt`) so engine concat correctly resolves to `<DIR_COMMON>/LPJG_main/IMOGEN/imogen_lpjg_flux.txt`; (b) add a launcher pre-flight check that warns/errors if `FILE_LPJG_FLUX` is an absolute path while `coupling_mode "tight"` is set; (c) add a Fortran-side defensive runtime warning per rule #8 (~5 LOC) at the engine read site if the resolved path looks suspicious. SURFACED at B19 Phase 1 CLOSE (2026-05-16 evening) via rediscovery from `COUPLED_MODEL_INVESTIGATION.md` §3.7 during user-driven double-counting investigation. | **~1 h** (sub-item (b) actual ~5 min bundled with B31 Commit 1; sub-item (a) actual ~30 min at Commit 2 incl. harness + cascade; sub-item (c) deferred to Commit 3) | **B19 Phase 2 (sub-item (b) ✅ CLOSED at Commit 1; sub-item (a) ✅ CLOSED at Commit 2; sub-item (c) at Commit 3)** | 🔧 **PARTIAL — sub-items (a) + (b) ✅ CLOSED; sub-item (c) ⏳ OPEN**. **(b) ✅ CLOSED 2026-05-16/17 night at commit `d7a0673`** (bundled with B31 in Commit 1): launcher pre-flight check in `scripts/run_coupled.sh` step 4.5 aborts if `coupling_mode "tight"` + active `FILE_LPJG_FLUX`/`FILE_LPJG_CH4_N2O_FLUX` starts with `"/`; ~12 LOC; defense-in-depth (auto-rewrite is primary defense; pre-flight catches edge cases). **(a) ✅ CLOSED 2026-05-17 noon at this commit** (Commit 2): inline-comment strengthening at `runs/SSP1-2.6/imogen_intermediary.ins` Option C block (lines 191-200 → expanded; +47/-6 LOC); pure documentation hardening covering 5-INQUIRE + 2-OPEN engine source-site citations + POSIX-concat collapse mechanic + canonical §3.7 cross-ref + 3-layer defense-in-depth narrative + maintainer directive. ZERO behavioral impact harness-verified (W0-W6 all PASS; harness + log at `_chat_artifacts/b19_phase2_c2_zero_behavior_check_{harness_,}2026-05-17.{sh,log}`; baseline post-toggle sha1 `e4e25ae3d7b9a6e63758e0a8c47623b5c0de3ede` matches Commit 1's V5 independent cross-verification). **Sub-item (c) remains ⏳ OPEN, scheduled for B19 Phase 2 Commit 3 immediately following** (the only TRUNK-RELEVANT piece — ~10-15 LOC of Fortran defensive `PRINT *` at 5 INQUIRE + 2 OPEN sites in `imogen_lpjg.f` lines 411-425 + 619-632 firing if the resolved concat path looks pathological). PARTIALLY-TRUNK-RELEVANT (sub-item (c) is TRUNK-RELEVANT; sub-items (a) + (b) are TRUNK-IRRELEVANT-by-novelty). |
