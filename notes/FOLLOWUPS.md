...
| F-12 | **PARTIALLY DONE** — C1 byte-equality ✅ CLOSED (TAGGED `v0.17.0-step17a-c1-year-outer-single-process` 2026-05-10); C1 substantive-validation ✅ CLOSED (B12 RESOLVED 2026-05-11 evening session 3; root cause: `ifcalccton 0` in smoke .ins; fix: `ifcalccton 1 + ifcalcsla 1`; ALL 4 xval scenarios PASS substantive validation); **C2 prep ✅ done** (`1405ada`); **C2 core ✅ done** (`f13b302`; MPI mechanics verified + xval harness hardened); **B12 ✅ DONE** (this commit; ~2 hours work); B1+B4+B2+B3+B6+B10 bundles ⏳ NEXT (~6-8 d); full mpirun -np 4 mimic + close-out tag after bundles; C3 ⏳ after C2 close-out | C1 byte-eq + substantive-eq closed; C2 prep + core done; B12 done; **B1+B4+B2+B3+B6+B10 + C2 close-out** (~9-13 days remaining; tag target `v0.17.5-step17b-c2-mpi-sync`) → C3 (~1-2 weeks SSH iterative; cluster) | C2 close-out + C3 still **Yes** (cluster MPI tight gates on C2 close-out + C3) |
...
**Adopted estimate going forward (2026-05-11 evening session 3 post-B12 resolution)**: **~52-55% done; ~45-48% remaining (~28-37 days median; range 22-43)**. The substantive-validation gap that previously discounted us to ~40-45% has been CLOSED — B12 took ~2 hours of session 3 work (vs the 2-10 d unbounded estimate), so the remaining work shrinks back to roughly the pre-B12-finding estimate of ~33-37 d minus B12's now-completed effort, plus the small B13 defensive-hardening item added. The C1 baseline now PRODUCES non-NaN scientific output for the first time in our rebuild; the byte-equality + non-NaN GO/NO-GO gate per session-2 §9.5 PASSES for all 4 xval scenarios.
...
| B1 | Fortran Rh + Wind COMPUTATION port | 3-5 d | **C2 era (step 17b); B12 ✅ done; safe to land now** | ⏳ NEXT |
| B2 | Fortran Tmin/Tmax write block | 0.5 d | **C2 era (step 17b); B12 ✅ done** | ⏳ ready |
| B3 | C++ Tmin/Tmax in REGRID branch | 0.5 d | **C2 era (step 17b); B12 ✅ done** | ⏳ ready |
| B4 | ImogenInput Rh/W/Tmin/Tmax consumer wiring expansion | 1 d | **C2 era (step 17b); B12 ✅ done** | ⏳ ready |
| B5 | F-9 / step 9.5c miscoutput diagnostic outputs | 1-2 d | **Step 18** (docs/reproducibility era) | ⏳ pending step 18 |
| B6 | F-2 Fortran T_anom 2× line count | 0.5 d | **C2 era (step 17b); B12 ✅ done** (with B1's Fortran work) | ⏳ ready |
...
| B10 | Symmetric Fortran engine writer fix | 0.5 d | **C2 era (step 17b); B12 ✅ done** (with B1's Fortran work) | ⏳ ready |
...
**C2 era combined sprint** (step 17b) — REVISED 2026-05-11 evening session 3 (B12 RESOLVED): C2 core (3-5 d done) + B12 (~2 hours done) + B1 (3-5 d) + B2 (0.5 d) + B3 (0.5 d) + B4 (1 d) + B6 (0.5 d) + B10 (0.5 d) = **~9-13 days remaining** (was 11-23+ before B12 closure).
