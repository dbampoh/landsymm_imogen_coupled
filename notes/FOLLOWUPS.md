...
| **B34** | **Year-range mismatch: `intermediary_py` outputs (1900-2100) vs smoke config (1871-1872)** — `runs/SSP1-2.6/inputs/imogen_lpjg_flux.txt` + `imogen_lpjg_ch4_n2o_flux.txt` (the step-13 adapter's natural-flux outputs for the `--backbone intermediary-py` Option B path) cover years 1900-2100 (201 rows); the `--smoke` run-mode configuration in `scripts/run_coupled.sh` hardcodes 1871-1872. Run A of B19 Phase 3 (2026-05-17 evening; first attempt with `--backbone intermediary-py --coupling-mode prescribed --scenario SSP1-2.6 --smoke --no-build --no-intermediary --no-adapter`) crashed with `Emission dataset does not match run` + `could not open ./IMOGEN/output/1871/T_anom.dat` (EC=99) because the engine could not find natural-flux data for year 1871 in the intermediary-py-derived flux files. Phase 3 fell back to Run B (`--backbone static-iiasa`) with year-range-compatible CMIP6 reference files (1850-2100, 251 rows) for the engine round-trip verification. Static-iiasa Option A (CMIP6 ref) is the path that works for smoke; `intermediary-py` Option B requires either pipeline-end year ≥1900 or extension of `intermediary_py` to pre-1900. Three options: **(α)** extend `intermediary_py` outputs to cover pre-1900 (zero-fill, baseline-repeat, or proper backwards-extrapolation; document the methodology); **(β)** adjust `--smoke` default year range to start ≥1900 (1900-1901 instead of 1871-1872; trivial; preserves intermediary-py compatibility but loses historical-coverage testing for static-iiasa); **(γ)** document `static-iiasa` as the v1.0 default for smoke + `intermediary-py` as the full-historical/projection workhorse; document the year-range constraint explicitly. Run A's crash log + post-mortem preserved at `_chat_artifacts/b19_phase3_smoke_run/launcher_full_2026-05-17.log`. SURFACED at B19 Phase 3 (2026-05-17 evening) during the first engine-round-trip smoke attempt with `intermediary-py` backbone. | **~1-3 h** depending on chosen option (α ~3 h; β ~30 min; γ ~1 h) | **B19 Phase 5 close-out OR post-B19 cleanup** (should be addressed before any tight-coupling work since the intermediary-py path is what tight-coupling will exercise) | ⏳ pending; TRUNK-IRRELEVANT-by-novelty (intermediary_py + launcher are per-fork) |
| **B35** | **Cosmetic launcher skip-message bug: `scripts/run_coupled.sh` skip messages hardcode `backbone=static-iiasa` text** even when `--backbone intermediary-py` is passed and the steps being skipped (steps 2 + 3 = intermediary_py end-to-end + step-13 adapter) are the ones controlled by the `--no-intermediary` + `--no-adapter` flags. Observed in B19 Phase 3 Run B launcher log (`_chat_artifacts/b19_phase3_smoke_run/launcher_static_iiasa_2026-05-17.log`) where Run B WAS using static-iiasa so the message happened to be correct; the bug only manifests cosmetically when the user passes `--backbone intermediary-py --no-intermediary --no-adapter`. Functional behavior is correct (the right steps are skipped based on flags); only the displayed text is wrong. Fix: replace the hardcoded "static-iiasa" string in the skip-message echo lines with the actual `${BACKBONE}` variable. ~5-10 LOC sed-style fix; bundle with any future `scripts/run_coupled.sh` revision (e.g., when addressing B34 or other launcher work). | **~5 min** | **Bundle with any future `scripts/run_coupled.sh` revision** (no need for standalone commit) | ⏳ pending; TRUNK-IRRELEVANT-by-novelty (launcher is per-fork) |
