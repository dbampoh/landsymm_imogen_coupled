...
**Last updated:** 2026-05-12 (session 3 continuation; **B2 LANDED** — Fortran Tmin/Tmax write block in `imogen/code/imogen_lpjg.f`; 9 surgical insertions adding `Tmin_anom.dat` (unit 100) + `Tmax_anom.dat` (unit 101) writers symmetrically to BOTH REGRID + non-REGRID branches via algebraic `Tmin = T - DTEMP/2`, `Tmax = T + DTEMP/2` (per Decision #11; same units = K); +59 LOC additive (no removals); canonical doc block at REGRID OPEN + 8 short cross-reference comments; symmetric with C++ in-process port at `lpjguess/modules/climatemodel.cpp` ~lines 952-953 (which has Tmin/Tmax in non-REGRID only — REGRID-side C++ addition is **B3**'s scope per `// TODO at step 9.5b` inline comment at `climatemodel.cpp` ~line 894). Unit numbers 100/101 chosen to mirror C++ ofstream variable names; verified unused in `imogen_lpjg.f` pre-B2. Verification: clean Fortran build (zero warnings; binary 133 696 B = +4096 vs pre-B2's 129 600 B); static unit-100/101 reference count = 26 (matches expected 4 OPENs + 2 CLOSEs + 8 LON/LAT writes + 8 data writes + 4 per-cell newlines); both `Tmin_anom.dat`/`Tmax_anom.dat` paths verified at REGRID branch lines 1041-1042 + non-REGRID branch lines 1136-1137; algebra parity with C++ confirmed (`/2.0` REAL division; same `(f10.3)` format; same Kelvin units). Empirical post-B2 engine smoke deferred (same as B10/B6: engine inputs not currently shipped; will be staged when B1 lands). Backport-RELEVANT (Fortran source change in standalone engine). **Cross-engine parity matrix post-B2**: Fortran has Tmin/Tmax in BOTH branches; C++ has it only in non-REGRID (B3 closes the C++ REGRID gap); Rh + Wind remain Fortran-side-only gaps (B1 closes them). C2-era estimate revised down to ~5.5-8.5 d remaining (was ~6.5-9.5 d). v1.0 % done estimate revised UP to ~55-58%; ~42-45% remaining. Step 17b remaining: B3 (next; ~0.5 d) → B4 (~1 d) → B1 (~3-5 d) → C2 close-out (~1-2 d). Full forensic record in `notes/STEP_17b.md` §3d.
...
| F-12 | **PARTIALLY DONE** — C1 byte-equality ✅ CLOSED (TAGGED `v0.17.0-step17a-c1-year-outer-single-process` 2026-05-10); C1 substantive-validation ✅ CLOSED (B12 RESOLVED 2026-05-11 evening session 3); **C2 prep ✅ done** (`1405ada`); **C2 core ✅ done** (`f13b302`); **B12 ✅ done** (`488c5a2`); **B10 ✅ done** (`3c00428`; symmetric Fortran engine writer fix; 7 conditional removals); **B6 ✅ done** (`24250b2`; F-2 subsumed by B10; ZERO source change; docs-only); **B2 ✅ DONE** (this commit, 2026-05-12; Fortran Tmin/Tmax write block in `imogen/code/imogen_lpjg.f`; 9 surgical insertions; +59 LOC additive; algebraic `Tmin = T - DTEMP/2`, `Tmax = T + DTEMP/2` in BOTH REGRID + non-REGRID branches; units 100/101); **B3+B4+B1 bundles ⏳ NEXT** (~4.5-6.5 d remaining per re-ordered Option A: B3→B4→B1, mechanical-first/physics-port-last); full mpirun -np 4 mimic + close-out tag after bundles; C3 ⏳ after C2 close-out | C1 byte-eq + substantive-eq closed; C2 prep + core done; B12+B10+B6+B2 done; **B3+B4+B1 + C2 close-out** (~5.5-8.5 days remaining; tag target `v0.17.5-step17b-c2-mpi-sync`) → C3 (~1-2 weeks SSH iterative; cluster) | C2 close-out + C3 still **Yes** (cluster MPI tight gates on C2 close-out + C3) |
...
| B3 | C++ Tmin/Tmax in REGRID branch | 0.5 d | **C2 era (step 17b); B12+B10+B6+B2 ✅ done** | ⏳ **NEXT** (per re-ordered Option A sequence: B10→B6→B2→**B3**→B4→B1; mirrors B2's algebra on the C++ side; closes `// TODO at step 9.5b` inline comment at `climatemodel.cpp` ~line 894) |
