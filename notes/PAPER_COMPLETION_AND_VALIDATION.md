...
>
> **✅ B64 CLOSED — remediation complete + empirically validated (session 13 day 2 2026-05-29 ~14:00 CEST)**: Daniel's workstation chat agent executed the splice fix (~33 min total wall) + rsynced the corrected 18 GB SSP4-6.0 climate library to the cluster. Cluster-side verification confirmed 5-way SSP T_anom byte-identity at years 1900/1950/1991/2000 (year 2000 non-CO2 RF now 1.133761 W/m² canonical; +21% bias removed; CO2_all.dat byte-identical at 373.813 ppm).
>
> **Two corrections to the planned-remediation framing above** (Rule #10 self-correction #26):
> 1. **Splice boundary = 2010/2011, NOT 2014/2015**. The non-460 SSPs already diverge slightly 2006-2010 and dramatically post-2010, indicating the rebuild's intermediary_py-era RF files use **2010/2011** as the historical/scenario boundary (not the CMIP6-standard 2014/2015). Corrected splice = shared CMIP6 historical 1850-2010 (from ssp126 source-of-truth) + ssp460 scenario 2011-2100. For Methods §2.2 / Supplementary: state the SSP4-6.0 non-CO2 RF historical (1850-2010) was re-spliced from the shared CMIP6 historical to remove a synthetic-interpolation artifact in the Tier-2-generated ssp460 file; the scenario period (2011-2100) was retained unchanged.
> 2. **CO2 trajectory UNCHANGED — only climate (temperature) forcing changed**. SSP4-6.0 year-2100 atmospheric CO2 = 631.42 ppm is identical pre- and post-fix, because engine-evolved atmospheric CO2 (Joos ocean uptake + C-cycle solver) is **decoupled** from the prescribed non-CO2 RF (CH4+N2O pass-through per Rule #10 #25). Only per-cell climate variables (T_anom etc.) for years 1900-2010 changed (now reflecting corrected historical non-CO2 RF). This is a useful Methods/Discussion point: the engine's CO2 cycle is independent of the prescribed non-CO2 RF channel, so the SSP4-6.0 CO2 results in the paper were never at risk — only the climate (and hence ecosystem-state) forcing for the historical period needed correction.
>
