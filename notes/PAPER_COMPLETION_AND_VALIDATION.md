...
**✅ HYBRID-PRE-BAKED ARCHITECTURE CLARIFICATION (2026-05-20 ~6:25 PM session 8.0.3 follow-up exchange; Rule #10 self-correction post block 8.0.3 acceptance close)**: v1.0 paper coupling is NOT "purely prescribed" as my prior over-statement framed it (~6:10 PM in same exchange) — it is **hybrid pre-baked**: intermediary_py merges CMIP6/SSP-RCP/EDGAR/RCMIP/FAO anthropogenic emissions WITH **pre-baked offline trunk_r13078 LPJG-natural-emission outputs** (`inputs/lpjg/{historical,scenarios/ssp*}/lpjg_{cflux,mch4,ngases}.out_*.gz`; ~1.5 GB total; 62,538 cells × 120 years historical + 80 years × 5 SSP-RCPs; one-shot inputs from prior trunk_r13078 production runs) into IMOGEN-format emission inputs. **Implication for Axis 4 design strength**: BOTH Track 1 (LPJG with standard `cfx` ISIMIP3b climate + prescribed CO2) AND Track 2 (LPJG with `imogencfx` IMOGEN-derived climate + IMOGEN-derived CO2 trajectory) trace back to **THE SAME pre-baked LPJG-natural-emission inputs** at intermediary_py's upstream — therefore any Track 1 vs Track 2 ecosystem-output difference is attributable purely to the IMOGEN-coupling (climate + CO2 trajectory) and NOT to differences in natural-emission inputs upstream of IMOGEN. This is actually a **cleaner experimental design** than I initially framed: Track 1 vs Track 2 isolates the IMOGEN-coupling effect very well (climate + CO2 = only differential). **v1.0 → v1.1+ transition framing** is NOT "prescribed-mode → tight-coupling" but rather "offline pre-baked LPJG-natural ingestion → live per-year LPJG↔intermediary_py↔IMOGEN handshake" (F-12 v1.1+; trunk_r13078 stays at backport-fork-parity per `notes/TRUNK_R13078_BACKPORT_LEDGER.md` §1.1; rebuild's `lpj-guess_imogen_landsymm/lpjguess/` becomes the live-handshake fork at v1.1+). **Paper Methods §2 should describe this hybrid-pre-baked architecture accurately + cite the upstream `inputs/lpjg/*.gz` provenance** (the LPJG runs that produced these `.gz` files need their own provenance documentation in Supplementary Materials — see **NEW B51** at `notes/FOLLOWUPS.md`). **Companion doc-review work**: `docs/scientific_framework.md` §5+§6 should be re-read for similar v1.0-coupling-mode mis-statements per **NEW B50** at `notes/FOLLOWUPS.md` (LOW priority; sessions 11-12 paper-writing phase). The original §1.4 text below remains correct in claiming LPJG-version-held-constant (T_seq design); only the upstream emissions framing needed clarification.
