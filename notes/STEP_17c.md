...
**Date 17c.0 PREP B17(b) decision (α vs β) + four-xval re-verification on B15+B16+B17-fixed HEAD landed:** _pending_ (sub-phase 17c.0.5; B17(b) decision blocks the four-xval re-verification because gates 7+8 will controlled-fail until B17(b) is either explicitly accepted via tolerance comparison (α) or root-caused and fixed (β))
...
## 1. 17c.0 PREP phase plan (revised post-B15 + post-B16 + post-B17 forensic + post-B17(a)-fix)

The 17c.0 PREP phase as originally planned (per session-3 handoff §0.4 → 5-phase plan item 1) was a single workstation `mpirun -np 4` mimic verification, fulfilling the deferred-from-C2 obligation. The B15 forensic (17c.0.0) surfaced two prerequisites that must land first; the B16 fix (17c.0.3) further surfaced B17 (a third prerequisite); the B17 forensic deep-dive (17c.0.4 Phase A) materially revised B17 from a single combined (a)+(b) sub-phase into a B17(a) close + B17(b) decision-deferral split. The revised PREP phase plan:
...
| 17c.0.4 | **B17 forensic deep-dive (Phase A; falsifies §3.4 hypothesis 1+2; reclassifies B17(b) as stochastic-process sensitivity per cell-iteration-order RNG slip; full empirical drift-magnitude characterization in new §3.8) + B17(a) fix (option (a1) from §3.6: harness sort-then-diff in `compare_outputs()` with `LC_ALL=C sort -k1,1n -k2,2n -k3,3n` on (Lon, Lat, Year), preserving header; ~100 LOC addition + 3 LOC modification in `scripts/cross_validate_year_outer.sh`).** B17(b) DECISION (α tolerance-based comparison vs β seed-tracking dprintf root-cause investigation) DEFERRED to 17c.0.5 because (1) B17(a) closure is the lower-risk well-understood deliverable; (2) Phase A clarified the empirical picture (1.4% max cell-total drift; classic stochastic-process sensitivity signature). Full landing record as §1.3 of this file. | 1 d (actual: ~5-6 h elapsed including Phase A forensic + implementation + dry-run verification + 8-gate verification + doc cascade) | ✅ DONE 2026-05-13 evening (this commit; B17(a) CLOSED via .sh-only harness change; B17(b) reclassified + decision deferred to 17c.0.5; ZERO C++ source change in this sub-phase) |
| 17c.0.5 | **B17(b) DECISION (α tolerance-based comparison upgrade to `compare_outputs()` vs β seed-tracking dprintf root-cause investigation in `lpjguess/modules/driver.cpp::randfrac()` + suspected loci in framework setup-phase ordering) + four-xval re-verification on B15+B16+B17(b)-fixed HEAD.** Per Phase A's empirically-clarified picture (per §1.3.3 + §3.8): (α) is now well-grounded as the science-aligned acceptance path (Decision-12 acceptance criterion 2; cell totals within ~1.4% relative); (β) is a +1-2 d focused investigation given that code-spelunking exhausted the easy hypotheses (per §3.8.4). Four-xval re-verify gates 7+8 must PASS substantive in EITHER (α) or (β) outcome. | 0.5-2 d depending on (α)/(β) | ⏳ NEXT (empirically triggered by 17c.0.4 gates 7+8 still controlled-fail at exit 2 with 17/37 SORTED_DIFFER) |
...
Total revised PREP estimate: ~4.5-7 d (was: ~4.5-7 d at end of 17c.0.3). The 17c.0.4 split (B17(a) close + B17(b) defer) preserves the total budget — same total work, just sequenced for lower-risk delivery. Sub-phases 17c.0.0 + 17c.0.1 + 17c.0.2 + 17c.0.3 + 17c.0.4 are landed; ~2-4 d remaining in PREP after this commit.
