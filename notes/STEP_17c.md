...
#### 1.7.8 What's next (session-5 opening agenda)

Per the user-authorised next-task cluster:

1. **B19** (essential closed-loop pipeline verification of stages 2-7 of the 8-stage coupled pipeline; ~2.5-4 d revised down from 3-7 d after 2026-05-15 night code-reality check). Phase 1 (re-run intermediary_py + step-13 adapter on current HEAD) → Phase 2 (wire production .ins to consume adapter outputs) → Phase 3 (IMOGEN engine round-trip in `coupling_mode = prescribed`) → Phase 4 (closed-loop cross-validation vs legacy A/B references; **the actual scientific validation milestone**) → Phase 5 (6-surface doc cascade landing + tag).
2. **B20** (recommended literature-comparison sanity check; ~1-2 d). Phase 1 (assemble published references; Tian 2020 GCB / Saikawa 2014 ACP / IPCC AR6 / Saunois 2020 ESSD) → Phase 2 (run B19 closed-loop pipeline at full-resolution global gridlist) → Phase 3 (plot vs literature envelope) → Phase 4 (doc cascade + tag).
3. **17c.1 → 17c.4 cluster phases** on KIT IMK-IFU `owl` (~1-2 weeks SSH-iterative).
