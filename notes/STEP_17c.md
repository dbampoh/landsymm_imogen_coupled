...
- [§3. B17 forensic surface — audit item B17 (this commit; fix in 17c.0.4)](#3-b17-forensic-surface--audit-item-b17-this-commit-fix-in-17c04)
  ...
  - [3.8 Reclassified B17(b) — stochastic-process sensitivity per cell-iteration-order RNG slip (NEW in 17c.0.4)](#38-reclassified-b17b--stochastic-process-sensitivity-per-cell-iteration-order-rng-slip-new-this-commit-17c04)
    - [3.8.5 Operational acceptance: provisional 2% tolerance (NEW in 17c.0.4-followup commit `2771939`)](#385-operational-acceptance-provisional-2-tolerance-decided-2026-05-13-night-session-4-supersedes-139--36-formal-implementation-path)
    - [3.8.5.5 Re-evaluation cadence (NEW in 17c.0.5; user-authorised 2026-05-13 night-late session 4)](#3855-re-evaluation-cadence-new-user-authorised-2026-05-13-night-late-session-4)
...
| 17c.0.6 | **C2 close-out tag annotation amendment decision (option a/b/c from §0.11).** Errata note OR re-tag based on 17c.0.5 evidence. ACTIONABLE as of 17c.0.5 commit since the four-xval re-verify on B15+B16+B17(a)+B17(b)-provisionally-accepted HEAD has now confirmed the underlying year_outer code substantively passes within the §3.8.5 provisional-acceptance envelope. (Numbering preserved from prior table; renumbered from 17c.0.5 in 17c.0.3 commit due to insertion of B17-fix sub-phase above.) | 0.2 d | ⏳ NEXT (after 17c.0.5; ACTIONABLE) |
| 17c.0.7 | **Workstation `mpirun -np 4` mimic verification** (the original PREP obligation). Adapts `scripts/run_parallel_mimic.sh` to support `coupling_mode "prescribed"` + `imogencfx` tight-mode variant for proper C2-core exercise. (Numbering preserved from prior table; renumbered from 17c.0.6 in 17c.0.3 commit due to insertion of B17-fix sub-phase above.) | 1 d | ⏳ pending (after 17c.0.6) |
| 17c.0.8 | **PREP phase close-out.** Doc cascade + (un-tagged) checkpoint commit on top of the cascade. (Numbering preserved from prior table; renumbered from 17c.0.7 in 17c.0.3 commit.) | 0.2 d | ⏳ pending (after 17c.0.7) |
| (deferred future sub-phase TBD) | **Formal Option α (tolerance-based comparison upgrade to `compare_outputs()`) OR Option β (seed-tracking dprintf root-cause investigation per §3.8.4) for B17(b) closure.** Reactivates only on a §3.8.5 re-evaluation trigger firing per §3.8.5.5 (routine drift exceeds 2% cell-total / per-PFT diverges in scientifically-consequential PFT/cell / C3-era cluster surfaces MPI-rank-specific drift / paper-stage figure surfaces qualitative finding sensitive to per-PFT noise). Default closure path: Option α (~0.5-1 d harness change). Identifier assigned at reactivation time. | 0.5-2 d (only if reactivated) | ⏸ DEFERRED (provisional acceptance per §3.8.5 active) |
Total revised PREP estimate: ~4.5-7 d (was: ~4.5-7 d at end of 17c.0.3). The 17c.0.4 split (B17(a) close + B17(b) defer) + 17c.0.4-followup operational acceptance + 17c.0.5 collapsed renumbering preserves the total budget — same total work, just sequenced for lower-risk delivery and with the formal Option α/β closure deferred outside the PREP critical path. Sub-phases 17c.0.0 + 17c.0.1 + 17c.0.2 + 17c.0.3 + 17c.0.4 + 17c.0.4-followup + 17c.0.5 are landed; ~1.4 d remaining in PREP after this commit (17c.0.6 + 17c.0.7 + 17c.0.8).
...
#### 3.8.5.5 Re-evaluation cadence (NEW; user-authorised 2026-05-13 night-late session 4)
