...
#### 1.7.8 What's next (POST-block-8.3 status update at 2026-05-28 evening, session 13 day 1 close — **Block 8.3 ✅ DONE — Cluster end-to-end smoke validated end-to-end on KIT IMK-IFU owl, milan/4×64=256; chosen-pipeline (δ-B-variant trunk-C++ engine throughout) Track 2 production workflow operational. Both HIST + SCEN smoke phases ✅ ran cleanly: HIST main `601955` ExitCode 0:0 in 1h29m on milan[05-08] (256-rank; 4 cells/rank; 500yr spinup loop FIRST_SPINUP_YEAR=1900 + history 1901-2020) + SCEN main `602049` ExitCode 0:0 in 9m45s on milan[03-06] (~9× faster as expected; restart=1 + restart_year=2020 + state_path absolute as designed; G6.1 NEW gate ✅ SCEN restart from HIST state empirically validated). 8/9 acceptance gates ✅ PASS + 1/9 ⚠️ PARTIAL (benign Svalbard barren-cell at lon=16.25 lat=78.25 missing from SCEN cmass.out: HIST showed Total cmass=0.000 throughout 1900-2020 for that cell; SCEN PLUM_scen LU forcing assigns no fractions → cell omitted; matches block 8.0.3 cell-2 high-Arctic LU-mismatch ⚠️ PARTIAL pattern). Rule #9 datapoint #34 fix from PREP commit `8e4ee8e` empirically validated (gridlist-split MPI parallelism via setup_run.sh sed substitution `(str "gridlist.txt")` → smoke gridlist basename per rank). **Rule #9 datapoint #35 NEW**: finishup_lpj_work.sh SLURM script-cache portability bug surfaced empirically when HIST finishup `601956` failed instantly with `ERROR: append_files.sh not found or not executable at /var/spool/slurmd/job601956/append_files.sh`; root cause = SLURM copies sbatch'd scripts to `/var/spool/slurmd/job<JOBID>/<scriptname>` cache; fix landed (env-overridable FINISHUP_SCRIPT_DIR with BASH_SOURCE fallback in finishup_lpj_work.sh + `--export=ALL,FINISHUP_SCRIPT_DIR=${SCRIPT_DIR}` in setup_run.sh's startguess.sh HEREDOC) + empirically validated by SCEN finishup `602050` running cleanly under SLURM in 54s vs HIST's 0s instant-fail. Per-biome physical sensibility (G5): 36/50 anchor cells within Phase F published Smith2014/Pugh2019/Hickler2012/Friedlingstein2025GCB literature ranges; biome-group counts ≥80% in-range across tropical evergreen + temperate deciduous + temperate evergreen + boreal + tundra + desert. Audit-evidence bundle at `_chat_artifacts/b8_3_cluster_smoke_2026-05-28/B8_3_evaluation_2026-05-28.md` (~365 LOC; 12 sections; gitignored). Full 9-surface doc cascade landed at this commit. Tag `v0.25.0-cluster-trunk-tseq-smoke-complete` lands at this commit (annotated). **POST-BLOCK-8.3 NEXT**: Track 2 production runs — 5 SSPs × HIST + SCEN; pre-launch checklist + wall-estimate refinement at B8_3_evaluation §12 + CLUSTER_SETUP §1 POST-BLOCK-8.3 banner. v1.0 % done UNCHANGED ~98-99%; calendar UNCHANGED ~5-9 weeks**)

#### 1.7.8 What's next (POST-block-8.2.5 FULL status update at 2026-05-27 afternoon, session 12 day 2 — Block 8.2.5 ✅ DONE — Switchable-regrid-strategy wiring Phase A-H ALL LANDED: both δ-B Fortran + δ-B-variant trunk-C++ pipelines BUILT + Phase F 50-cell biome-stratified production-config smoke side-by-side + Phase G user-chose δ-B-variant for v1.0 paper Track 2 + Phase H 9-surface doc cascade; B57 + B59 ✅ CLOSED; 2 new Rule #9 datapoints (#32 + #33) + 1 new Rule #10 datapoint (#25); **cluster path ACTIVE NEXT** per user direction = cluster prep + block 8.3 cluster smoke + block 8.4 cluster production-config delta + block 8.5 MPI pre-flight + Track 2 production runs on owl genius/256)

**Block 8.2.5 ✅ DONE 2026-05-27 afternoon session 12 day 2** (canonical landing record at `_chat_artifacts/b8_2_5_switchable_regrid_2026-05-26/B8_2_5_evaluation_2026-05-27.md` ~400 LOC; 8-gate acceptance for Phase D + E + F + Phase G chosen-pipeline disclosure + Rule #9 #23-#33 + Rule #10 #25 + §5 per-biome differential + §6 Methods §2.2 disclosure text + §7 files modified inventory + §8 cumulative state + §9 post-block-8.2.5 cluster path). **Both pipelines built**: 5 × 18 GB δ-B Fortran 62892-grid library at `runs/<SSP>/Common-directory-fortranengine/IMOGEN/output_62892/` + 5 × 18 GB δ-B-variant trunk-C++ 62892-grid library at `forks/trunk_r13078_runs/<SSP>/Common-directory/IMOGEN/output_62892_cppengine/` + 5 × 1.1 GB δ-B-variant NN-intermediate at `output_3698_cppengine/`. **Both pipelines re-run cleanly at session 12 day 1 post Rule #9 #33 fix** (the FastRegrid header-treatment bug AFFECTED the δ-B 62892 library produced at session 11 PARTIAL; user direction = fix + re-run BOTH). **Phase F 50-cell biome-stratified production-config smoke**: 4 run-dirs (2 pipelines × hist+scen with save_state=1+save_years="2020" hist → restart=1+restart_year=2020 scen per user's wpeat hist+ssp126 production pattern); per-dir ./guess symlinks; ISIMIP-rooted soilmap/simfire/ndep/popdens paths; both pipelines ~23-24 min wall each in parallel; all 8 Phase F gates PASS for both. **Phase F per-biome literature compliance**: 7-8 of 10 biomes within published Smith2014/Pugh2019/Hickler2012/Friedlingstein2025GCB NPP literature ranges for both pipelines (δ-B 7/10 @ 2000+2100; δ-B-variant 8/10 @ 2000 + 7/10 @ 2100; TIE). **Per-biome bias pattern is climate-forcing-driven NOT CO2-driven** (CO2 differs ~3% but ecosystem metrics differ ±50%+; sample cell -91.25,17.75 year 2050 January T_anom = δ-B 300.71 K vs δ-B-variant 297.69 K = ~3 K cooler in δ-B-variant): δ-B-variant favors warm/wet biomes (+24% tropical evergreen NPP); δ-B favors cold/dry biomes (boreal δ-Bv -40%; tundra -55%). **Phase G user choice (~15:30 CEST 2026-05-27) = δ-B-variant** per Methods §2.2 framing locked at block 8.2.4 ("trunk_r13078 throughout") + double-precision numerics (B62-clean) + cleaner CO2_all.dat output ergonomics + warm/wet biome NPP fidelity. δ-B retained as v1+ switchable alternative per B57/B59 v1+ trajectory.

**Rule #9 cumulative**: #33 (was #22 at block 8.2.4 close; +11 at block 8.2.5). **Rule #10 cumulative**: #25 (was #24 at block 8.2.4 close; +1 at block 8.2.5 self-correction on 2-axis pipeline differential — climate forcing + atmospheric CO2 both differ; CH4+N2O bit-identical as prescribed-input pass-through). **B57 + B59 ✅ CLOSED**. **B62** ⏳ NEW (Fortran-vs-C++ ~15-31 ppm secular CO2 drift hypothesis; v1+ investigation; NOT blocking v1.0 since both within published ranges). **B63** ⏳ NEW (gridlist filename misnomers; v1+ housekeeping).

**Phase H 9-surface doc cascade** (THIS COMMIT): FOLLOWUPS dashboard FULL entry + B57 + B59 closure (PARTIAL preserved per Rule #10) / TRUNK_R13078_BACKPORT_LEDGER §3 NEW "Block 8.2.5 LANDED" entry above PARTIAL (forensic) / CHANGELOG (FULL entry; PARTIAL preserved) / EXECUTION_PLAN row 17c FULL marker / STEP_17c §1.7.8 (this entry) / PAPER_COMPLETION §4.5.0 chosen-pipeline disclosure / CLUSTER_SETUP §1 POST-BLOCK-8.2.5-FULL / forks/README.md + forks/trunk_r13078_runs/README.md chosen-pipeline banners. **Block 8.2.5 source-edits LANDED at this commit** (Rule #3: user-runs-commit). Tag `v0.24.0-switchable-regrid-strategy-complete` reserved for this close commit.

**POST-BLOCK-8.2.5 FULL NEXT (per user direction = cluster path)**: cluster prep (git pull cluster mirror at `/bg/data/lpj/bampoh-d/lpj-guess_imogen_landsymm/` to v0.24.0 tag + trunk binary rebuild via cmake+make in build_owl/ + rsync 90 GB δ-B-variant 62892 library + 2.2 GB native 1631 library; ~half-day) → block 8.3 cluster end-to-end smoke (~0.5-1 day; tag candidate `v0.25.0-cluster-trunk-tseq-smoke-complete`) → block 8.4 cluster production-config delta + two-track directory restructure (`<SSP>_local/` + `<SSP>_cluster/` per session-11 pivot #2; main_hist.ins + main_scen.ins per SSP × 5; ~1-1.5 days) → block 8.5 cluster MPI pre-flight (~0.5 day) → Track 2 production runs (5 SSPs × 62538 cells × 1900-2100 × strict production npatch=25+spinup=500+save_state/restart on owl genius/256 × 3-day walltime; ~5-15 h cluster wall) → SCP outputs + validation triad + paper writing + v1.0 GMD submission (~5-9 weeks). Track 1 baseline uses user's existing wpeat production runs at `/bg/data/lpj/bampoh-d/landsymm_imogen_runs/integrated-4.1-ins2_landsymm_{hist,ssp*}_wpeat/` (block 8.6 moot).

---

#### 1.7.8 What's next (POST-block-8.2.4 status update at 2026-05-26 afternoon, session 11 day 2 close — Block 8.2.4 ✅ DONE — Trunk-engine forward-port (engine-side Installment-2 slice ~1300 LOC) + 5-SSP trunk-engine library production + 250/250 byte-identity verification with rebuild's lpjguess engine; B61 ✅ CLOSED; **block 8.2.5 switchable-regrid-strategy wiring ACTIVE NEXT** with δ-B Fortran + δ-B-variant trunk-cpp-engine pipelines via FastRegrid + 4-cell smoke side-by-side acceptance)

**Block 8.2.4 ✅ DONE 2026-05-26 afternoon session 11 day 2 close** (canonical landing record at `_chat_artifacts/b8_2_4_trunk_engine_forwardport_2026-05-24/B8_2_4_evaluation_2026-05-26.md` ~620 LOC; 8-gate acceptance evaluation + Phase A-H summary + Rule #9 datapoints #20/#21/#22 + Rule #10 datapoint #24 + 250/250 byte-identity table + paper Methods §2.2 lock-in). Source-edits in 5 files (`forks/trunk_r13078/framework/parameters.{h,cpp}` ~30 LOC + `modules/climatemodel.cpp` wholesale cp from lpjguess + `modules/imogencfx.cpp` head+tail reconstruction excising ~438-LOC year_outer block + `modules/CMakeLists.txt` 2 LOC) + 2 NEW files (`modules/imogenoutput.{h,cpp}` 821 LOC NEW) + 5 SSPs × imogen_intermediary.ins wholesale-replace from rebuild + 4 per-SSP relative-path depth-adjustments + 5 SSPs × main_engine_only.ins FILE_NON_CO2_VALS path-adjust + `scripts/run_trunk_engine_only.sh` +22 LOC bootstrap + TRUNK_BIN env-overridable. Trunk's freshly-built `forks/trunk_r13078/build_b824/guess` binary (2,943,808 bytes; sha1 `9ef4c4cfcef0d10d51a12c2e261147000cee7d6c`) produced 5-SSP engine libraries at `forks/trunk_r13078_runs/<SSP>/Common-directory/IMOGEN/output/` (5 × 202 year-dirs × 443 MB = ~2.2 GB) in ~30 min total wall (Phase D SSP1-2.6 canary ~13 min serial + Phase E 4-way parallel SSPs 2-5 ~17 min). **Phase G byte-identity verification: 250/250 ✅ md5 matches** (5 SSPs × 5 sentinel years × 10 climate variables). CO2 trajectory values numerically identical to block 8.2 Phase D lpjguess-engine values (SSP1-2.6 2100 = 427.62 ppm peak-then-decline; SSP2-4.5 = 590.815; SSP3-7.0 = 826.34; SSP4-6.0 = 631.473; SSP5-8.5 = 1092.59; all within IPCC AR6 / Friedlingstein 2025 GCB ranges). Both forks use intermediary_py adapter outputs (NOT legacy IIASA) + CMIP6 MRI-ESM2-0 GCM patterns (`imogen/patterns/CEN_CMIP6_MOD_MRI-ESM2-0/`) + B39 Law Dome 1900 init seeds. **B61 ✅ CLOSED** at this block (climatemodel.cpp non-byte-identity resolved). **Rule #9 datapoints**: now at #22 (added #20 missing bootstrap + #21 trunk-runs imogen_intermediary.ins predecessor-era drift + #22 relative-path depth mismatch). **Rule #10 datapoint #24**: scope-vs-Installment-2 honest re-estimate (B61's ~263 LOC under-counted; actual block 8.2.4 ~1300 LOC = ~80% of LEDGER §1.2's ~2900-3100 LOC; residual ~1300 LOC for v1+ Backport Sprint). **Paper Methods §2.2 framing LOCKED IN** at `notes/PAPER_COMPLETION_AND_VALIDATION.md` §4.5.0: "Both Track 1 and Track 2 use `trunk_r13078` for the LPJG ecosystem state simulations; the C++ IMOGEN engine producing Track 2 climate library is the port in `forks/trunk_r13078/modules/`, brought to functional byte-identity with rebuild's engine port at block 8.2.4." Cumulative T_seq Installment-1 source-edit UNCHANGED at ~297 LOC; cumulative TRUNK-RELEVANT source-edit across both installments now ~1600 LOC. v1.0 % done ~97-99% (slight increment); calendar ~5-9 weeks UNCHANGED. Audit-evidence bundle: `_chat_artifacts/b8_2_4_trunk_engine_forwardport_2026-05-24/` (~10 files; ~3000 LOC). Tag candidate `v0.23.0-trunk-engine-forwardport-complete` reserved (annotated; engine-side Installment-2 slice LANDED). **POST-BLOCK-8.2.4 NEXT**: block 8.2.5 switchable-regrid-strategy wiring (δ-B Fortran + δ-B-variant TRUNK-CPP-ENGINE pipelines via FastRegrid + 4-cell smoke side-by-side; ~1.5-2 d) → block 8.3 cluster smoke (~0.5 d) → block 8.4 cluster production-config delta + two-track directories (~1-1.5 d) → blocks 8.5-8.7 + sessions 9-11 Track 2 cluster production + sessions 11-12 validation triad + paper writing + v1.0 GMD submission.

#### 1.7.8 (prior; POST-block-8.2 status update at 2026-05-23 early morning, session 10 day 1 close — Block 8.2 ✅ DONE — 5-SSP C++ engine library production + γ-physical Common-directory separation + paired runs/+trunk-runs/ scaffolding + production-grade .ins-config alignment + v1+ trunk-engine groundwork (deferred per Rule #10 datapoint #23 self-correction on climatemodel.cpp byte-identity); NEW B60 + B61 filed; **block 8.2.5 switchable-regrid-strategy wiring ACTIVE NEXT** with δ-B Fortran + δ-B-variant chained-FastRegrid C++ 4-cell smoke side-by-side acceptance)

**Block 8.2 ✅ DONE 2026-05-23 early morning session 10 day 1 close** (canonical landing record at `_chat_artifacts/b8_2_engine_libraries_2026-05-22/B8_2_engine_libraries_evaluation_2026-05-22.md` ~250 LOC; 8-gate acceptance evaluation + Phase A-F summary + Rule #9 datapoints #17/#18/#19 + Rule #10 datapoints #22/#23 + B60+B61 NEW filing rationale). **5-SSP C++ engine library production** at `runs/<SSP>/Common-directory/IMOGEN/output/` (202 year-dirs 1900-2101 × 443 MB = ~2.2 GB total) via `scripts/run_coupled.sh --backbone intermediary-py --coupling-mode prescribed --scenario <SSP> --no-build --no-intermediary --engine-only-mode` (B44 productisation). All 5 use intermediary_py adapter outputs (RCMIP/CMIP6-backboned per Option B; Decision #1; legacy IIASA paths COMMENTED OUT). CO2 trajectories physically sensible per IPCC AR6 / Friedlingstein 2025 GCB across all 5 SSPs: SSP1-2.6 2100 = 427.62 ppm peak-then-decline ✅; SSP2-4.5 = 590.815 ✅; SSP3-7.0 = 826.34 ✅; SSP4-6.0 = 631.473 ✅; SSP5-8.5 = 1092.59 ppm ✅. **γ-physical Common-directory separation** (per user direction at session 10 day 1): all 5 `forks/trunk_r13078_runs/<SSP>/Common-directory/IMOGEN/` cp'd as physical directories (no symlinks; previous SSP1-2.6 symlink converted to physical cp; 4 new SSPs scaffolded + post-engine-production cp); additional ~2.2 GB at trunk-runs side; total ~4.4 GB engine libraries disk. **Phase F authored** NEW `forks/trunk_r13078_runs/<SSP>/main_engine_only.ins` × 5 + NEW `scripts/run_trunk_engine_only.sh` wrapper as v1+ groundwork (currently NOT USABLE at v1.0 due to climatemodel.cpp non-byte-identity ~263 LOC divergence per B61 NEW — trunk's older engine lacks B37/B44 path-iv done-marker mechanism + step-9.5 8-field writers; would produce inferior output if exercised separately; T_seq design per B47 §4 intentionally uses rebuild engine library = Phase E cp matches design). **Source-edits TRUNK-IRRELEVANT-by-novelty in entirety**: `tools/imogen_inputs_to_lpjg_format.py` SANITY_RANGES bump (Rule #9 datapoint #18 SSP5-8.5 peak ~131 GtCO2/yr per Friedlingstein 2025 GCB) + `runs/SSP1-2.6/main.ins` production-grade alignment + `forks/trunk_r13078_runs/SSP1-2.6/imogen_intermediary.ins` ssprcp typo fix (Rule #9 datapoint #17 pre-existing block-8.0.2 bug) + 64 NEW .ins files at `runs/SSP{2-4.5,3-7.0,4-6.0,5-8.5}/` + 72 NEW .ins files at `forks/trunk_r13078_runs/SSP{2-4.5,3-7.0,4-6.0,5-8.5}/` + 5 NEW main_engine_only.ins + 1 NEW wrapper script. **Cumulative T_seq Installment-1 source-edit UNCHANGED at ~297 LOC** (block 8.2 additions are TRUNK-IRRELEVANT-by-novelty); **Installment-2 LEDGER §1.2 ~2900-3100 LOC accounting refined via B61 NEW**: ~263 LOC climatemodel.cpp engine-side delta now quantified as B61 sub-item (rebuild has step-7 polling guards + step-8 imogenoutput integration + step-9.5 8-field writers + step-17a engine writer fix + step-17a skip_inprocess_engine_run + B19/B37/B39/B44/B45 deltas; trunk's is pre-rebuild-deltas baseline). Rule #9 datapoints #17/#18/#19 (pre-existing block-8.0.2 ssprcp typo + SANITY_RANGES upper bound + climatemodel.cpp non-byte-identity); Rule #10 datapoints #22/#23 (Option A→B→C-hybrid pivot honest framing + climatemodel.cpp byte-identity self-correction). v1.0 % done UNCHANGED at ~96-98%; calendar UNCHANGED at ~5-10 weeks to v1.0 GMD submission.

**POST-BLOCK-8.2 NEXT**: (i) block 8.2.5 switchable-regrid-strategy wiring (~1.5-2 d focused work): Fortran engine settings alignment at `imogen/code/imogen_settings.txt` (REGRID=.TRUE. + NGPOINTS=3698 + B39 init values + intermediary_py adapter input paths) + clone `version_B/FastRegrid/` → `tools/FastRegrid/` + extend `file_types` vector for all 10 climate variables (~5 LOC per block 8.1.5 §3) + δ-B Fortran-engine standalone × 5 SSPs + δ-B FastRegrid IDW 3698→62892 + δ-B-variant chained FastRegrid NN 1631→3698 + IDW 3698→62892 (Option C-hybrid; no in-engine C++ REGRID port at v1.0 per Option C-hybrid + B59 advancement) + 4-cell smoke side-by-side acceptance (trunk-T_seq LPJG × both pipelines) + user picks ONE for paper Track 2 cluster production; tag candidate `v0.24.0-switchable-regrid-strategy-complete`. (ii) block 8.3 cluster end-to-end smoke (~0.5 d; first cluster runtime test); tag candidate `v0.23.0-cluster-trunk-tseq-smoke-complete`. (iii) block 8.4 production-config delta authoring (~1-1.5 d; parallel `_fortranengine/` + `_cppengine/` 12 run-dirs). (iv) blocks 8.5-8.7 + sessions 9-11 Track 2 cluster production runs + sessions 11-12 validation triad + paper writing + v1.0 GMD submission (~5-10 weeks calendar). **PRIOR ENTRY** (session 10 day 1 cascade-gap-fill 2026-05-22 evening):

#### 1.7.8 What's next (POST-session-10-day-1-cascade-gap-fill + NEW B59 status update at 2026-05-22 evening — Block 8.1.5 cascade-gap-fill closes 3 doc gaps from block 8.1.5 commit `184a5007` (Opus-4.6-downgrade-truncation per user prompt §13 flag) + NEW B59 filed capturing δ-B-variant C++-engine-pipeline alternative to δ-B Fortran-engine-pipeline; **block 8.2 engine library completion for 4 remaining SSPs ACTIVE NEXT** with block 8.2.5 switchable-regrid-strategy wiring + δ-B + δ-B-variant 4-cell smoke comparison + side-by-side acceptance + user pick-one-for-paper-cluster-production next)

**Session 10 day 1 cascade-gap-fill + NEW B59 LANDED 2026-05-22 evening** (this commit; doc-only; ZERO source change; 8 in-tree doc surfaces). Two-part scope: (Part A) closes 3 cascade gaps from block 8.1.5 commit `184a5007` per user-flagged Opus-4.6-downgrade concern: `notes/CLUSTER_SETUP_AND_PRODUCTION_RUNS.md` §1 missing post-8.1.5 block ordering (block 8.2 + 8.2.5; NEW POST-BLOCK-8.1.5 OPERATIONAL ORDERING subsection prepended above existing BLOCK 8.1 LANDED subsection per Rule #10 amendment-vs-rewrite corollary; includes 8-row naming-convention table); `forks/README.md` missing switchable-regrid-strategy section (NEW 5-option summary + naming-convention table); `forks/trunk_r13078_runs/README.md` missing δ-B / δ-B-variant pipeline mentions (NEW ✅ banner + 12-run-dir block-8.4 restructure layout). (Part B) **NEW B59 δ-B-variant decision filing** per user direction at session 10 day 1 ~18:40-18:56 evening exchange: build BOTH **δ-B (Fortran engine `imogen/code/imogen_lpjg.f` native @ 3698 → FastRegrid IDW → 62,892)** AND **δ-B-variant (C++ port `lpjguess/modules/climatemodel.cpp::RUN_IMOGEN_ENGINE()` native @ 1631 → FastRegrid IDW → 62,892)** pipelines at block 8.2.5 acceptance test + side-by-side 4-cell smoke comparison + user picks ONE for paper main Track 2 cluster production runs based on smoke results + operational maneuverability for v1+ live-coupling trajectory. **Naming convention**: `forks/trunk_r13078_runs/integrated_tseq_<scenario>_wpeat_fortranengine/` for δ-B + `_cppengine/` for δ-B-variant. **No C++ engine source-edit needed for δ-B-variant** (FastRegrid handles all post-engine regridding externally; vestigial `bool regrid` + `NGPOINTS=3698` constants stay v1.1+ scope per B54 + B56; independent of B59). **Scientific value**: validates C++ port against Fortran engine (per `climatemodel.cpp:2` "fairly a direct translation"); near-identical smoke outputs → empirical equivalence demonstrated; meaningful divergence → Rule #9 datapoint surfaces engine implementation difference worth investigating. **Effort impact**: block 8.2.5 +3-4h analyst + block 8.4 +2h analyst + block 8.3 cluster smoke +0.5h cluster wall = ~5-6h analyst + 0.5h cluster wall extra; well within v1.0 ~5-10 week envelope. Cumulative T_seq Installment-1 source-edit UNCHANGED at ~297 LOC (block 8.1.5 + this commit are doc-only). Rule #10 datapoint #21. **POST-SESSION-10-DAY-1-CASCADE-GAP-FILL NEXT**: (i) block 8.2 engine library completion for 4 remaining SSPs SSP2-4.5+SSP3-7.0+SSP4-6.0+SSP5-8.5 via `--engine-only-mode` on workstation (~0.5 d wall; ~40 min if 4-way parallelized in terminal tabs); produces 5 × ~443 MB C++ engine libraries at `runs/<SSP>/Common-directory/IMOGEN/output/<year>/`; feeds δ-B-variant directly + provides Option α baseline-set; (ii) block 8.2.5 switchable-regrid-strategy wiring (~1.5-2 d focused work): Fortran engine settings alignment + clone `version_B/FastRegrid/` into `tools/FastRegrid/` + extend `file_types` vector for all 10 climate variables (~5 LOC) + Fortran-engine@3698 smoke + FastRegrid IDW 3698→62,892 smoke (δ-B) + FastRegrid IDW 1631→62,892 smoke (δ-B-variant) + 4-cell end-to-end δ-B + δ-B-variant acceptance tests + side-by-side comparison + user picks ONE for paper Track 2; (iii) block 8.3 cluster end-to-end smoke (~0.5 d; SCP chosen pipeline's engine library workstation → cluster + first cluster runtime test; tag candidate `v0.23.0-cluster-trunk-tseq-smoke-complete`); (iv) block 8.4 production-config delta authoring (~1-1.5 d; parallel `_fortranengine/` + `_cppengine/` 12 run-dirs; chosen pipeline used for cluster production + unchosen kept as v1+ alternative); (v) blocks 8.5-8.7 cluster MPI pre-flight + Track 1 baseline + intermediate smoke; (vi) sessions 9-11 Track 2 cluster production runs single run-set with chosen pipeline; (vii) sessions 11-12 validation triad + paper figures; (viii) v1.0 GMD paper submission (~5-10 weeks calendar from this commit). **PRIOR ENTRY** (block 8.1 close 2026-05-21 evening + block 8.1.5 close 2026-05-22 afternoon):

#### 1.7.8 What's next (POST-block-8.1 status update at 2026-05-21 evening session 9 day 2 close — Block 8.1 ✅ DONE — Cluster reconnaissance under T_seq retargeting fully complete: all 5 acceptance gates G0-G4 PASS; KIT IMK-IFU `owl` ready for T_seq Track 2 production runs; **block 8.3 cluster end-to-end smoke test ACTIVE NEXT** with block 8.2 cluster build verification mostly absorbed into G2+G3 already)

**Block 8.1.5 architectural clarification 2026-05-22 afternoon session 9 day 3** — IMOGEN engine identity + REGRID semantics + switchable-regrid-strategy (β/δ-A/δ-B) adopted for v1.0 paper production runs. Systematic investigation (8 items) revealed v1.0 production-IMOGEN = C++ port `climatemodel.cpp::RUN_IMOGEN_ENGINE()` (not Fortran as some docs implied); REGRID dead code in C++ port; predecessor architecture = IMOGEN@3698 + LPJG@62892 (departure from current α path); switchable-regrid-strategy adopted with **Option δ-B (predecessor architecture; engine→3698 + FastRegrid IDW→62892 + LPJG@62892) as v1.0 expedited path** (~1.5-2 d; all components operational). 6 NEW B-rows filed (B53-B58). Block plan revised: NEW block 8.2.5 for Option δ-B wiring. ZERO source change; doc-only. Full evidence at `_chat_artifacts/b8_1_5_architectural_clarification_2026-05-22/B8_1_5_architectural_clarification_findings_2026-05-22.md` (390 LOC; 18 sections).

**Block 8.1 ✅ DONE 2026-05-21 evening session 9 day 2 close** (no separate canonical landing record; comprehensive evidence at `_chat_artifacts/b8_1_cluster_reconnaissance_2026-05-21/B81_cluster_reconnaissance_evaluation_2026-05-21.md` + `B81_AGENDA_2026-05-21.md` ~+975 LOC combined; +8 supporting log/Markdown files with bundle total 2328 LOC / 162 KB / 10 files). **Cluster reconnaissance under T_seq retargeting fully complete**: all 5 acceptance gates G0-G4 ✅ PASS via 7 rounds of iterative SSH paste-back + 1 local mirror rsync of `/bg/data/lpj/bampoh-d/landsymm_imogen_runs/` to `/media/bampoh-d/landsymm_imogen_runs_cluster_mirror_2026-05-21/` (121 MB; canonical reference for direct file inspection per Rule #11 + #13). **Test workflow**: 7 round-trip user-SSH-paste-back iterations covering: round 1 G0 bash env (AlmaLinux 9 + Spack 0.19.0 + 7 auto-loaded modules in user's `.bash_profile`); round 2 G1 cluster discovery (11 partitions inventoried; canonical production target = `genius × 2 nodes × 128 CPUs/node = 256 ranks × 3-day walltime` matching user's wpeat sacct + setup_run.sh; milan as best-currently-idle alternative; cclake as legacy fallback; site-wide newer orchestrator at `/bg/data/lpj/scripts/setup_run_owl_with_scratch_lpj_work.sh` Jan 21 2026 with improved gridlist split + `srun --label` improvements); round 3 G4 partial input path verification; round 4 NEW `landsymm_lu/` tree structure discovery (CORRECTED my earlier framing of `landsymm_imogen/SSP1_RCP26/` as canonical reference); rounds 5-7 G4 ✅ full verification + sub-task 5.2.0 project mirror clone via GitHub HTTPS at `/bg/data/lpj/bampoh-d/lpj-guess_imogen_landsymm/` (63 MB; HEAD matches workstation `9561f1e6…`; tag `v0.22.0-tseq-installment-1-complete` present) + sub-task 5.2.2 trunk fork cluster build (`forks/trunk_r13078/build_owl/guess` 2,594,392 bytes sha1 `40d36db64e5eaa2f10ff1b9a1cdab954ea57c0b5`; cmake 1.88s + make 17.67s wall login node) + sub-task 5.2.3 lpjguess fork cluster build (`lpjguess/build_owl/guess` 2,639,744 bytes sha1 `4da4462a9f6be5bc433830f1f0b003b2f14f8b38`; same Spack-managed dep chain) + rsync inspection enabling comprehensive cross-SSP _wpeat .ins-set verification + ndep fallback strategy confirmation. **B48 hypothesis CONFIRMED Ubuntu-specific** (cluster builds succeeded WITHOUT `-DCMAKE_EXE_LINKER_FLAGS="-lcurl"` workaround; Spack-managed HDF5 1.12.2 + libcurl 7.85.0 + rpath chain is consistent; B48 priority STAYS LOW-MEDIUM as Ubuntu-only impact). **All 5 cluster input categories verified present**: soilmap + 4-NetCDF wet/dry NHx+NOy ndep (ssp126 + ssp370 + ssp585 SSP-specific variants present; SSP2-4.5 + SSP4-6.0 use `histsoc-wetdry-lpjguess/` 1850-2015 fallback already encoded in user's wpeat runs from 2026-03-14/15 production) + popdens + SimFire BLAZE + `_peatland` LU (hist + 5 scenarios). **8 substantive discoveries** (per Rule #9 datapoint #17; details in `B81_AGENDA_2026-05-21.md` §2): D1 B48-immunity + D2 canonical production resource allocation = genius/256 (NOT cclake/160 as 2023 template suggested) + D3 site-wide newer orchestrator + D4 HOME=/bg/home/ path-translation case-add needed at block 8.4 + D5 ndep fallback strategy pre-encoded + D6 Rule #10 self-correction on canonical reference dir framing (mid-recon at round 6; user-corrected at ~6:04 PM; preserved per amendment-vs-rewrite corollary) + D7 cross-SSP _wpeat parallelism perfect (5 scenario dirs IDENTICAL 18 .ins file sets; 30-31 line deltas per scenario; clean copy-with-replace pattern for block 8.4 cluster main.ins authoring) + D8 NEW v1.1+ trajectory finding (IMOGENCXX C++ legacy backport from version_A/B as switchable-alternative to rebuild's improved Fortran IMOGEN per LEDGER §1.1 dual-fork policy applied to 4th component-pairing; **NEW B52 row filed at FOLLOWUPS** this commit). **Source-edit at this commit**: 1 file (`scripts/cluster/env_owl.sh` placeholder commented module-loads replaced with canonical 7-module load matching user's `.bash_profile` auto-load; ~20 LOC; for future maintainers who don't have user's bash profile). **Rule #9 datapoint #17 + Rule #10 at 18th consecutive datapoint** (5-gate evaluation cites concrete artifacts per gate; honest D6 self-correction with concrete user-evidence + amendment-vs-rewrite corollary applied; no goalpost-shifting). **Cumulative T_seq Installment-1 source-edit**: ~297 LOC (~277 LOC cumulative from blocks 8.0.1-8.0.3 + ~20 LOC env_owl.sh at block 8.1; Installment-2 still pending ~2900-3100 LOC at post-paper Backport Sprint per LEDGER §1.2; UNCHANGED). **v1.0 % done UNCHANGED at ~96-98%; calendar UNCHANGED at ~6-11 weeks** to v1.0 GMD submission. **POST-BLOCK-8.1 NEXT**: (i) block 8.3 cluster end-to-end smoke test (~0.5 d; SCP engine library workstation → cluster ~443 MB SSP1-2.6 + first cluster runtime test); (ii) block 8.4 production-config delta authoring (~1-1.5 d; cluster main_hist.ins + main_scen.ins templates mirroring user's canonical wpeat .ins-config pattern per `/media/bampoh-d/landsymm_imogen_runs_cluster_mirror_2026-05-21/integrated-4.1-ins2_landsymm_{hist,ssp126}_wpeat/` reference with cfx → imogencfx swap + skip_inprocess_engine_run=1 + restructure `forks/trunk_r13078_runs/` to mirror cluster naming convention + T_seq retargeting of `scripts/cluster/setup_run.sh` + `run_coupled.sbatch` per agenda §3.1+3.2+3.6 reconciliation points); (iii) blocks 8.5-8.7 cluster MPI pre-flight + Track 1 baseline cluster runs + (optional) intermediate-cell production smoke; (iv) sessions 9-11 Track 2 cluster production runs (5 SSP-RCPs × 62538 cells × 1900-2100; ~5-15 hours total cluster wall on genius/256); (v) sessions 11-12 validation triad + paper figures per `notes/PAPER_COMPLETION_AND_VALIDATION.md` §1; (vi) v1.0 GMD paper submission (~6-11 weeks calendar from this commit). **PRIOR ENTRY** (block 8.0.3 close 2026-05-20 late afternoon):

#### 1.7.8 What's next (POST-block-8.0.3 status update at 2026-05-20 late afternoon session 8.0.3 close — T_seq Installment-1 acceptance test ✅ PASS; tag `v0.22.0-tseq-installment-1-complete` reserved for close commit; block 8.1+ cluster reconnaissance under T_seq retargeting ACTIVE NEXT)

**Block 8.0.3 ✅ DONE 2026-05-20 late afternoon session 8.0.3 close** (no canonical landing record; acceptance-test operational milestone; cumulative state captured in `_chat_artifacts/b47_tseq_acceptance_4cell_2026-05-20/B47_tseq_acceptance_evaluation_2026-05-20.md` ~250 LOC + this row + FOLLOWUPS dashboard + CHANGELOG + LEDGER §3 NEW entry + B47.md status note). **T_seq Installment-1 acceptance test PASS** — trunk-T_seq sequential-standalone workflow end-to-end validated at production-grade configuration. **Test workflow**: Step A (rebuild engine library pre-existing from B44 acceptance test 2026-05-19; 202 year-dirs 1900-2101) → Step B-equivalent (LOCAL symlink `forks/trunk_r13078_runs/SSP1-2.6/Common-directory → ../../../runs/SSP1-2.6/Common-directory`) → Step C (`forks/trunk_r13078/build/guess -input imogencfx main.ins` from `forks/trunk_r13078_runs/SSP1-2.6/`). **Test parameters**: 4-cell smoke gridlist `gridlist_test480.txt` × 5 patches × nyear_spinup=100 + freenyears=50 + firsthistyear=1900 + lasthistyear=1930 (extended from original 1903 plan after NEW B49 storage-formula constraint surfaced; see below) with full production knobs (BLAZE + LandSyMM full crop pipeline + 4-NetCDF wet/dry NHx+NOy ndep + popdens + `_peatland` LU + ifcalccton/sla 1 per B12). **Test result**: ~27 sec wall; 3 of 4 cells fully simulated 1900-1930 (cell 1 NW Canada Arctic 76°N + cell 3 Central Asia 54°N + cell 4 Patagonia 33°S complete; cell 2 NW Russia high-Arctic 80°N gracefully skipped per benign HILDA+ v2 LU coverage edge case at 80°N — would not occur with production-gridlist `gridlist_in_62892_and_climate.txt` which excludes such cells by construction). **8-acceptance-gate evaluation (per B47 §4.3)**: G0 ✅ PASS (build sha1 `08ac61a08c5f37046122fa9e4c4581547a626b1e`); G1 ✅ PASS (git HEAD `4d6fc4947da43ae4cc18e605fb5d214532dfc94d` block 8.0.2 commit); G2 ✅ PASS (engine library intact 202 year-dirs T_anom.dat 246,281 bytes); G3 ⚠️ PARTIAL 3/4 (benign cell-2 LU-coverage edge case); G4 ⚠️ PARTIAL 2 ERROR lines (both = cell-2 G3 LU-mismatch corollary; zero T_seq-mechanism ERRORs); G5 ✅ PASS (8-field B4 wiring + storage constraint satisfied post-lasthistyear=1930 fix; zero OOB markers); G6 ✅ PASS (per-year CO2.dat consumed via trunk existing pattern ZERO LOC backport; CO2 trajectory 1900=295.844 ppm → 1930=300.06 ppm matches B44 engine output exactly); G7 ✅ PASS (cflux.out NEE ±0.05 kgC/m²/yr near-balance + cmass.out per-stand-type partitioning + ngases.out N2O_soil 1.36 + NH3_soil 5.65 at Patagonia agricultural cell production-grade N-cycle). **Overall: 6 of 8 ✅ PASS + 2 ⚠️ PARTIAL (both G3+G4 benign cell-2 LU-coverage)**. **3 latent defects surfaced + addressed at block 8.0.3 (Rule #9 datapoint #16)**: (i) duplicate `import "landcover.ins"` in main.ins (block 8.0.2 authoring bug; surfaced via `landcover.ins:103 "crop_management": Duplicate group name` parser error at first launch attempt; fixed by removing direct import — transitive chain `main.ins → crop_n.ins → crop.ins → landcover.ins` is canonical pattern); (ii) hygiene-cleanup of leftover Windows-style paths at imogen_intermediary.ins:121-128 (block 8.0.2 missed at that pass; functionally dormant per .ins parser "latest-param-wins" semantics; visually confusing for future maintainers); (iii) **NEW B49 filed** — latent storage-array sizing constraint `lasthistyear ≥ FIRST_SPINUP_YEAR + NYEAR_SPINUP - 1 = 1929` in trunk's `imogencfx.cpp::init()` (block 8.0.3's FIRST_SPINUP_YEAR=1900 from surgical site (h) + initial main.ins lasthistyear=1903 → nyears=4 < 30 = OOB on spinup_year_idx=4 → SEGFAULT in first foreground launch attempt; operational workaround `lasthistyear 1903 → 1930` (nyears=31 ≥ 30); proper long-term fix B49 = parametrize FIRST_SPINUP_YEAR as .ins (per B45 framework) + storage formula MAX-fix `nyears = max(lasthistyear - FIRST_SPINUP_YEAR + 1, NYEAR_SPINUP);` in trunk + rebuild's imogencfx.cpp::init() (~10-25 LOC; LOW-MEDIUM priority; v1.1+ era; TRUNK-RELEVANT to BOTH forks). **Block 8.0.3 source-edit additions beyond block 8.0.2**: +1 LOC source change at `forks/trunk_r13078/modules/imogencfx.h:224` (FIRST_SPINUP_YEAR 1871 → 1900; surgical site (h) added to B47 §4.2) + ~25 LOC comment block citing block 8.0.3 + B47 §4.2 amendment + NEW B49 cross-reference + ~30 LOC main.ins amendments (duplicate-import fix + lasthistyear 1903→1930 + acceptance-test gridlist override) + ~25 LOC imogen_intermediary.ins hygiene cleanup. **Cumulative T_seq Installment-1 source-edit**: ~277 LOC (close to upper end of B47 §0 estimate range 180-310). **Audit-evidence bundle at `_chat_artifacts/b47_tseq_acceptance_4cell_2026-05-20/`**: 10 files (guess.log + b803_acceptance_run.log + 5 representative *.out files cflux/anpp/cmass/ngases/mch4 + main.ins + imogen_intermediary.ins snapshots + 8-gate acceptance evaluation Markdown ~250 LOC with full per-gate evidence + physical sensibility tables). **Tag `v0.22.0-tseq-installment-1-complete`** reserved for the block 8.0.3 close commit (annotated tag marking T_seq Installment-1 full operational milestone — first time any LPJG main loop in any fork successfully consumes a pre-baked engine library end-to-end; pre-T_seq this was blocked by F-10 deadlock in rebuild's `lpjguess/` + `exit(200)` regression in `forks/trunk_r13078/`). Rule #10 at 17th consecutive datapoint (8-gate evaluation cites concrete artifacts per gate; honest PARTIAL framing for cell-2 LU-mismatch; 3 latent-defect honest disclosures; no goalpost-shifting). **POST-BLOCK-8.0.3 NEXT**: (i) block 8.1+ cluster reconnaissance under T_seq retargeting (covers trunk-build module dependencies for `forks/trunk_r13078/build_owl/` alongside rebuild's `lpj-guess_imogen_landsymm/build_owl/`; engine sbatch + trunk-T_seq sbatch patterns); (ii) block 8.4+ full-1900-2100 production-config delta authoring (extend trunk-Track-2 main.ins from acceptance window 1900-1930 to full historical+scenario 1900-2100 with save_state/restart pattern per Decision #10; predecessor's `integrated-4.1-ins2_landsymm_{hist,ssp126}` historical/scenario split provides the canonical pattern; ~3-5 .ins files per SSP × 4 SSPs); (iii) sessions 9-11 Track 2 cluster production runs (5 SSP-RCPs × full 62538-cell × 1900-2100; trunk-T_seq on `owl` with MPI parallelization); (iv) sessions 11-12 validation triad + paper figures per `notes/PAPER_COMPLETION_AND_VALIDATION.md` §1 (4-axis validation); (v) v1.0 GMD paper submission (~6-11 weeks calendar from this commit). **PRIOR ENTRY** (block 8.0.2):

**Block 8.0.2 ✅ DONE 2026-05-20 late afternoon session 8.0.2 close** (no canonical landing record; source-edit + .ins authoring operational milestone; cumulative state captured in `forks/trunk_r13078_runs/README.md` + this row + FOLLOWUPS dashboard + CHANGELOG + LEDGER §3 NEW entry). Per T_seq Installment-1 plan from B47, applied **251 LOC C++ source-edit across 4 files** in `forks/trunk_r13078/` (modules/imogencfx.cpp +208 / modules/imogencfx.h +29 / framework/parameters.cpp +11 / framework/parameters.h +15) implementing all 6 surgical sites per B47 §4.2: (a) `exit(200);` regression removed at `imogencfx.cpp:483` (matches LandSyMM_LPJ-GUESS upstream state per LEDGER §2 6-file-delta correction); (b) `IMOGENConfig::skip_inprocess_engine_run` parameter declared (default `false`) with declare_parameter call; (c) `RUN_IMOGEN_ENGINE();` wrapped in `if (!skip)` else `dprintf("skipped")` block; (d) `file_relhum`+`file_wind`+`file_tmin`+`file_tmax` parameter declarations + storage members + all_d* 3D vectors added; (e) `readenv()` extended for 4 new per-year ASCII reads via existing `read_lines_from_file` machinery (identical signature parity per U1 pre-flight verification); (f) `get_climate_for_gridcell()` extended for Rh/W/Tmin/Tmax monthly+daily branches + K→C conversion for dtemp/dtmin/dtmax (latent K-vs-C bug that didn't surface pre-T_seq because F-10 deadlock blocked LPJG main loop in trunk) + restored-and-reframed BLAZE compatibility check at start of monthly branch + climate.relhum/u10/tmin/tmax assignments in getclimate(); (g) per-year CO2.dat reader ZERO LOC needed (trunk's existing `co2.load_file(gen_filename(..., imogen_year, false))` pattern already supports YYYY-templated per-year ASCII reads). Build verification ✅ PASS post-edit: `cmake -DCMAKE_EXE_LINKER_FLAGS="-lcurl" .. && make -j$(nproc)` produces working `guess` binary at 2,920,856 bytes (+7,968 bytes vs pre-edit 2,912,888 byte block-8.0.1 baseline; matches expected B4 wiring + skip-flag + comments delta). NEW trunk-Track-2 `.ins` set authored at `forks/trunk_r13078_runs/SSP1-2.6/`: 19 files (NEW README.md ~190 LOC documenting T_seq workflow + local vs cluster path overrides + block 8.0.3 plan; NEW main.ins ~165 LOC for T_seq imogencfx + skip_inprocess_engine_run + production knobs + B12-defensive ifcalccton/sla compliance + `_peatland` LU; modified imogen_intermediary.ins [Windows→Linux paths + skip_inprocess_engine_run 1 + B39 296.1/875.6/277.4 Law Dome 1900 seeds]; 16 cp'd predecessor `.ins` files from `lpjg_landsymm_integration/integrated-4.1-ins2_landsymm_hist/` production-grade for trunk_r13078 from Track 1 runs). Cumulative backport-debt: Installment-1 LANDED at ~251 LOC C++ source-edit; Installment-2 still pending ~2900-3100 LOC at post-paper Backport Sprint per LEDGER §1.2. B48 workaround still required. Rule #10 at 16th consecutive datapoint. **POST-BLOCK-8.0.2 NEXT**: block 8.0.3 local 4-cell smoke acceptance test (~0.5-1 d): pre-run setup symlink `cd forks/trunk_r13078_runs/SSP1-2.6/ && ln -s ../../../runs/SSP1-2.6/Common-directory ./Common-directory` (the engine library produced by B44 acceptance test 2026-05-19) + adjust gridlist override to rebuild's 4-cell `../../../data/gridlist/gridlist_test2.txt` + invoke `../../trunk_r13078/build/guess -input imogencfx main.ins` → 8-acceptance-gate evaluation G0-G7 per B47 §4.3 → audit-evidence bundle at `_chat_artifacts/b47_tseq_acceptance_4cell_2026-05-20/` → tag candidate `v0.22.0-tseq-installment-1-complete` after ✅ PASS. **PRIOR ENTRY** (block 8.0.1):

**Block 8.0.1 ✅ DONE 2026-05-20 afternoon session 8.0.1 close** (no canonical landing record; structural import + build verification operational milestone; cumulative state captured in `forks/README.md` + this row + FOLLOWUPS dashboard + CHANGELOG). Per T_seq Installment-1 plan from B47, brought `trunk_r13078` LPJG fork into rebuild repo via `rsync -a --exclude='build_*' --exclude='.vscode' --exclude='*.o' --exclude='guess' --exclude='*.log' --exclude='qsat_output.txt' --exclude='.git' version_A/LPJG-IMOGEN-COUPLED-MODEL-FRAMEWORK/Integrations/trunk/trunk_r13078/ forks/trunk_r13078/`; 23 MB / 341 source files; ZERO source-edit at import. NEW `forks/README.md` (~115 LOC) authored documenting two-fork policy + dual-fork long-term trajectory (rebuild's `lpjguess/` = primary active-development surface; `forks/trunk_r13078/` = backport fork eventually brought to full fork-parity at Installment-2 as switchable alternative not replacement per `notes/TRUNK_R13078_BACKPORT_LEDGER.md` §1.1) + build instructions (with NEW B48 `-DCMAKE_EXE_LINKER_FLAGS="-lcurl"` workaround) + operational invocations (rebuild engine standalone Step A via `scripts/run_coupled.sh --engine-only-mode`; backport-fork LPJG against pre-baked library Step C via `forks/trunk_r13078/build/guess -input imogencfx`) + cross-references. **Baseline build verification ✅ PASS**: `cd forks/trunk_r13078 && mkdir build && cd build && cmake -DCMAKE_EXE_LINKER_FLAGS="-lcurl" .. && make -j$(nproc)` produces working `guess` binary (2912888 bytes; runs cleanly emitting standard `Usage: ./guess [-parallel] [-input <module_name>...]` line). **NEW B48 filed** — system-packaging issue (Ubuntu 24.04+ `libhdf5-310` 1.14.5+ has transitive `libcurl@CURL_OPENSSL_4` symbol dependency that neither fork's `CMakeLists.txt` auto-resolves); affects BOTH `lpjguess/` (rebuild fresh build also fails identically; pre-built May-16 binaries still work because linked before system update) AND `forks/trunk_r13078/`; workaround documented; proper fix `find_package(CURL REQUIRED)` + `target_link_libraries(guess PRIVATE CURL::libcurl)` in BOTH forks' `CMakeLists.txt` ~10 LOC total; LOW-MEDIUM priority; TRUNK-RELEVANT to BOTH forks. Rule #9 datapoint #15 (baseline build verification surfaced latent B48 defect); Rule #10 at 15th consecutive datapoint (binary-size byte-exact match to pre-built rebuild proves code-unchanged-only-link-affected; baseline-build claim cites concrete cmake+make invocation + binary file output). **POST-BLOCK-8.0.1 NEXT**: block 8.0.2 source-edit (~180-310 LOC C++ in `forks/trunk_r13078/modules/imogencfx.{cpp,h}` per `notes/B47.md` §4.2 surgical sites) + trunk-Track-2 `.ins` authoring at `forks/trunk_r13078_runs/SSP*/` (structure-decision at 8.0.2 start) → block 8.0.3 local 4-cell smoke acceptance test 8-gate evaluation. Cumulative backport-debt accounting: Installment-1 ~180-310 LOC remains pending (block 8.0.2) + NEW potential B48 ~10 LOC bumps Installment-1 + LANDSYMM-upstream contributions when implemented; UNCHANGED through block 8.0.1 since structural import has zero source-edit. **PRIOR ENTRY** (B47 decision-record):

**B47 ✅ DONE (decision-recorded) 2026-05-20 afternoon session 8.0 opening** (canonical landing record at `notes/B47.md` ~400 LOC; ZERO source change at this decision-record commit; implementation work scheduled at blocks 8.0.1-8.0.3; cumulative backport-debt accounting refined into two-installment trajectory: Installment-1 ~180-310 LOC pre-paper to `forks/trunk_r13078/` + Installment-2 ~2900-3100 LOC post-paper for full fork-parity + rebuild→LANDSYMM_LPJ-GUESS-upstream direction UNCHANGED at +145 LOC). Per session 8.0 C0-C5 verification + U1-U4 pre-flight reconnaissance, **Option T_seq adopted** as the C5 strategic decision: bring `trunk_r13078` into rebuild repo at `forks/trunk_r13078/` (sibling to `lpjguess/`) at block 8.0.1; apply minimal source-edit ~180-310 LOC C++ (skip_inprocess_engine_run flag + B4 8-field consumer wiring for Rh/W/Tmin/Tmax + per-year CO2.dat reader option-α + 1-line `exit(200)` regression removal) at block 8.0.2; local 4-cell smoke acceptance test 8-gate evaluation at block 8.0.3. **Workflow is sequential standalone**: Step A — rebuild's IMOGEN engine standalone per SSP via `scripts/run_coupled.sh --engine-only-mode` (B44 productised) producing 8-field climate + per-year CO2 + non-CO2 atm-conc library; Step B — `scp -r` library to cluster (~2.5 GB); Step C — `forks/trunk_r13078/build/guess -input imogencfx main.ins` runs on cluster as pure pre-baked-library consumer using legacy `owl_hpc_cluster_scripts/scripts/` pattern. **Two-fork long-term trajectory** clarified at session 8.0: `lpjguess/` = primary active-dev for v1.0+; `forks/trunk_r13078/` = backport fork brought to bare-minimum Track-2-runnable state at Installment-1 + eventually full fork-parity at post-paper Backport Sprint Installment-2 (~2900-3100 LOC remaining) so both forks serve as switchable alternatives (alternative not replacement) per `notes/TRUNK_R13078_BACKPORT_LEDGER.md` §1.1 two-fork policy. **POST-B47 NEXT**: block 8.0.1 structural import of `forks/trunk_r13078/` (~0.5 d; user-runs-commit + 3-remote push per §I.5 of session 8 prompt) → block 8.0.2 source-edit + .ins authoring (~1-1.5 d) → block 8.0.3 acceptance test (~0.5-1 d) → block 8.1 cluster reconnaissance now also covers trunk-build module dependencies for `forks/trunk_r13078/build_owl/` (alongside rebuild's `build_owl/`). Total estimated calendar time from this commit to v1.0 GMD paper submission: **~6-11 weeks** (revised from ~6-10 weeks by adding T_seq's ~0.5-1 week; still within original budget envelope per `notes/PRODUCTION_RUN_CONFIG.md` §6.2). Rule #9 + Rule #10 at 14th consecutive datapoint each. **PRIOR ENTRY** (B44):
