...
## 🔧 2026-06-07 (session 18 follow-up) — SHORTWAVE `MAX(...,0)` floor applied to BOTH trees + Fortran (NO backport debt)

The field reconstruction floored precip/RH/DTEMP/wind but not shortwave — an original-Fortran characteristic (`imogen/code/imogen_lpjg.f::CLIM_CALC`) faithfully carried into the verbatim C++ port (`modules/climatemodel.cpp`; the unfloored `sw_daily` line is from the original import `f2405da`, NOT from the oceanfix). Exposed by the CRU-JRA baseline (near-zero polar-winter SW) + negative SW anomaly → tiny negative absolute SW that LPJ-GUESS `interp_monthly_means_conserve` rejects. **FIX applied fork-symmetrically** (one line each, mirroring the sibling floors): `lpjguess/modules/climatemodel.cpp`, `forks/trunk_r13078/modules/climatemodel.cpp`, AND `imogen/code/imogen_lpjg.f` — preserving the engine byte-identity (LEDGER §1); **no new backport debt**. Binaries rebuilt: `forks/trunk_r13078/{build,build_b824,build_oceanfix}` + `lpjguess/{build,build_mpi}` + the Fortran `imogen/code/imogen_lpjg`. The existing v1.0 library was brought to non-negative SW by an equivalent post-hoc clamp (`scripts/clamp_sw_anom_nonneg.py`; numerically identical to re-running, since the per-month field is day-invariant). Magnitude negligible (mean-rsds shift ≤0.0011 W m⁻²) → paper unaffected. Full detail: `CHANGELOG.md` 2026-06-06 entry (7).
...
**Backport classification**: **ALL TRUNK-IRRELEVANT-by-fork-novelty**. No trunk fork source files touched. **Cumulative LEDGER §1 accumulators UNCHANGED at this addendum commit** (T_seq Installment-1 ~297 LOC + Installment-1.5 engine slice ~1300 LOC + Installment-2 v1+ residual ~1300 LOC + Fortran tree fork-shared ~156 LOC; FastRegrid C++ infrastructure ~1057 LOC; wrapper scripts ~542 LOC).

**Scope summary** (cluster scaffolding + rebuild-only operational infrastructure refinements; no trunk source touched):
- NEW `data/gridlist/gridlist_in_62892_and_climate_and_PLUMmask.txt` (62,512 cells; production gridlist ∩ PLUM SSP mask coverage; tracked rebuild-only data file)
- `scripts/cluster/setup_run_tseq_template.sh` (~25 LOC delta) + cp to 10 cluster `setup_run_tseq.sh` (rebuild-only cluster launcher template; allocation defaults switched from genius/2×128=256 to milan/8×64=512 with documented genius/4×128=512 interchangeability + WALLTIME default → 3-day + GRIDLIST default → PLUM-mask gridlist)
- 4 cluster SCEN main.ins (`SSP1-2.6, SSP3-7.0, SSP4-6.0, SSP5-8.5_cluster_scen`): state_path retargeted to `SSP2-4.5_cluster_hist/state/` per Track-1-style shared HIST decision (rebuild-only cluster .ins set; ~6 LOC inline annotation + 1 LOC path change per file)
- `notes/FOLLOWUPS.md` B64 NEW audit-item filed: **Hypothesis 2 ✅ RESOLVED** at workstation-agent verification 2026-05-29 ~01:00 CEST (provenance chain confirmed clean: anthropogenic emissions ARE intermediary_py-derived per Decision #1 Option B; legacy IIASA paths commented out + INERT; FILE_NON_CO2_VALS is CMIP6 non-CO2 RF time-series by intentional architectural design with CH4+N2O pass-through; methodologically clean for paper Methods §2.2; full provenance chain documented at `notes/PAPER_COMPLETION_AND_VALIDATION.md` §4.5.0a-cluster). **Hypothesis 1 ✅ CONFIRMED + REMEDIATION PLANNED** at workstation-agent diagnostic 2026-05-29 ~01:30 CEST (5-SSP RF cross-comparison at year 2000 → ssp126/245/370/585 = 1.133761 W/m² 4-way byte-identical canonical CMIP6 historical baseline vs ssp460 = 1.369924 W/m² ~21% higher synthetic monotonic ~2%/yr exponential growth, NOT real CMIP6 historical with Pinatubo signal; root cause = corrupted ssp460 historical 1850-2014 from CMIP6 Tier-2 SSP4-6.0 generation pathway gap; scenario period 2015-2100 reasonable; **5-step remediation recipe** ~30-45 min total wall on workstation: backup → splice → re-run engine SSP4-6.0 → re-FastRegrid → re-rsync 18 GB SSP4-6.0 climate library to cluster). Hypothesis 3 (GCM pattern data quirk) MOOT since Hyp 1 explains anomaly fully. **TRUNK-IRRELEVANT-by-novelty for `forks/trunk_r13078/` source-edit accumulators** (the corrupted file is a CMIP6-era data input file at `imogen/emiss/CMIP6/Non-Co2-CH4-N2O-RF/`; remediation is a workstation-side input-file fix + downstream re-run; no source-edit involved; not eligible for backport debt). **Production launch implications**: Phase 1 SSP2-4.5 HIST + Phase 3a 4-of-5 SCEN UNAFFECTED + can launch as planned; Phase 3b SSP4-6.0 SCEN deferred until corrected library rsync'd; workstation remediation overlaps cluster Phase 1 or 3a → 0 calendar-day delta on cluster side

**Doc cascade addendum** (7 surfaces tracked + 1 gitignored bundle): CHANGELOG + FOLLOWUPS dashboard banner amendment + B64 NEW row + this LEDGER §3 entry + EXECUTION_PLAN row 17c addendum marker + CLUSTER_SETUP §1 banner amendment + PRODUCTION_RUN_CONFIG banner amendment + PAPER_COMPLETION §4.5.0a-cluster (paper-ready Methods §2.2 text + per-SSP file accounting table + provenance chain ASCII diagram) + B8_3_evaluation §13 expanded (gitignored).

**POST-ADDENDUM NEXT — Track 2 production launch sequence** (revised post-Hyp-1-confirmation): Phase 1 (1× SSP2-4.5 HIST on milan/8×64=512 or genius/4×128=512; ~52h or ~38h wall) → Phase 2 (wait for `SSP2-4.5_cluster_hist/state/` populated) → Phase 3a (4-of-5 SCEN parallel-ready restart from shared HIST state: SSP1, SSP2, SSP3, SSP5; each ~10h wall) → Phase 3b (SSP4-6.0 SCEN AFTER workstation remediation completes + corrected SSP4-6.0 climate library rsync'd to cluster; ~10h wall). Total ~3-4 days cluster wall (workstation remediation ~30-45 min overlaps cluster Phase 1 or 3a → 0 calendar-day delta on cluster side; vs ~13 days for original 5 SSP-specific HIST plan).
