...
> **🎉 SESSION 18 CLUSTER RE-RUN (2026-06-08) — TRACK 2 PRODUCTION ✅ RE-DONE on CORRECTED climate (supersedes the 2026-06-03 set below).** After the 7 climate corrections (incl. the 6th/SW-floor), all 6 runs re-ran clean on **4 genius nodes** (weber-j freed them; not 8 milan): HIST (`SSP2-4.5_cluster_hist/output-2026-06-07/`, 6h08m, state 509/509) + 5 SCEN (`<SSP>_cluster_scen/output-2026-06-0[78]/`, ~1h20-28m ea., ~7.4-7.6 GB), all 62,512 cells, `COMPLETED` 0:0, **509 ranks** (#36/#37). 6th-correction validated: first attempt failed ~1977 on negative SW; after the SW-floor fix it ran clean past 1977→2020. Sensibility ✅ (now climate-responsive): cmass gain SSP1-2.6 +0.51 < SSP3-7.0 +0.60 < SSP2-4.5 +0.61 < SSP4-6.0 +0.68 < SSP5-8.5 +0.71 kgC/m² (CO₂-fertilization-vs-warming tradeoff; 2020 ~3.41 identical; 0 NaN/neg). Pre-run cleanup: work base wiped + state cleared + 69 GB superseded outputs removed. **⚠️ each output dir has a ~27 GB benign `warnings.txt` (loose-mode noise) → EXCLUDE from rsync.** NEXT: rsync corrected outputs (excl. warnings.txt) + state + `_chat_artifacts` → workstation. (Recompile note: HIST finishup went to milan and took ~3 h under weber's I/O contention — for future HIST re-runs send the finishup to genius via `APPEND_PARTITION=genius`.)
>
> **🎉 SESSION 13 DAY 5 FINAL (2026-06-03) — TRACK 2 PRODUCTION ✅ COMPLETE: HIST + ALL 5 SCENs banked + physically verified.** *(used the PRE-correction climate; SUPERSEDED by the 2026-06-08 re-run above)* All 5 SCENs launched at 509 ranks (per #37), each `COMPLETED` 0:0 in ~1.5 h at full 8-node load (#36+#37 validated 5×); SSP4-6.0 used B64-corrected lib. Outputs: HIST `SSP2-4.5_cluster_hist/output-2026-06-02/` (32 GB) + 5 SCEN `<SSP>_cluster_scen/output-2026-06-0[23]/` (~7.3-7.4 GB each, 62512 cells, 2020-2100). Sensibility: CO₂-fertilization gradient monotonic with CO₂ (SSP1-2.6<2-4.5<4-6.0<3-7.0<5-8.5); HIST→SCEN transition smooth (benign ~0.17% LU-switch offset at restart year); 0 NaN/neg. NEXT: rsync outputs + `_chat_artifacts` → workstation; workstation `git pull`; resume paper validation triad. (Original day-5 launch note preserved below for the 509-rank recipe.)
>
