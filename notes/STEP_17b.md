...
**C2 era combined sprint total** (this step 17b): C2 core (3-5 d ✅) + B1 (3-5 d ⏳) + **B2 (0.5 d ✅)** + **B3 (0.3 d ✅; close-by-architectural-reframing)** + **B4 (~0.5 d ✅ this commit; lighter than budgeted 1 d because direct mirror of IMOGENCFXInput pattern)** + B6 (0.0 d ✅, subsumed by B10) + B10 (0.5 d ✅) = **~7.3-11.3 days actual+pending** (revised down by an additional 0.5 d on 2026-05-12 night after B4 landed in ~0.5 d vs the budgeted 1 d).
...
3. **B1+B2+B3+B4+B6+B10 bundles (~4-6 d remaining) — IN PROGRESS** (now safe to land on substantively-validated baseline). Per user-confirmed Option A 2026-05-11, ordered B10 → B6 → B2 → B3 → B4 → B1 (mechanical-first; physics-port-last). Status:
   ...
   - **B1 ⏳ NEXT** (~3-5 d; heaviest, LAST in re-ordered sequence): Fortran Rh + Wind COMPUTATION port from C++ in-process engine to standalone Fortran (~70-100 LOC of Fortran physics). Post-B1: B4's wiring (this commit) consumes the live Fortran-engine Rh + Wind anomaly files directly without further code changes on the LPJG side.
...
**Revised C2 era combined sprint estimate**: **~4-7 days remaining** (was 5-8; B4 landed in this commit at ~0.5 d via direct mirror of C1.1 pattern, saving 0.5 d vs the budgeted 1 d). Remaining: B1 ~3-5 d (heaviest, last in sequence) + C2 close-out + mpirun -np 4 verification ~1-2 d.
