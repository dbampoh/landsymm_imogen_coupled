...
C Define basic timestepping variables
     ...
     &,IYEAR             !WORK Year we are at in the run
      ...
      LOGICAL
     & FILE_NON_CO2      !IN If true, then non-CO2 radiative forcings are
...
C Note that switching off INCLUDE_CO2 and INCLUDE_NON_CO2 is effectively
     ...
     &,C_EMISSIONS       !IN If true, means CO2 concentration is calculated
...
C                                  !name of the climate data directory
      ...
      REAL LATMIN_DAT,LATMAX_DAT,LONGMIN_DAT,LONGMAX_DAT     !Anomaly pa
     ...
     & KEEPRUNNING      !WORK Exit logical for while loop, provided by LPJ-GUESS - TP 29.07.15
      ...
      PARAMETER(NGPOINTS=3711) !NOTE: Needs to be hard-coded consistent with gridlist file - TP 06.08.15
...
CINQUIRE(FILE=TRIM(ADJUSTL(DIR_COMMON))//'/LPJG_main/IMOGEN/'//
C&    'done',EXIST=DONE_EXIST)
...
c Use line 452 for linux and line 453 for windows
cCALL SYSTEM('mkdir -p '//TRIM(ADJUSTL(DIR_COMMON))//'/IMOGEN/output/'//THISYEAR)
      CALL SYSTEM('mkdir '//TRIM(ADJUSTL(DIR_COMMON))//'\IMOGEN\output\'//THISYEAR)
...
C IMOGEN operated interactively.
        ...
        !If in the spin-up period then writing to CO2.dat is carried out earlier on in the program
          IF(IYEAR.EQ.YEAR1) THEN
cOPEN(91,FILE=TRIM(ADJUSTL(DIR_COMMON))//'/IMOGEN/output/'//THISYEAR//'/'//'CO2.dat') !Send to DIR_COMMON - TP 13.07.15
            OPEN(98,FILE=TRIM(ADJUSTL(DIR_COMMON))//'/IMOGEN/'//'CO2_all.dat',ACCESS='APPEND')
          ENDIF

          OPEN(91,FILE=TRIM(ADJUSTL(DIR_COMMON))//'/IMOGEN/output/'//THISYEAR//'/'//'CO2.dat')
...
C             Also want the number of wet days per month as an output
     ...
     &       ,LON_OUT,LAT_OUT,DIR_COMMON,THISYEAR,FILE_GRIDLIST)
            ...
            PRINT *, 'VALUE OF THIS YEAR: ',THISYEAR
            !PAUSE
cIF(IYEAR.EQ.YEAR1) THEN
...
!----------------------------------------------------------------------
     ...
     & CO2_RF_FAIR)
      ...
      CHARACTER(LEN=180) FILE_GRIDLIST  !file containing gridlist DKB 27/07/2022
